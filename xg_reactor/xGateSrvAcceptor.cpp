...
{
  CLog::Warning(THISMODULE, "xGateSrvAcceptor::xGateSrvAcceptor");
}
...
{
  CLog::Warning(THISMODULE, "Enter xGateSrvAcceptor::handle_connection Fd:%d",aceHandle);
  ...
  {
    CLog::Error(THISMODULE, "xGateSrvAcceptor::handle_connection Received"\
  ...
  if(MGC_MG_SIGNAL_PORT == remoteAddr.get_port_number()) {
    ...
    CLog::Warning(THISMODULE, "xGateSrvAcceptor::handleConnection registering" \
        " Connection with Mg-Receiver");
  }

  if(pReceiver == NULL || pReceiver->get_reactor() == NULL)
  {
    CLog::Error(THISMODULE, "xGateSrvAcceptor::handleConnection Failed" \
  ...
  CLog::Warning(THISMODULE, "Exit xGateSrvAcceptor::handle_connection sucessfully"\
