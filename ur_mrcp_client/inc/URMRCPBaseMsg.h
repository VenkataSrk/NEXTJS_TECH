#ifndef _URMRCP_BASE_MSG_H
#define _URMRCP_BASE_MSG_H
...
#include "../xg_iurmodule/IURModuleMsg.h"

class URMRCPBaseMsg : public ACE_Message_Block,public IURModuleMsg
{
  public:
    URMRCPBaseMsg(IURDefines::MODULE_ID srcModuleId) :
      m_srcModuleId(srcModuleId),
      m_dstModuleId(IURDefines::UR_MODULE_UNKNOWN),
      m_uid(""),
      m_opcode(0){};
    //DebugVM start
    /*URMRCPBaseMsg(xGateMsgType  type): m_msgType(type) {
      this->msg_priority((unsigned long)EN_XGATE_MSG_PRIORITY_LOW);
    }*/
    //DebugVM end
    virtual ~URMRCPBaseMsg(){};
    ...
    void setFeatureType(unsigned int featureType)
    {
   ...
   //DebugVM end
  protected:
    URMRCPBaseMsg& operator=(const URMRCPBaseMsg&);

  //private:
  public:
    // Not Implemented Methods
    URMRCPBaseMsg(void);
    URMRCPBaseMsg(const URMRCPBaseMsg&);
  private:
    //DebugVM start
