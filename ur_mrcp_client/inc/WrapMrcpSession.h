/*
	@Author: PraveenMS
	 Speech synthesis wrapper functions for unimrcp library
*/

#ifndef WRAP_MRCP_SESSION_H
#define WRAP_MRCP_SESSION_H
...
public:
	...
	virtual void ExitSession(WrapMrcpSession* pUmcSession) = 0;
};

class WrapMrcpSession : protected UmcSessionEventHandler
{
public:
	WrapMrcpSession(const MrcpCliAppSynthScenario* pScenario);
	virtual ~WrapMrcpSession();
  ...
  std::string FrameXml(std::string Content);
...
//protected:
	...
	const MrcpCliAppSynthScenario*          m_pScenario;
