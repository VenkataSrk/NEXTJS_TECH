...
import { sendMessageCustomer } from './meta';
...
// get appointment details
export async function AppointmentInfoPaymentCheck(data: any) {
  try {
    const body = {
      p_app_id: data?.appId,
      p_company_id: data?.companyId
    ...
    logger.error('AppointmentInfoPaymentCheck:', error);
  }
}
...
// get appointment
export async function getWorkingHours(appointmentId: any) {
  try {
    const body = {
      p_companyid: appointmentId?.companyId,
      p_user_id: appointmentId?.userId
    };
    const appointmentDate: any = await axios.post(process?.env?.getAppointmentDate, body);
    if (appointmentDate?.data?.status_code === 200) {
      return appointmentDate?.data?.finalresponse[0];
    }
    return [];

  } catch (error) {
    logger.error('getWorkingHours err:', error);
  }
}
...
export async function getdurationDetails(getAppointmentFlow: any) {
  try {
    ...
    logger.error('getdurationDetails err:', error);
  }
}
...
export async function getLeaveData(getAppointmentFlow: any) {
  try {
    ...
    console.error('getdurationDetails err:', error);
  }
}
...
export async function getCalenderViewDetails(appointment: any, getAppDetails: any, DurationType: any, getAppointmentFlow: any) {
  try {
    ...
    logger.error('getCalenderViewDetails err:', err);
  }
}
...
// get appointment
export async function bookingPayment(jsonString: any, getAppointmentFlow: any, getAppDetails: any, EndTime: any, typeValue: any, whatsappCcaasTable: any, FilterPayment: any) {
  const jdata: any = [];
  jsonString.forEach((item: any) => {
    const key = Object.keys(item)[0]?.replace(/\s+/g, '_');
    jdata.push(key);
  ...
  try {
    const body = {
      ...
      calledNumber: whatsappCcaasTable?.calledNumber,
      customerNumber: whatsappCcaasTable?.customerNumber,
      domainId: whatsappCcaasTable?.domainId,
    ...
    console.log('PayNowUrl', PayNowUrl);
    await sendMessageCustomer(whatsappCcaasTable, PayNowUrl);
  } catch (error) {
...
export async function getConsultantData(appointment: any, id: any, getAppDetails: any) {
  try {
    ...
    logger.error('getConsultantData err:', error);
  }
}

// get all user name
export async function getAppointmentEmailData(appointment: any, id: any) {
  try {
    const body = {
      p_companyId: appointment?.companyId,
      p_serviceid: id?.serviceId
    ...
    logger.error('getAppointmentEmailData err:', error);
  }
}

// get all user name
export async function getEmailDetails(companyId: any) {
  try {
    const body = {
      p_companyId: companyId
    };
    const appointmentUser: any = await axios.post(process?.env?.getEmailDetails, body);
    if (appointmentUser?.data?.status_code === 200) {
      return appointmentUser?.data?.result;
    }
    return [];

  } catch (error) {
    logger.error('getEmailDetails err:', error);
  }
}
