...
export default function ProductConversation() {
  ...
  const { aiAgentChatMessage, aiAgentChatTyping } = useSelector((state: any) => state.productStore);
  ...
  const onChange = (value: any, files: any) => {
    ...
    dispatch(productDataSliceActions.setproductAiAgentChat({ ...body, isBot: 0, query: body?.query }));
    dispatch(productDataSliceActions.setproductAiAgentChatTyping({ sessionId: chatsessionId, typing: true }));
  };
          ...
          <div className={styles.bubbleblock}>
            ...
            {!!aiAgentChatTyping?.[`${chatsessionId}`] && (
              <div className="msgbubble left">
                <div className="chatContent">{aiAgentChatTyping?.[`${chatsessionId}`] || "Typing ..."}</div>
              </div>
            )}
            <div ref={chatEndRef} />
          </div>
