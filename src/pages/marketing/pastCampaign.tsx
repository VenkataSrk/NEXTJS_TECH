...
export default function MarketingPastCampaign() {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [pastCampaignData, setPastCampaignData] = useState<any>([]);
    const [loader, isLoader] = useState<any>(true);
    console.log("pastDAta", pastCampaignData)
    const handlePastCampaigns = (item: any) => {
        dispatch(commonDataSliceActions.setTabMenuPush({ path: `/marketing/template-conversation?campaignId=${item?.campaignId ?? item}&page=pastCampaign`, menuName: item?.name, close: 1, type: "marketing" }));
        navigate(`/marketing/template-conversation?campaignId=${item?.campaignId ?? item}&page=pastCampaign`);
    }

    const fetchPastCampaign = async () => {
        try {
            const body: any = {
                campaignTypeId: 1
            }
            await CoreServices.post(body, '/get_past_campaign').then((res: any) => {
                console.log("res", res?.data?.getPastCampaignHandlerDataRes);
        ...
        <div>
            <div className={`${styles.marketmain} maincontainer`}>
                <div className={styles.marketmaininner}>
                    ...
                    <div className={`${styles.topblockarea} ${pastCampaignData?.length == 0 && styles.datafound}`}>
                        {pastCampaignData?.length == 0 && !loader &&
                        ...
                        {loader ?
                            <div className={styles.campaignblockmain}>
                                <div className={`${styles.campaignblockmain} ${styles.skeletonStyles}`} style={{paddingTop: 16}}>
                                    {Array.from({ length: 6 }, (_, index) => (
                                        <div className={styles.campaignblock}>
                                            ...
                                            <div className={styles.campaignblockbottom}>
                                                ...
                                                <div className={`${styles.campaignblockbottomright} iconStack camstack`}>
                            ...
                            : <>
                                {pastCampaignData?.length != 0 &&
                                    <>
                                        <div className={styles.campaignblockmain}>
                                            {pastCampaignData?.map((item: any) => {
                                                return (<>

                                                    <div className={styles.campaignblock} onClick={() => handlePastCampaigns({ name: `${item?.campaign_name}`, campaignId: `${item?.campaignchatid}` })}>
                                                        <div className={styles.campaignblocktop} >
                                                            <div className={styles.campaignblocktopinner}>Created date<span>{moment(item?.createdate).format("YYYY-MM-DD")}</span></div>
                                                            {/* <div className={styles.campaignblocktopinner}>Scheduled time<span>24/2</span></div> */}
                                                            <div className={styles.campaignblocktopinner}>Created by<span>{item?.created_by}</span></div>
                                                        </div>
                                                        <div className={styles.campaignblockbottom}>
                                                            <div className={styles.campaignblockbottomleft}>
                                                                <div className={styles.title}>{item?.campaign_name}
                                                                    {/* <span>Retention</span><span>PB-2025-004</span> */}
                                                                </div>
                                                                <p>Primary Goal: {item?.campaign_goal}</p>
                                                            </div>
                                                            <div className={`${styles.campaignblockbottomright} iconStack camstack`}>
