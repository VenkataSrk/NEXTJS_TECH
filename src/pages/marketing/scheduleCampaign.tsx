...
export default function MarketingScheduleCampaign() {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [scheduleData, setScheduleData] = useState<any>([]);
    const [loader, isLoader] = useState<any>(true);
    const handleScheduleCampaigns = (item: any) => {
        dispatch(commonDataSliceActions.setTabMenuPush({ path: `/marketing/template-conversation?campaignId=${item?.campaignId ?? item}&page=scheduleCampaign`, menuName: item?.name, close: 1, type: "marketing" }));
        navigate(`/marketing/template-conversation?campaignId=${item?.campaignId ?? item}&page=scheduleCampaign`);
    }

    const fetchScheduleCampaign = async () => {
        try {
            const body: any = {
                campaignTypeId: 3
            }
            await CoreServices.post(body, '/get_schedule_campaign').then((res: any) => {
                console.log("res", res);
                setScheduleData(res?.data?.getScheduleCampaignDataRes?.[0]);
                isLoader(false);
            })
        } catch (error) {
            ...
            <div>
                ...
                <div className={`${styles.marketmain} maincontainer`}>
                    <div className={styles.marketmaininner}>
                        ...
                        <div className={`${styles.topblockarea} ${scheduleData?.length == 0 && styles.datafound}`}>
                            {scheduleData?.length == 0 && !loader &&
                                    ...
                                    <>
                                        <div className={styles.campaignblockmain}>
                                            {scheduleData?.map((item: any) => {
                                                return (<>
                                                    <div className={styles.campaignblock} onClick={() => handleScheduleCampaigns({ name: `${item?.campaign_name}`, campaignId: `${item?.campaignchatid}` })}>
                                                        <div className={styles.campaignblocktopmain}>
                                                            <div className={styles.campaignblocktop}>
                                                                <div className={styles.campaignblocktopinner}>Created date<span>{moment(item?.createdate).format("DD/MM/YYYY")}</span></div>
