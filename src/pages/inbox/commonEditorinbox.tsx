...
const Commoneditorinbox = ({ onChange,templateChat=false }: any) => {
  ...
  <div className={`maincontainer  ${templateChat?'chatmaincontainer':""}`}>
    ...
    <div className={`${styles.editor} editorpadding`}>
       {saveFiles?.length > 0 && (
      ...
      <div 
          ...
          style={{ alignItems: ['24px', 'auto'].includes(textareaRef?.current?.style?.height) || !textareaRef?.current?.style?.height ?  "center" : 'end'}}>
        ...
        <textarea
          ...
          className={`${styles.textarea} customscroll`}
        />
      <div className={styles.speechtextblock}>
          <div className={styles.editorblockicon} onClick={() => setMicOpen(true)}>
            <SpeachToText
              ...
              onText={(newText: string) => handleChange(newText)}
            />
          </div>
...
export default Commoneditorinbox;
