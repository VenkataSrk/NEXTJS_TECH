...
function Createknowledge() {
    ...
    // Placeholder functions (do nothing)
    const handleInput = () => { console.log('UI interaction: handleInput'); };
    const handleSend = () => { console.log('UI interaction: handleSend'); };
    const startVoiceToText = () => { console.log('UI interaction: startVoiceToText'); };
    const handleView = (channelKey: string) => { console.log(`UI interaction: Viewing template for: ${channelKey}`); };
    ...
    return (
        <>
            <div className={`${styles.elevatebrandbody} createcampaignhome `}>
                <div className={styles.innerbody}>
                    <div className={styles.knowledgebasearticleheader}>
                           <h4>Knowledge base</h4>
                           <div className={styles.articletabcontainer}>
                              <p>Create article <span><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
                        ...
                        <div className={styles.msgbody}>
                            <div style={{ marginTop: "20px" }} ref={bottomRef} className={`${styles.scroller} ${styles.chatflexscroll}`}>
                                <div className={styles.maincontainer}>
                                    {/* STATIC MESSAGE 2: User Response */}
                                    <div className={`${styles.msgbubble} ${styles.right}`}>
                                        <div className="chatContent" dangerouslySetInnerHTML={{ __html: "I want to run a campaign for our new product launch focused on boosting conversions for mobile users." }} />
                                    </div>
                                    {/* STATIC MESSAGE 3a: Bot Response Text */}
                                    <div className={`${styles.msgbubble} ${styles.left}`}>
                                        <div className="chatContent" dangerouslySetInnerHTML={{ __html: "Understood. Here are some draft templates for your multi-channel campaign:" }} />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={` ${styles.searchmessage}`}>
                            <div className={`${styles.searchmessageText}`}>
                                ...
                                <textarea
                                    ...
                                    rows={1}
                                    style={{ minHeight: `${MIN_HEIGHT}px`, maxHeight: "120px", height: `${MIN_HEIGHT}px` }}
                                    onInput={handleInput}
                                    value={campaignMessage}
                                    onChange={() => { /* UI-only: prevents React warning */ }} 
                                />
                                ...
                                {campaignMessage && !isRecording ? (
                                    ...
                                    <svg width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={startVoiceToText} aria-pressed={isRecording}>
                                        {/* SVG path data is kept for visual fidelity */}
                                        ...
                                        <defs>
                                            {/* All filter/gradient defs are kept for SVG rendering */}
