...
// import { leftnavassets } from '../../styless/assets/assets';
...
const dedupeContacts = (contacts: any[] = []) => {
  ...
  return contacts.filter((contact) => {
    const identifier =
      ...
      contact?.email ??
      `${contact?.firstName ?? ""}-${contact?.lastName ?? ""}-${contact?.source ?? ""}-${contact?.createdAt ?? ""}`;
...
const Contacts = () => {
  ...
  const [pageLoader, setPageLoader] = useState<any>(true);
  const [contactSummary, setContactSummary] = useState<any>(null);
  const [searchValue, setSearchValue] = useState<any>("");
  const [selectedItem, setSelectedItem] = useState<any>(0);
  const colors =["orange","blue","pinkcw","skycw","cwill","subviol","grey"];
  const selectedColor=useRef<string>('orange')            
      ...
      whatsappService
        ...
        .then((response) => {
          if (
            ...
            const uniqueContacts = dedupeContacts(response?.data?.contacts);
            setContactDatas(uniqueContacts);
            setContactDatasCopy(uniqueContacts);
            setContactSummary(uniqueContacts?.[0]);
            setLoader(false);
          } else {
            setContactDatas([]);
            setLoader(false);
  ...
  const handleContactSummary = (item: any) => {
    setContactSummary(item);
  };
...
const handleSearch = (e: any) => {
  ...
  if (trimmedValue.length > 0) {
    const filtered = contactDatasCopy.filter((item: any) =>
      item?.Name?.toLowerCase()?.includes(trimmedValue)
    );
    setContactDatas(filtered);
  } else {
    setContactDatas(contactDatasCopy);
  }
};
  ...
  useEffect(() => {
    fetchContacts();
  }, [!contactMessageData?.isOpenContactModal]);
  ...
  const createContact=()=>{
    try {
        dispatch(contactsSliceActions.reset());
        dispatch(contactsSliceActions.setIsOpenContactModal(true))   
    } catch (error) {
        console.log(error)
    }
  }
 ...
 <h4 className={styles.aipheader}>Contacts</h4>
          <p className={styles.aipdesc}>
            Add, import and manage customer details.
          </p>
          </div> 
            {/* <div className={styles.rights}>
                  ...
                  >
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
                      d="M3 4.51288C4.2339 4.51288 5.2435 3.50328 5.2435 2.26938C5.2435 1.03548 4.2339 0.0258789 3 0.0258789C1.7661 0.0258789 0.7565 1.03548 0.7565 2.26938C0.7565 3.50328 1.7661 4.51288 3 4.51288ZM3 6.75638C1.7661 6.75638 0.7565 7.76598 0.7565 8.99988C0.7565 10.2338 1.7661 11.2434 3 11.2434C4.2339 11.2434 5.2435 10.2338 5.2435 8.99988C5.2435 7.76598 4.2339 6.75638 3 6.75638ZM0.7565 15.7304C0.7565 14.4965 1.7661 13.4869 3 13.4869C4.2339 13.4869 5.2435 14.4965 5.2435 15.7304C5.2435 16.9643 4.2339 17.9739 3 17.9739C1.7661 17.9739 0.7565 16.9643 0.7565 15.7304Z"
                      fill="#757676"
                    />
                  </svg>
                </div>
        ...
        >
          <div className="dropdown-item" >
            <span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 19 19" fill="none">
  <path d="M11.45 5.54L13.12 7.26L3.2 16.87L1.5 16.84L1.53 15.14L11.45 5.53M11.47 3.63C11.34 3.63 11.21 3.68 11.12 3.77L0.35 14.21C0.16 14.4 0.0400002 14.66 0.0400002 14.93L0 17.31C0 17.87 0.44 18.34 1 18.34L3.38 18.38C3.38 18.38 3.39 18.38 3.4 18.38C3.66 18.38 3.92 18.28 4.11 18.09L14.88 7.65C15.08 7.45 15.09 7.13 14.89 6.93L11.84 3.78C11.74 3.68 11.61 3.63 11.47 3.63Z" fill="black"/>
  <path d="M15.4 1.71L17.07 3.43L15.97 4.49L14.3 2.77L15.4 1.71ZM15.41 0C15.16 0 14.91 0.09 14.71 0.28L12.52 2.4C12.33 2.58 12.32 2.89 12.51 3.08L15.6 6.27C15.69 6.37 15.82 6.42 15.95 6.42C16.08 6.42 16.19 6.37 16.28 6.28L18.47 4.16C18.87 3.78 18.88 3.14 18.49 2.75L16.12 0.31C15.92 0.11 15.66 0.00999999 15.4 0.00999999L15.41 0Z" fill="black"/>
</svg></span>
            Edit
          </div>
          <div className="dropdown-item" >
            <span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 18 21" fill="none">
  <path d="M7.2999 15.9995C6.8858 15.9995 6.5499 15.6636 6.5499 15.2495V6.6797C6.5499 6.2656 6.8858 5.9297 7.2999 5.9297C7.714 5.9297 8.0499 6.2656 8.0499 6.6797V15.2495C8.0499 15.6636 7.714 15.9995 7.2999 15.9995Z" fill="black"/>
  <path d="M10.5401 15.9995C10.126 15.9995 9.7901 15.6636 9.7901 15.2495V6.6797C9.7901 6.2656 10.126 5.9297 10.5401 5.9297C10.9542 5.9297 11.2901 6.2656 11.2901 6.6797V15.2495C11.2901 15.6636 10.9542 15.9995 10.5401 15.9995Z" fill="black"/>
  <path d="M17.0899 2.5298H13.1397V1.9199C13.1397 0.8613 12.3057 0 11.2803 0H6.5498C5.5244 0 4.6899 0.8613 4.6899 1.9199V2.5298H0.75C0.3359 2.5298 0 2.8657 0 3.2798C0 3.6939 0.3359 4.0298 0.75 4.0298H1.3543L2.1216 16.6734C2.2339 18.6119 3.8452 20.1304 5.7901 20.1304H12.0303C13.9737 20.1304 15.5938 18.6099 15.7188 16.6656L16.4866 4.0299H17.0899C17.504 4.0299 17.8399 3.694 17.8399 3.2799C17.8399 2.8658 17.504 2.5298 17.0899 2.5298ZM6.19 1.9199C6.19 1.6885 6.3516 1.5 6.5499 1.5H11.2804C11.4786 1.5 11.6398 1.6885 11.6398 1.9199V2.5298H6.1901L6.19 1.9199ZM14.2217 16.5718C14.1475 17.7261 13.1846 18.6304 12.0303 18.6304H5.7901C4.6392 18.6304 3.6851 17.7329 3.6187 16.5845L2.8569 4.0298H14.983L14.2217 16.5718Z" fill="black"/>
</svg></span>
            Delete
          </div>
        </div> */}
        ...
        <div className={`${styles.customercontainer} customercontactcontainer`}>
          <div className={styles.customerinsigh}>
            <div className={styles.sigsearch}>
              <div className={styles.createcampaign}>
                <div className={styles.lefts}   onClick={createContact}>
                  <span>
                    <span>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                      ...
                      >
                        <path
                          d="M9.99999 0.00439453C4.48228 0.00439453 0.00415039 4.48253 0.00415039 10.0002C0.00415039 15.5179 4.48228 19.9961 9.99999 19.9961C15.5177 19.9961 19.9958 15.5179 19.9958 10.0002C19.9958 4.48253 15.5277 0.00439453 9.99999 0.00439453ZM14.2982 10.7499H10.7497V14.2984C10.7497 14.7183 10.4198 15.0481 9.99999 15.0481C9.58016 15.0481 9.2503 14.7183 9.2503 14.2984V10.7499H5.70178C5.29195 10.7499 4.95209 10.4201 4.95209 10.0002C4.95209 9.58041 5.29195 9.25054 5.70178 9.25054H9.2503V5.70202C9.2503 5.29219 9.59016 4.95233 9.99999 4.95233C10.4098 4.95233 10.7497 5.29219 10.7497 5.70202V9.25054H14.2982C14.718 9.25054 15.0579 9.5904 15.0579 10.0002C15.0579 10.4101 14.718 10.7499 14.2982 10.7499Z"
                          fill="#00E2A4"
                        />
                      </svg>
                    </span>
        ...
        >
          ...
          <div className="dropdown-item" >
            ...
            <div className={styles.contactscroller}>
              {contactDatas?.length > 0 ? (
                <>
                  {contactDatas?.map((item: any, ind: any) => {
                    return (
                      <div
                        className={`${styles.willamtopbar} ${
                          selectedItem == ind ? styles.active : ""
                        }`}
                        onClick={() => {
                          handleContactSummary(item);
                          setSelectedItem(ind);
                          selectedColor.current =colors[(ind%7)];
                        }}
                      >
                        <span className={`${styles[colors[(ind%7)]]}`}>
...
<span style={{fontFamily:"intermedium", fontSize:"14px"}}> No contacts found</span>
              </div>
              </div> 
          ...
          {contactSummary && (
            <div className={styles.customerleftpanel}>
              <div className={styles.topbarpanel}>
                <h4 className={styles?.[(selectedColor.current)]}>{`${contactSummary?.firstName?.[0]?.toUpperCase()}${
                  contactSummary?.lastName?.[0]?.toUpperCase()??''
                }
                `}</h4>
                <h3>
                  {`${contactSummary?.firstName ?? ""} ${contactSummary?.lastName??""}`}
                  {/* <span>{"dfdf"}</span> */}
                </h3>
                <p>{`Stage - ${contactSummary?.lifeCycleStage}`}</p>
              </div>

              <div className={styles.generalinformation}>
                <h3>Contact Summary</h3>
                <p className={styles.createmanage}>
                  {" "}
                  View and manage detailed subscriber information including SIM
                  type, account validity, usage patterns (calls, data, SMS),
                  network type, and purchased plans. Track customer journeys,
                  analyze retention offers, and review payment preferences to
                  ensure seamless service and personalized experiences.{" "}
                </p>
                ...
                <div className={styles.generaldetails}>
                  ...
                  <p>{`${contactSummary?.phoneNumber ?? "-"}`}</p>
                </div>
                <div className={styles.generaldetails}>
                  <span>WhatsApp Number</span>
                  <p>{`${contactSummary?.whatsappNumber ?? "-"}`}</p>{" "}
                </div>
                <div className={styles.generaldetails}>
                  <span>Source of Contact</span>
                  <p>{`${contactSummary?.source ?? "-"}`}</p>
                </div>
                <div className={styles.generaldetails}>
                  <span>Postal Code</span>
                  <p>{`${contactSummary?.postCode ?? "-"}`}</p>
                </div>
                <div className={styles.generaldetails}>
                  <span>Created Date</span>
                  <p>{`${contactSummary?.createdAt?moment(contactSummary?.createdAt).format('YYYY-MM-DD'):"-"}`}</p>
                </div>
              </div>
            </div>
          )}
        ...
        {contactMessageData?.isOpenContactModal && 
        <CreateContact
        headerTitle="Hi there! Let’s create a new contact."
        isOpen={contactMessageData?.isOpenContactModal}
        />}
      </ContentLayout>
    </>
  );
