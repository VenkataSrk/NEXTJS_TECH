...
import Initialhomepagenew from '../initialhomepagenew/initialhomepagenew'
...
function Createcampaigns({ isCVM, CreateNewCampaign, campaignId, isNewChat }: any) {
  ...
  console.log("campaignmsgdaa",campaignMessageData)
  const showBuildCampaignData = useSelector((state: any)=>state.commonData.showBuildCampaign);
  console.log("showbuild",showBuildCampaignData)
  const initialCampaignChatType = useSelector((state :any)=>state.commonData.initialMsg)
  console.log("show",showBuildCampaignData)
   const commonData = useSelector((state :any)=>state.commonData)
  console.log("show",commonData)
  const isCreateNewCampaign = useSelector((state :any)=>state.commonData.isCreateNewCampaign);
  console.log("iscreateen",isCreateNewCampaign)
  ...
  const[ whatsappTemplateEdit,setWhatsappTemplateEdit]=useState<any>(false);
  const[ whatsappTemplateDatas,setWhatsappTemplateDatas]=useState<any>(null);
  console.log("whatsappTemplateDatas",whatsappTemplateDatas)
  console.log("whtat",whatsappTemplateEdit)
  ...
  console.warn("currentchatid>>:",campaignChatId);
  const clearFiles :any=useSelector((state :any)=>state.commonData.clearFiles);
  ...
  useLayoutEffect(() => {
    ...
    bottomRef.current.scrollTo({
      top: bottomRef.current.scrollHeight+20,
      behavior: "smooth",
    });
  ...
  const handleSend = () => {
    try {
      console.log("handleSend>>>",campaignMessage)
      if (!campaignMessage.trim() && uploadedFiles?.length==0){
          return;
      } 
      const newMsg = {
        sessionId,
        ...
        createdBy:data?.first_name??data?.profileName
      };
    ...
    try {
      ...
      const newMsg = {
        ...
        campaignChatId,
        messageId: uuidv4(),
        message: campaignMessageData?.campaignMessage?.[0]?.message??"",
        uploadUrl: uploadedFiles ?? '',
        sender: "userText",
        date: new Date().toISOString(),
        domainId: data?.domainId,
  ...
  const handleViewEmail = (channelKey: any, channelData: any, index: number,allTemplates :any) => {
    dispatch(campaignSliceActions.setTempleteView({ channels: channelKey, ...channelData, index,allTemplates:allTemplates }));
    // setOnEdit(true);
    // setEditValue('');
    // setEditPromptEmail('')
    // setMainChat();
    dispatch(commonDataSliceActions.setShowMainChat(false));

  };

  const handleViewSms = (channelKey: any, channelData: any, index: number,allTemplates :any) => {
    dispatch(campaignSliceActions.setTempleteView({ channels: channelKey, ...channelData, index,allTemplates:allTemplates }));
    // setOnEdit(true);
    // setEditValue('');
    // setEditPromptSms('');
  ...
  const handleViewAppPush = (channelKey: any, channelData: any, index: number,allTemplates :any) => {
    dispatch(campaignSliceActions.setTempleteView({ channels: channelKey, ...channelData, index ,allTemplates:allTemplates}));
    // setOnEdit(true);
    // setEditValue('');
  ...
  const handleViewWebPush = (channelKey: any, channelData: any, index: number,allTemplates :any) => {
    dispatch(campaignSliceActions.setTempleteView({ channels: channelKey, ...channelData, index ,allTemplates:allTemplates}));
    // setOnEdit(true);
    // setEditValue('');
  ...
  const handleViewWhatsapp = (channelKey: any, channelData: any, index: number,allTemplates :any) => {
    // console.log("channelKey>>>",channelKey,"channelData>>>",channelData,"index>>>",index,"alltemp",allTemplates);
    dispatch(campaignSliceActions.setTempleteView({ channels: channelKey, ...channelData, index ,allTemplates:allTemplates}));
    // setOnEdit(true);
    // setEditValue('');
  ...
  const handleViewWhatsappTemplate =(templateId :any)=>{
     dispatch(campaignSliceActions.setTempleteView({ channels: "whatsapp_template", template:[{template_id:templateId,channel:"whatsapp_template"}],allTemplates:{"whatsapp_template":templateId}}));
     dispatch(commonDataSliceActions.setShowMainChat(false));
  }

  const handleViewInapp = (channelKey: any, channelData: any, index: number,allTemplates :any) => {
    dispatch(campaignSliceActions.setTempleteView({ channels: channelKey, ...channelData, index,allTemplates: allTemplates }));
    // setOnEdit(true);
    // setEditValue('');
    // setEditPromptInAPP('');
...
<table border="1" style="width:100%; border-collapse: collapse;">
  ...
  <tbody>
    <tr data-id="101" data-action="row-click">
      <td>Aiswarya</td>
      <td>Chennai</td>
    </tr>
    <tr data-id="102" data-action="row-click">
      <td>Yogesh</td>
      <td>Madurai</td>
    </tr>
    <tr data-id="103" data-action="row-click">
      <td>Yokesh</td>
      <td>Salem</td>
    </tr>
  </tbody>
...
`;
      ...
      const init = async () => {
        ...
        if (state?.data && campaignMessageData?.product?.type != "CVM") {
          ...
          const newMsg = {
            ...
            campaignChatId,
            messageId: uuidv4(),
            message: state?.selectedMyPlanData?state?.selectedMyPlanData: state?.data ?? '',
            sender: "userText",
            date: new Date().toISOString(),
            domainId: data?.domainId,
            ext: +(data?.ext),
          ...
          SocketService.campaignMessage(newMsg);
          dispatch(campaignSliceActions.setCampaignMessage([...newMsg]));
  ...
  const campaignChatIdRef = useRef(campaignChatId);

useEffect(() => {
  campaignChatIdRef.current = campaignChatId;
}, [campaignChatId]);

useEffect(()=>{
  if( initialCampaignChatType!=""){
  // if(initialCampaignChatType!="campaign" && initialCampaignChatType!=""){
    const timeout = setTimeout(() => {
     try {   
      sessionId=uuidv4();
      const newMsg = {
        sessionId,
        campaignChatId:campaignChatIdRef.current,
        messageId: uuidv4(),
        message: initialCampaignChatType ?? '',
        uploadUrl: uploadedFiles ?? '',
        sender: "userText",
        date: new Date().toISOString(),
        domainId: data?.domainId,
      ...
      dispatch(commonDataSliceActions.setInitialChatMsg(""));
    }
  },300)
  return () => clearTimeout(timeout);

  }
},[initialCampaignChatType])
...
//        console.log(error)
//      }
//       dispatch(commonDataSliceActions.setIsNewCampaign(false));
//   }
// },[isCreateNewCampaign])
...
useEffect(()=>{
  if(clearFiles){
    setUploadedFiles([]);
    dispatch(commonDataSliceActions.setClearFiles(false));
  }
   
},[clearFiles])

useEffect(()=>{
  if(campaignMessageData?.campaignMessage?.[0]?.myplan && campaignMessageData?.campaignMessage?.length==1){
    handleSendMyplanInit();
  }
},[campaignMessageData?.campaignMessage])
                        ...
                        <div className={styles.searchmessageText}>
                          ...
                          <div className={`${styles.actionbottom} ${styles.actionbottomtop}`}>
                            <div>
                              {campaignMessageData?.isTyping || campaignMessageData?.progress ? (
                              ...
                              ) : ((campaignMessage || uploadedFiles.length > 0)&& !isRecording ) ? (
                                // <img
                                //   src={assets.sndarrowchat}
                                //   alt="send"
                                //   onClick={handleSend}
                                //   style={{ cursor: "pointer", height: "38px" }}
                                // />
                                <svg style={{ cursor: "pointer", height: "38px" }} onClick={handleSend} xmlns="http://www.w3.org/2000/svg" width="20" height="17" viewBox="0 0 20 17" fill="none">
  <path d="M19.5711 0.434772C19.1688 0.0265723 18.5623 -0.107228 18.0247 0.0890722L0.994009 6.25407C0.447109 6.44257 0.0697089 6.91617 0.00860893 7.48947C-0.0514911 8.05197 0.206809 8.58127 0.666809 8.85957L3.71171 10.8254C4.07791 11.0578 4.55351 11.0647 4.92361 10.8381L15.2629 4.06547L5.58281 11.7609C5.38211 11.9328 5.24691 12.1711 5.20191 12.4289L4.75561 14.9562C4.64381 15.589 4.91971 16.1964 5.45921 16.505C5.99631 16.8116 6.64961 16.7501 7.12471 16.3429L9.34741 14.4367L12.761 16.6681C12.9968 16.8224 13.2737 16.9015 13.5535 16.9015C13.7215 16.9015 13.8909 16.8732 14.053 16.8146C14.4876 16.6574 14.8147 16.3156 14.9422 15.9015L19.8797 1.94147C20.0853 1.41507 19.9671 0.837072 19.5711 0.434772Z" fill="black"/>
</svg>
                              ) : (
                                <>
            ...
            {(campaignMessageData?.campaignMessage?.length > 0 || showMainChat) &&
              <div className={`${styles.msgbody} msgbody`}>
                <div style={{ marginTop: "20px" }} ref={bottomRef} className={`${styles.scroller} ${styles.chatflexscroll}`}>
                  ...
                  <div className={styles.maincontainer}>
                    {Array.isArray(campaignMessageData?.campaignMessage) && campaignMessageData?.campaignMessage?.map((msg: any, index: number) => {
                      if (msg.sender === "userText" && msg?.template_chat==0) {
                        return (
                          <>
                            ...
                            {(msg?.hiddenText==1 ||!msg?.message?.trim())?<></>:

                            <div key={msg?.messageId} className={`${styles.msgbubble} ${styles.right}`}>
                             
                              {/* <div className="chatContent" dangerouslySetInnerHTML={{ __html: msg?.message?.replace(/\n/g, '<br/>').replace(/ /g, '&nbsp;') }} /> */}
                                <div
                        ...
                        return (
                          <>
                           {(msg?.graph_url) && ( <iframe srcDoc={contentHtml.current.get(msg?.graph_url)} /> )}
                            ...
                            {(msg?.intent === "refresh_dashboard" && msg?.chatType == "analytics") && (
                              <div className={`${styles.msgbubble} ${styles.left} ${styles.cardsresponse}`}>
                                <div className={styles.cardscontainerscroller}>
                                  {msg?.analytics_data && Object.entries(msg?.analytics_data)?.map(([key, data]: any) => (
                                    <div key={key} className={styles.card}>
                                      <div className={styles.top}>
                                        ...
                                        <div className={styles.rights}>
                                          ...
                                          {key == "delivered" && <p>{parseInt(data) > 0 ? `${data} messages delivered successfully (${msg?.percentDelivered}% of your target).` : `No messages have been delivered yet.`} </p>}
                                          {key == "openRate" && <p>{parseInt(data) > 0 ? `${data} recipients opened your message (${msg?.percentOpened}%).` : `No recipients have opened the message yet.`} </p>}
                                          {key == "clickRate" && <p>{parseInt(data) > 0 ? `${data} recipients clicked links in your message (${msg?.percentClicked}%).` : `No recipients have clicked the links in the campaign yet.`} </p>}
                                        </div>
                                      </div>
                            ...
                            {msg?.intent === "create_campaign" && (
                              <div className={`${styles.msgbubble} ${styles.left} ${styles.cardsresponse}`}>
                                <div className={styles.cardscontainerscroller}>
                                  {Object.entries(msg?.campaign_data?.channels)?.map(([channelKey, channelData]: any) => (
                                    <div key={channelKey} className={styles.card}>
                                      ...
                                      <div className={`${styles.top} ${styles.bottom}`}>
                                        ...
                                        {channelKey === "email" &&
                                          <button className={`viewbutton`} onClick={() => handleViewEmail(channelKey, channelData, index,msg?.campaign_data?.channels)}>View</button>
                                        }
                                        {channelKey === "sms" &&
                                          <button  className={`viewbutton`} onClick={() => handleViewSms(channelKey, channelData, index,msg?.campaign_data?.channels)}>View</button>
                                        }
                                        {channelKey === "apppush" &&
                                          <button  className={`viewbutton`} onClick={() => handleViewAppPush(channelKey, channelData, index,msg?.campaign_data?.channels)}>View</button>
                                        }
                                        {channelKey === "webpush" &&
                                          <button  className={`viewbutton`} onClick={() => handleViewWebPush(channelKey, channelData, index,msg?.campaign_data?.channels)}>View</button>
                                        }
                                        {channelKey === "whatsapp" &&
                                        <>
                                        
                                        <button   className={`viewbutton`} onClick={() => handleViewWhatsapp(channelKey, channelData, index,{...msg?.campaign_data?.channels,isWhatsappCard:false})}>View</button>
                                        
                                          
                                        </>
                                        }
                                        {channelKey === "inapp" &&
                                          <button   className={`viewbutton`} onClick={() => handleViewInapp(channelKey, channelData, index,msg?.campaign_data?.channels)}>View</button>
                                        }
                                      </div>
                                    </div>
                                  ))}
                                  ...
                                  {/* Email Modal */}
                                  {(false && onEdit && campaignMessageData?.templeteView?.channels === "email") && (
                                            ...
                                            {campaignMessageData?.isRegenerate ? (
                                              ...
                                              <div className={styles.responseslider}>
                                                {campaignMessageData?.templeteView?.template?.length > 1 &&
                                                  <button className={`${styles.arrow} viewbutton`} onClick={() => handleEmailClick('-')}><svg xmlns="http://www.w3.org/2000/svg" width="6" height="13" viewBox="0 0 6 13" fill="none" >
                                          ...
                                          <div className={styles.lgbgblock}>
                                            <div className={styles.footerPagination}>
                                              ...
                                              {pages.map((page: any, ind: any) => (
                                                ...
                                                <button
                                                  key={page}
                                                  className={`${styles.page } ${active === page ? styles.active : ""} viewbutton`}
                                            ...
                                            <div className={`${styles.actionsblock} ${styles.actionblocknew}`}>
                                              ...
                                              <div className={styles.btncontainer}>
                                                {!editPromptEmail && <button className="whitebtnnew viewbutton" onClick={setEditPromptEmail}>
                                                  <img src={assets.editPrompt} alt="Edit" />
                                                  Edit prompt</button>}
                                                {!editPromptEmail && <button className="whitebtnnew viewbutton" onClick={() => regenerateEmail(campaignMessageData?.templeteView)}>
                                  ...
                                  {whatsappTemplateEdit && (
                                    ...
                                    <div className='closetting lgbgmodal smstmode footerpadnone' >
                                      <WorktualModal show={true} mdsize="modalxxl" footer={<></>
                                        ...
                                        <div className={`${styles.emailsmsent} stickyheader `}>
                                          <div className={styles.leftcampaign}>
                                            <h3>{whatsappTemplateDatas?.templateName}</h3>
                                            {/* <p>{`Created on: ${format(new Date(), "dd MMM, yyyy • hh:mm a")}`}</p> */}
                                            <p>{`Created on: ${whatsappTemplateDatas?.createdAt}`}</p>
                                          </div>
                                          <div className={styles.rightcampaign}>
                                            {/* <button className="primarybtn"
                                              onClick={() => handleViewWhatsappTemplate(whatsappTemplateEdit) }>
                                              Use Template
                                            </button> */}
                                            <button style={{border:"none", minWidth:"32px"}} className={styles.schedulebtn} onClick={() => setWhatsappTemplateEdit(false)}>

                                              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M9.98115 8.92095L17.6215 1.28035C17.9145 0.98735 17.9145 0.51275 17.6215 0.21985C17.3286 -0.0730501 16.8539 -0.0731501 16.561 0.21985L8.92065 7.86045L1.28025 0.21975C0.98725 -0.07325 0.51265 -0.07325 0.21975 0.21975C-0.07315 0.51275 -0.07325 0.98735 0.21975 1.28025L7.86015 8.92085L0.21975 16.5614C-0.07325 16.8544 -0.07325 17.3291 0.21975 17.622C0.36625 17.7684 0.55815 17.8417 0.75005 17.8417C0.94195 17.8417 1.13385 17.7684 1.28035 17.622L8.92075 9.98135L16.5611 17.622C16.7076 17.7684 16.8995 17.8417 17.0914 17.8417C17.2833 17.8417 17.4753 17.7684 17.6217 17.622C17.9147 17.329 17.9147 16.8543 17.6217 16.5614L9.98115 8.92095Z" fill="#23232B" />
                                              </svg>
                                              ...
                                              <div className={`${styles.mobilemainsec} ${styles.mobilemargin} ${styles.whtsappmobmargin}`}>
                                                <div className={styles.userinfotop}>
                                                  ...
                                                  <div className={`${styles.contentsec} ${styles.contentsecwhtapp}`}>
                                                    <div className={styles.msgsec}>
                                                      {/* Header */}
                                                      {whatsappTemplateDatas?.headerType === "Image" && (
                                                        <div className={styles.imgcon}>
                                                          <img
                                                            src={whatsappTemplateDatas?.headerValue}
                                                            alt="header image"
                                                          />
                                                        </div>
                                                      )}
                                                      {whatsappTemplateDatas?.headerType === "Text" && (
                                                        <div className={styles.headertxtwhtsapp}>

                                                          <p>{whatsappTemplateDatas?.headerValue}</p>
                                                        </div>
                                                      )}
                                                      {/* Body */}
                                                      <div
                                                      ...
                                                      <div
                                                        className={styles.footerwrapper}
                                                        dangerouslySetInnerHTML={{
                                                          __html: whatsappTemplateDatas?.footer ?? "",
                                                        }}
                                                      />
                                                      {/* Buttons */}
                                                      {
                                                      Array.isArray(JSON.parse(whatsappTemplateDatas?.buttonType))?
                                                      JSON.parse(whatsappTemplateDatas?.buttonType)?.map(
                                                        (button: any, index: number) => {
                                                          let label = "";
                                                          if (button?.action?.value === "calltoAction") {
                                                            label = button?.website?.label ?? button?.buttonText;
                            ...
                            {msg?.intent === "campaign_analytics" && (
                              <div className={`${styles.msgbubble} ${styles.left} ${styles.cardsresponse}`}>
                                <div className={styles.cardscontainerscroller}>
                                  {/* {Object.entries(msg?.campaign_data?.channels)?.map(([channelKey, channelData]: any) => ( */}
                                  {/* <div key={channelKey} className={styles.card}> */}
                                  <div key={"open"} className={styles.card}>
                                    <div className={styles.top}>
                                      <div className={styles.lefts}>
                                      ...
                                      <div className={styles.rights}>
                                        ...
                                        {/* <p>{channelData?.customer_description}</p> */}
                                        <p>{85000}</p>
                                        <p>{`${85000} times the email was delivered/opened/seen`}</p>
                                      </div>
                                    </div>
                                    <div className={`${styles.top} ${styles.bottom}`}>
                                      <div className={styles.lefts}>
                            ...
                            {(msg?.intent === "whatsapp_status" && msg?.whatsapp_status === "PENDING") && (
                              <div className={`${styles.msgbubble} ${styles.left} ${styles.containwhatsapp}`}>
                                <div className={styles.frstrow}>
                                  <div>
                                    <div className={styles.cardheader}>
                                      <h5>WhatsApp</h5><span style={{ display: "grid" }}><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                                        <path d="M0.788983 10.907C0.788455 12.7133 1.26041 14.4771 2.15793 16.0317L0.703125 21.3437L6.13909 19.9183C7.63674 20.7346 9.32295 21.1653 11.0391 21.166H11.0436C16.6949 21.166 21.2952 16.5672 21.2975 10.915C21.2986 8.176 20.2329 5.60058 18.2968 3.66299C16.361 1.72542 13.7863 0.657813 11.0433 0.656586C5.39134 0.656586 0.791286 5.25492 0.788983 10.907Z" fill="url(#paint0_linear_2026_21431)" />
                                        <path d="M0.422494 10.9035C0.421814 12.775 0.910794 14.6019 1.84026 16.2122L0.333313 21.7145L5.96426 20.238C7.51566 21.0841 9.26241 21.53 11.0401 21.5306H11.0447C16.8987 21.5306 21.6642 16.7666 21.6666 10.9121C21.6676 8.07466 20.5638 5.40676 18.5584 3.39966C16.5528 1.39264 13.8859 0.28684 11.0447 0.285538C5.18964 0.285538 0.424872 5.04885 0.422494 10.9035ZM3.77577 15.9349L3.56558 15.6011C2.68178 14.1958 2.21521 12.5718 2.21597 10.9042C2.21784 6.03798 6.17813 2.07907 11.048 2.07907C13.4063 2.08003 15.6227 2.99922 17.2897 4.6674C18.9566 6.33546 19.8739 8.55301 19.8732 10.9114C19.8711 15.7778 15.9107 19.7373 11.0447 19.7373H11.0413C9.45678 19.7364 7.90293 19.3111 6.54769 18.5068L6.22516 18.3157L2.88365 19.1917L3.77577 15.9349Z" fill="url(#paint1_linear_2026_21431)" />
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M8.38974 6.46469C8.19092 6.02276 7.98166 6.01385 7.7925 6.00611C7.63779 5.9995 7.46068 5.99991 7.28387 5.99991C7.10694 5.99991 6.81935 6.06639 6.57618 6.33196C6.33286 6.59754 5.64716 7.23956 5.64716 8.54536C5.64716 9.85115 6.59829 11.113 6.73087 11.2903C6.86356 11.4673 8.56699 14.2325 11.2646 15.2964C13.5066 16.1805 13.9628 16.0046 14.4495 15.9604C14.9361 15.9161 16.0197 15.3185 16.2409 14.6987C16.4621 14.0791 16.4621 13.5479 16.3958 13.4369C16.3294 13.3263 16.1524 13.2599 15.887 13.1272C15.6216 12.9945 14.3167 12.3523 14.0734 12.2638C13.8301 12.1754 13.6531 12.1311 13.4762 12.3968C13.2993 12.6623 12.791 13.2599 12.636 13.4369C12.4812 13.6143 12.3264 13.6364 12.0609 13.5037C11.7955 13.3706 10.9407 13.0906 9.92663 12.1865C9.13764 11.483 8.60501 10.6143 8.45008 10.3486C8.29526 10.0831 8.43354 9.93935 8.56665 9.80703C8.68583 9.68812 8.83211 9.49726 8.96481 9.34226C9.09724 9.18729 9.14151 9.07668 9.22997 8.8997C9.31843 8.72247 9.2742 8.56746 9.20784 8.43477C9.14149 8.30204 8.62576 6.98945 8.38974 6.46469Z" fill="white" />
                                        <defs>
                                          <linearGradient id="paint0_linear_2026_21431" x1="11" y1="19.5323" x2="11" y2="-1.0617" gradientUnits="userSpaceOnUse">
                                            <stop stop-color="#20B038" />
                                            <stop offset="1" stop-color="#60D66A" />
                                          </linearGradient>
                                          <linearGradient id="paint1_linear_2026_21431" x1="10.999" y1="19.9122" x2="10.999" y2="-1.42053" gradientUnits="userSpaceOnUse">
                                            <stop stop-color="#F9F9F9" />
                                            <stop offset="1" stop-color="white" />
                                          </linearGradient>
                                        </defs>
                                      </svg></span> 
                                      <span style={{ marginLeft: "12px" }} className={`${styles.statuselem} ${styles.processed}`}>{msg?.whatsapp_status}</span>
                                    </div>
                                    <p>Your ad campaign review is still in progress. We’re checking it against Meta Advertising Policies.</p>
                                  </div>
                               
                                  <button className='cancelbtn' onClick={() => handleViewWhatsapp('whatsapp',msg?.campaign_data,index,{...msg?.campaign_data,isWhatsappCard:true})}>View</button>
                                
                                </div>
                                <div className={styles.yellowbox}>
                                  <p> Your campaign begins once when templates approved</p>
                                </div>
                              </div>
                            )}
                             ...
                             <div
                              ...
                              onClick={(e) => {
                                ...
                                if (row && row.dataset.action === "row-click") {
                                  ...
                                  setWhatsappTemplateEdit(id);
                                  setWhatsappTemplateDatas(JSON.parse(msg?.whatsapptemplatedatalist)[`${id}`]);
                                  // handleViewWhatsappTemplate(id)
                                  console.log("Clicked ID:", id,JSON.parse(msg?.whatsapptemplatedatalist)[`${id}`]);
                            ...
                            {statusViewCard && (
                              <div className='closetting lgbgmodal smstmode footerpadnone' >
                                <WorktualModal show={statusViewCard} mdsize="modalxxl" footer={
                                  <div style={{ position: "relative" }} className={styles.lgbgblock}>
                                    {/* <div className={`${styles.actionsblock} ${styles.actionblocknew} ${styles.smsactionblock}`}>
                                          {editsPromptWhatsapp &&
                                            <p
                                  ...
                                  <div className={`${styles.emailsmsent} stickyheader `}>
                                    ...
                                    <div className={styles.rightcampaign}>
                                      ...
                                      <button className={styles.schedulebtn} onClick={() => setStatusViewCard(false)}>
                                        Close
                                      </button>
                                    </div>
                                  </div>
                                        ...
                                        <div className={`${styles.mobilemainsec} ${styles.mobilemargin} ${styles.whtsappmobmargin}`}>
                                          ...
                                          {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                            ...
                                            <div className={`${styles.contentsec} ${styles.contentsecwhtapp}`}>
                                              <div className={styles.msgsec}>
                                                {/* Header */}
                                                {whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.headerType === "Image" && (
                                                  <div className={styles.imgcon}>
                                                    <img
                                                      // src={campaignMessageData?.templeteView?.template?.[0]?.headerValue}
                                                      src={whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.headerValue}
                                                      alt="header image"
                                                    />
                                                  </div>
                                                )}
                                                {whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.headerType === "Text" && (
                                                  <div className={styles.headertxtwhtsapp}>

                                                    <p>{whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.headerValue}</p>
                                                  </div>
                                                )}
                                                {/* Body */}
                                                <div
                                                  ...
                                                  dangerouslySetInnerHTML={{
                                                    __html:
                                                      whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.bodymessage ?? "",
                                                  }}
                                                />
                                                {/* Footer */}
                                                <div
                                                  className={styles.footerwrapper}
                                                  dangerouslySetInnerHTML={{
                                                    __html: whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.footer ?? "",
                                                  }}
                                                />
                                                {/* Buttons */}
                                                {whatsappTemplateData?.campaign_data?.whatsapp?.template?.[0]?.buttonType?.map(
                                                  (button: any, index: number) => {
                                                    let label = "";
                                                    if (button?.action?.value === "calltoAction") {
                                                      label = button?.website?.label ?? button?.buttonText;
                        ...
                        <div className={styles.frstrow}>
                          <div>
                            <div className={styles.cardheader}>
                              ...
                              </svg></span> <span style={{ marginLeft: "12px" }} className={`${styles.statuselem} ${styles.processed}`}>active</span>
                            </div>
                            <p>Your ad campaign review is still in progress. We’re checking it against Meta Advertising Policies.</p>
                          </div>
                          <button className='cancelbtn'>View</button>
                        </div>
                        <div className={styles.yellowbox}>
                          <p> Your campaign begins once when templates approved</p>
                        </div>
                ...
                <div className={styles.searchblockouter}>
                  <div  className={`${styles.searchblock} ${styles.newinput}`}>
                    <div className={styles.searchmessage}>
                      <div className={styles.searchmessageText}>
                        {uploadedFiles.length > 0 && <div className="attachment-container">
                          {uploadedFiles.map((file, index) => (
...
export default React.memo(Createcampaigns)