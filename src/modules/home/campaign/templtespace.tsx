...
function CreateTemplates({ isCVM, campaignId, isNewChat }: any) {
  ...
  console.log("campaigntemplateData",campaignTemplateData)
...
// console.log("camptemp11",Object.keys(campaignTemplateData?.templeteView?.allTemplates));
  ...
  const handleInput: React.FormEventHandler<HTMLTextAreaElement> = (e) => {
    ...
    const handleInputResize :  React.FormEventHandler<HTMLTextAreaElement> = (ref ) => {
    try {
      const el = ref.currentTarget;
      el.style.height = "auto";
      if (el.value.trim() === "") {
        el.style.height = `${MIN_HEIGHT}px`;
      } else {
  ...
  //     console.log("API error:",error)
  //   }
  // }, [campaignTemplateData?.templateConversation?.length,loader]);
  ...
  useLayoutEffect(() => {
    ...
    return () => cancelAnimationFrame(frame);
  }, 1500); // 👈 adjust delay (100–300ms feels good)
  ...
  const handleSend = () => {
    try {
      if (!campaignMessage.trim() && uploadedFiles?.length==0) return;
      const newMsg = {
        sessionId,
        campaignChatId,
        messageId: uuidv4(),
        ...
        template_id:(emailVariant==2 &&campaignTemplateData?.templeteView?.template?.[0]?.channel=="email")?campaignTemplateData?.templeteView?.template?.[1]?.template_id:campaignTemplateData?.templeteView?.template?.[0]?.template_id,
  ...
  const variantEmail =(variant :any)=>{
  ...
  useEffect(()=>{
    const init= async()=>{
      ...
      if(emailVariant!=2 && !campaignTemplateData?.templeteView?.allTemplates?.isWhatsappCard){
 console.log("called123",campaignTemplateData?.templeteView?.allTemplates?.isWhatsappCard)
        try {
        let body ={
          templateId:campaignTemplateData?.templeteView?.template?.[0]?.template_id??0
        }
            ...
            try {
              conversation = JSON.parse(
                res?.data?.getPastCampaignListResponse?.[0]?.conversation || "[]"
                 ...
                 console.log("called345",conversation)
            } catch (e) {
        ...
        setLoader(false);
         ...
         setWhatsAppCard(campaignTemplateData?.templeteView?.allTemplates);
        setLoader(false);
      }
      console.log("called")
    }
  ...
  },[campaignTemplateData?.templeteView?.template?.[0]?.template_id])

  const handleChange=(channel :any)=>{
    console.log("clicked???")
    setCampaignMessage("");
    setLoader(true);
    dispatch(
  campaignSliceActions.setTempleteView({
    channels: channel,
    allTemplates: campaignTemplateData?.templeteView?.allTemplates,
    ...campaignTemplateData?.templeteView?.allTemplates?.[channel]
  })
);
 ...
 useEffect(() => {
  ...
  const [accountDatas,setAccountDatas] :any= useState(null);
  console.log("accountDatas",accountDatas?.[0]?.mobileNumber);
  const getWhatsappDetails = async() => {
    try {
     
      await whatsappService.get(`/get_business_account/null`)
        .then((response) => {
  ...
  return (
    <>
      ...
      {/* {loader || true? <div className='loadersinner'><Loaders /></div> : */}
              ...
              <div className={`${styles.msgbody} msgbody `}>
                    <div className={styles.headertopmost}>
                      ...
                      <div className={styles.tabcontainer}>
                          ...
                          { Object.keys(campaignTemplateData?.templeteView?.allTemplates||{})?.map((item: any, ind: any) => {
                            return (
                              <>
                                {item == 'email' && (<div className={`${styles.tab} ${campaignTemplateData?.templeteView?.template?.[0]?.channel == "email" ? styles.active : ''}`} onClick={()=>campaignTemplateData?.templeteView?.template?.[0]?.channel != "email"?handleChange('email'):console.log("already clicked email")}> Email</div>)}
                                {item == 'sms' && (<div className={`${styles.tab} ${campaignTemplateData?.templeteView?.template?.[0]?.channel == "sms" ? styles.active : ''}`} onClick={()=>campaignTemplateData?.templeteView?.template?.[0]?.channel != "sms" ?handleChange('sms'):console.log("already clicked sms")}> Sms</div>)}
                                {item == 'whatsapp' && (<div className={`${styles.tab} ${(campaignTemplateData?.templeteView?.template?.[0]?.channel == "whatsapp" ||campaignTemplateData?.templeteView?.allTemplates?.isWhatsappCard)? styles.active : ''}`} onClick={()=>campaignTemplateData?.templeteView?.template?.[0]?.channel != "whatsapp"?handleChange('whatsapp'):console.log("already clicked whatsapp")}> Whatsapp</div>)}
                                {item == 'webpush' && (<div className={`${styles.tab} ${campaignTemplateData?.templeteView?.template?.[0]?.channel == "webpush" ? styles.active : ''}`} onClick={()=>campaignTemplateData?.templeteView?.template?.[0]?.channel != "webpush" ?handleChange('webpush'):console.log("already clicked webpush")}> Web Push</div>)}
                                {item == 'apppush' && (<div className={`${styles.tab} ${campaignTemplateData?.templeteView?.template?.[0]?.channel == "apppush" ? styles.active : ''}`} onClick={()=>campaignTemplateData?.templeteView?.template?.[0]?.channel != "apppush"?handleChange('apppush'):console.log("already clicked apppush")}> App Push</div>)}
                                {item == 'whatsapp_template' && (<div className={`${styles.tab} ${campaignTemplateData?.templeteView?.template?.[0]?.channel == "whatsapp_template" ? styles.active : ''}`} onClick={()=>campaignTemplateData?.templeteView?.template?.[0]?.channel != "whatsapp_template"?handleChange('whatsapp_template'):console.log("already clicked whatsapp_template")}> Whatsapp Template</div>)}
                                {item == 'inapp' && (<div className={`${styles.tab} ${campaignTemplateData?.templeteView?.template?.[0]?.channel == "inapp" ? styles.active : ''}`} onClick={()=>handleChange('inapp')}> In-App</div>)}
                              </>
                            )
                          })}
                      </div>
                    </div>
                <div ref={bottomRef} style={{ marginTop: "20px"  }}  className={`${styles.scroller} ${styles.chatflexscroll}`}>
                  <div className={styles.maincontainer}>
                    ...
                    {Array.isArray((campaignTemplateData?.templateConversation)) && (campaignTemplateData?.templateConversation)?.map((msg: any, index: number) => {
                      if (msg.sender === "userText" && campaignTemplateData?.templeteView?.template?.[0]?.channel==msg?.template_channel) {
                        return (
                          <>
                      ...
                      if (msg.sender === "botText" && campaignTemplateData?.templeteView?.template?.[0]?.channel==msg?.template_channel) {
                        ...
                        return (
                          <>
                                  ...
                                  {/* sms */}
                                 {msg?.intent =="sms_template" && <div className={`${styles.mobilelayoutviewsec} ${editsPromptSms ? styles.mobileheightzoom : ''}`}>
                                              ...
                                              {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                                  ...
                                                  <div className={styles.contentsec}>
                                                      <div className={styles.msgsec}>
                                                          <div dangerouslySetInnerHTML={{ __html: msg?.campaign_data?.template ??msg?.template?.template}} />
...
{/* inapp */}
                            {  msg?.intent =="inapp_template" &&    <div className={`${styles.mobilelayoutviewsec} ${editsPromptInAPP ? styles.mobileheightzoom : ''}`}>
                                      <div className={styles.bodywrapper}>
                                          <div className={styles.mobileheader}>
                                              <img className={styles.topbarimg} src={assets.mobile_topbar} alt="topbarimg" />
                                              <img className={styles.camerafront} src={assets.camera_front} alt="camerafront" />
                                              ...
                                              {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                                  ...
                                                  <div className={styles.contentsec}>
                                                      ...
                                                      <div className={styles.bigsaleummer}>
                                                          <div dangerouslySetInnerHTML={{ __html: msg?.campaign_data?.template??campaignMessageData?.templeteView?.template?.[0]?.template }} />
                                          ...
                                          <div className={`${styles.mobilemainsec} ${styles.mobilemargin} ${styles.whtsappmobmargin}`}>
                                              <div className={styles.userinfotop}>
                                                  ...
                                                  <div style={{ display: "flex", alignItems: "center", gap: "5px", width: "calc(100% - 54px)" }} className={styles.contactinfo}>
                                                      ...
                                                      <p style={{ margin: "0px" }}>{accountDatas?.[0]?.mobileNumber?`+${accountDatas?.[0]?.mobileNumber}`:"Business Whatsapp Number"}</p>
                                                  </div>
                                              </div>
                                              {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                                  <div className="spinnerbody" style={{ height: "320px" }}>
                                                  ...
                                                  <div className={`${styles.contentsec} ${styles.contentsecwhtapp}`}>
                                                      <div className={styles.msgsec}>
                                                          {/* Header */}
                                                          {(msg?.campaign_data?.headerType=="Image" || (campaignMessageData?.templeteView?.template?.[0]?.headerType === "Image" && index==0)  )&& (
                                                          // {(msg?.campaign_data?.headerType=="Image"  )&& (
                                                          ...
                                                          {((msg?.campaign_data?.headerType=="Text" || campaignMessageData?.templeteView?.template?.[0]?.headerType === "Text" )&& msg?.campaign_data?.headerType!="Image")&& (
                                                              <div className={styles.headertxtwhtsapp}>

                                                                  <p>{msg?.campaign_data?.headerValue??campaignMessageData?.templeteView?.template?.[0]?.headerValue}</p>
                                                                  {/* <p>{msg?.campaign_data?.headerValue?msg?.campaign_data?.headerValue:""}</p> */}
                                                                   {/* campaignMessageData?.templeteView?.template?.[0]?.headerValue}</p> */}
                                                              </div>
                                                          )}
                                                          ...
                                                          <div
                                                              ...
                                                              dangerouslySetInnerHTML={{
                                                                  __html:msg?.campaign_data?.footer?? campaignMessageData?.templeteView?.template?.[0]?.footer ?? "",
                                                              }}
                                                          />
                                                          {/* Buttons */}
                                                                  ...
                                                                  return (
                                                                      <>
                                                                          <div className={styles.insidebtn} key={index}>
                                                                             ...
                                                                             {button?.buttonText?.includes("https:")? <a href={button?.buttonText} target="_blank" className={`${styles.clickabletxt} clickabletxt`}>{label}</a>
                                                                              :<span className={`${styles.clickabletxt} clickabletxt`} onClick={()=>console.log("label>>>>>>>>", label,button?.action?.value,button)}>{label}</span>}
...
{/* chrome */}
                                 ...
                                 {  msg?.intent =="webpush_template" && 
                                  <div className={`${styles.mobcontainerwrapper} ${styles.chromendedgewrapper}`}>
                                 <div className={`${styles.discoversub} ${styles.chromecampaign} `}>
                                 ...
                                 <div className={`${styles.discoversub} ${styles.safaricampaign} `}>
                                      <div className={styles.discoversubcontent}>
                                          <div className={styles.prompteditor}>
                                              <img src={assets.Productivedots} alt="" />
                                          </div>
...
{/* email */}
                                  {msg?.intent =="email_template" &&
                                    <div className={`${styles.campaignmangerbody} ${styles.scrollerinner} `}>
                                          <div className={`${styles.scroller} ${styles.scrollerfix} ${styles.emailscroller} ${styles.heightreset}`}>
                                            {campaignMessageData?.isRegenerate ? (
                                              ...
                                              <div className={styles.responseslider}>
                                                ...
                                                <div className={styles.response}>
                                                  <div className={styles.subjtxt}>{`Subject: ${msg?.campaign_data?.subject? msg?.campaign_data?.subject:campaignMessageData?.templeteView?.template?.[emailVariant-1]?.subject || ""}`}</div>
                                                  <div
                                                    className="chatContent"
                                                    dangerouslySetInnerHTML={{
                                                      __html: msg?.campaign_data?.template?msg?.campaign_data?.template: campaignMessageData?.templeteView?.template?.[emailVariant-1]?.template || "",
                                                    }}
                                                  />
                                                </div>
                                              </div>)}
...
{/* app push */}
                                 {/* {true &&  */}
                                 {msg?.intent =="apppush_template" && 
                                <div className={styles.mobcontainerwrapper}>
                                 {/* <div className={styles.mobcontainerwrapper}> */}
                                          <div className={`${styles.mobilemainsec} ${styles.mobilemargin}`}>
                                            {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                              <div className="spinnerbody" style={{ height: "320px" }}>
                                              ...
                                              <div className={styles.contentsec}>
                                                <div className={`${styles.discoversub} ${styles.heightreset}`}>
                                                  <div className={`${styles.mobilelayoutviewsec} ${styles.mobilelayoutapppushnotify} ${ styles.lockscreenandroid } `}>
                                                    <div className={styles.bodywrapper}>
                                          ...
                                          <div className={`${styles.mobilemainsec} ${styles.mobilemargin}`}>
                                            {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                              ...
                                              <div className={styles.contentsec}>
                                                <div className={`${styles.discoversub} ${styles.heightreset}`}>
                                                  <div className={`${styles.mobilelayoutviewsec} ${styles.mobilelayoutapppushnotify} ${styles.lockscreeniphone} `}>
                                                    <div className={styles.bodywrapper}>
                                                      <div className={styles.pushnotifications}>
                                                        <img className={styles.notifylogo} src={assets.notify_logo} alt="notifylogo" />
                                                        <div className={styles.pushnotifycenter}>
       ...
       {  whatsappCard?.isWhatsappCard &&
                                          ...
                                          <div className={`${styles.mobilemainsec} ${styles.mobilemargin} ${styles.whtsappmobmargin}`}>
                                              ...
                                              {campaignMessageData?.isRegenerate || campaignMessageData?.isEdit ? (
                                                  ...
                                                  <div className={`${styles.contentsec} ${styles.contentsecwhtapp}`}>
                                                      <div className={styles.msgsec}>
                                                          {/* Header */}
                                                          {(whatsappCard?.whatsapp?.template?.headerType=="Image" || whatsappCard?.whatsapp?.template?.headerType === "Image" )&& (
                                                              <div className={styles.imgcon}>
                                                                  <img
                                                                      src={whatsappCard?.whatsapp?.template?.headerValue??whatsappCard?.whatsapp?.template?.headerValue}
                                                                      alt="header image"
                                                                  />
                                                              </div>
                                                          )}
                                                          {(whatsappCard?.whatsapp?.template?.headerType=="Text" || whatsappCard?.whatsapp?.template?.headerType === "Text" )&& (
                                                              <div className={styles.headertxtwhtsapp}>

                                                                  <p>{whatsappCard?.whatsapp?.template?.headerType??whatsappCard?.whatsapp?.template?.headerValue}</p>
                                                              </div>
                                                          )}
                                                          {/* Body */}
                                                          {/* <div
                                                              ...
                                                              dangerouslySetInnerHTML={{
                                                                  __html:
                                                                      whatsappCard?.whatsapp?.template?.bodymessage??whatsappCard?.whatsapp?.template?.bodymessage ?? "",
                                                              }}
                                                          /> */}
                                                          ...
                                                          <div
                                                              className={styles.footerwrapper}
                                                              dangerouslySetInnerHTML={{
                                                                  __html:whatsappCard?.whatsapp?.template?.footer?? whatsappCard?.whatsapp?.template?.footer ?? "",
                                                              }}
                                                          />
                                                          {/* Buttons */}
                                                          {(whatsappCard?.whatsapp?.template?.buttonType??whatsappCard?.whatsapp?.template?.buttonType)?.map(
                                                              (button: any, index: number) => {
                                                                  let label = "";
                                                                  if (button?.action?.value === "calltoAction") {
                                                                      label = button?.website?.label ?? button?.buttonText;
                      ...
                      <>
                        <div style={{ paddingBottom:"10px" }}>
                          <p className="generatings">
                            ...
                            {/* <span style={{lineHeight:"2px"}}></span> */}
                          </p>
                        </div>
                      </>
                    )}
               ...
               {!whatsappCard?.isWhatsappCard &&
                <div className={styles.searchblockouter}>
                  <div  className={`${styles.searchblock} ${styles.newinput} ${styles.variantinput}`}>
                    <div className={styles.searchmessage}>
                      <div className={styles.searchmessageText}>
                        ...
                        <div className={styles.inputcontainer}>
                          <textarea
                            ref={textareaRef}
                            placeholder="Modify the content as per your use case"
                        ...
                        {/* old template space editor working fine but changrd for ui */}
                        {/* <div className={`${styles.actionbottom} ${styles.actionbottomtop}`}>
                          ...
                          {(campaignTemplateData?.templeteView?.template?.length==2 && campaignTemplateData?.templeteView?.template?.[0]?.channel=='email')? < VariantDropdown variantEmail={variantEmail}/>:<></>}
                            </div>
                          <div>
                            <input
                              type="file"
