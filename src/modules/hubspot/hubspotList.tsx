...
export const HubspotList = () => {

    let navigate = useNavigate();
    const [getHubspotList, setHubspotList] = useState<any>([]);
    const [modal, isModal] = useState<boolean>(false);
    const [isLoading, setIsLoading] = useState<boolean>(false);

    useEffect(() => {
        try{
        whatsappService.get(`/privAppList`).then((resp: any) => {
            let response = resp.data;
            if (response.statusCode === 200) {
                setHubspotList(response.result);
            } else {
                setHubspotList([]);
            }
            setIsLoading(true);
        }).catch((err: any) => {
            console.log("err api app list", err);
            setHubspotList([]);
            setIsLoading(true);
        })
    }
    ...
    const removeConnection = () => {
        try{
        let particularConnection = getHubspotList[0];
        let payload = {
            connectorName: "HubSpot",
            connectionName: particularConnection.connectionName,
            contactSyncType: "pull",
            shopDomain: "",
            syncLength: 1
        }
        console.log("payload----",payload)
        whatsappService.post(payload, `/deleteConnection`).then((resp: any) => {
            let response = resp.data;
            if (response.statusCode === 200) {
                setHubspotList([]);
                isModal(false);
     ...
     <div className={styles.ticketright}>
            ...
            <div className={styles.topheading}>
                <div className={styles.tophdlefts}>
                    <h2>HubSpot</h2>
                    <p className={styles.headerdesctxt}>Integrate HubSpot for data synchronization and efficient campaign handling.</p>
                </div>
                <div className='formbtns'>
                  <button type='button' className='cancelbtn' onClick={() => isModal(true)}> Disable Connection</button>
                    {/* {getHubspotList.length === 0 && <button type='button' className='primarybtn' onClick={() => { navigate('/hubspotconnection') }}>New Connection</button>} */}
                </div>
            </div>

            {Array.isArray(getHubspotList) && getHubspotList.length > 0 && getHubspotList.map((data: any, index: number) => {
                return (
                    <>
                        <div className='viewtablecontainer' key={index}>
                            <div className='row'>
                                <div className='td first'>
                                    <div className={styles.viewrow}>
                                        <span className={styles.boldtxt}>{data.connectionName}</span>
                                        <div>
                                            <span>Last sync: {moment(data.syncTime).format('lll')}</span>
                                            {/* <span className={`${styles.statuselem} ${styles.processed}`}>{data.isConnected === 1 ? "Active" : "Disabled"}</span> */}
                                        </div>
                                    </div>
                                </div>
            ...
            {isLoading && getHubspotList.length === 0 && <div className={styles.hubspots}>
                <div className={styles.hubspotlefts}>
            ...
            {
                !isLoading && <div className={styles.hubspots}>
                    <div className={styles.hubspotlefts}>
                        <p>Loading</p>
                    </div>
                </div>
            }
            <div className='cancelnonemodal'>
                <WorktualModal
                    show={modal}
                    mdsize='modalsm'
                    footer={
                        <div className={styles.footerbtns}>
                            <button className='cancelbtn' onClick={() => isModal(false)} >Cancel</button>
                            <button className='cancelbtn disconnect' onClick={removeConnection} >Disconnect</button>
                        </div>
                    }
                >
                    <>
                        <p style={{ fontFamily: "intermedium", fontSize: "16px" }}> Are you sure you want to disable the connection?</p>
                    </>
                </WorktualModal>
