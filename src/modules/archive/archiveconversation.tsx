...
function ArchiveConversation() {
    ...
    const selectedDraftData=useSelector((state :any)=>state.commonData.archiveData);
                        ...
                        <div className={`${styles.innerbody} `}>
                            ...
                            {<div className='draftheader'>
                                ...
                                <span style={{ display: "flex", gap: "16px", alignItems: "center" }}>
                                    ...
                                    <span style={{ cursor: "pointer" }}><button className='primarybtn' onClick={() =>{ handleArchive(selectedDraftData?.draft)}}>UnArchive</button></span>

                                </span>

                            </div>}
                            <div className={`${styles.msgbody} msgbody`}>
                                <div style={{ marginTop: "20px" }} className={`${styles.scroller} ${styles.chatflexscroll}`}>
                                                                    ...
                                                                    {Object.entries(msg?.campaign_data?.channels)?.map(([channelKey, channelData]: any) => (
                                                                        <div key={channelKey} className={styles.card}>
                                                                            ...
                                                                            <div className={`${styles.top} ${styles.bottom}`}>
                                                                                ...
                                                                                {channelKey === "email" &&
                                                                                    <button onClick={() => handleViewEmail(channelKey, channelData, index)}>View</button>
                                                                                }
                                                                                {channelKey === "sms" &&
                                                                                    <button onClick={() => handleViewSms(channelKey, channelData, index)}>View</button>
                                                                                }
                                                                                {channelKey === "apppush" &&
                                                                                    <button onClick={() => handleViewAppPush(channelKey, channelData, index)}>View</button>
                                                                                }
                                                                                {channelKey === "webpush" &&
                                                                                    <button onClick={() => handleViewWebPush(channelKey, channelData, index)}>View</button>
                                                                                }
                                                                                {channelKey === "whatsapp" &&
                                                                                    <button onClick={() => handleViewWhatsapp(channelKey, channelData, index)}>View</button>
                                                                                }
                                                                                {channelKey === "inapp" &&
                                                                                    <button onClick={() => handleViewInapp(channelKey, channelData, index)}>View</button>
                                                                                }
                                                                            </div>
                                                                        </div>
                                                                    ))}
