...
const B2ccarousel = (props) => {
  const navigate = useNavigate();
  const [currentIndex, setCurrentIndex] = useState(0);

  const handleNext = () => {
    if (currentIndex < props?.b2cProductData.length - 1) {
  ...
  const currentProduct = props?.b2cProductData[currentIndex];
  const hierarchybranch = currentProduct?.hierarchybranch?.split('>')


  // function toRadians(degrees) {
  //   return degrees * (Math.PI / 180);
  // }
  // function calculateDistance(lat2, long2) {

  //   let Latitude = userLatitude == undefined ? 0 : userLatitude;
  //   let Longitude = userLongitude == undefined ? 0 : userLongitude
  //   const lat1_r = toRadians(Latitude);
  //   const long1_r = toRadians(Longitude);
  //   const lat2_r = toRadians(lat2);
  //   const long2_r = toRadians(long2);
  //   const distance = Math.acos(
  //     Math.sin(lat1_r) * Math.sin(lat2_r) +
  //     Math.cos(lat1_r) * Math.cos(lat2_r) * Math.cos(long2_r - long1_r)
  //   ) * EARTH_RADIUS;
  //   const distanceMiles = distance * KM_TO_MILES;
  //   return parseInt(distanceMiles);
  // }
  // function getDistance(address) {
  //   console.log("address", JSON.parse(address))
  //   let addressdata = JSON.parse(address)
  //   let minDistance = Infinity;
  //   addressdata?.map(store => {
  //     const distance = calculateDistance(parseFloat(store.latitude), parseFloat(store.longitude));
  ...
  return (
    <div className="worktualcarosuelmain">
      ...
      <svg width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handlePrev} className={`${currentIndex === 0 ? "disable" : ""}`}>
      ...
      <div class="content-wrapper b2ccare" >
        <div >
          <div className='worktualcarosuelblock worktualselect'>
            <div className="worktualcarosuelimages">
              <img src={currentProduct?.productImages} alt='product_image' />
            </div>
            <h3 className='carosuelproduct'>{hierarchybranch[hierarchybranch?.length - 1]}</h3>
            <h3 className='productunique'>{currentProduct?.productName}</h3>
            <h3 className="worktualpricing">£ {currentProduct?.price}</h3>
            <h3 className="worktualpricings">{currentProduct?.sellerName}</h3>
            {/* <div className="selectablerow">
            <button className='selectfirst'>Select</button>
            <button className='selectfirst'>Available at </button>
            </div> */}
            {/* <h3 className="worktualpricing">{getDistance(currentProduct?.companyAddress)} Miles Away</h3> */}
          </div>
        </div>
      </div>


      {/* <svg width="63" height="81" viewBox="0 0 63 81" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handleNext} className={`${props?.b2cProductData?.length === currentIndex + 1 ? "disable" : ""}`}>
      ...
      <svg width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handleNext} className={`${props?.b2cProductData?.length === currentIndex + 1 ? "disable" : ""}`}>
