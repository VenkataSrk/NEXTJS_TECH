...
xGateConnector::xGateConnector(const char* serverType): 
  ...
  m_connectTimeOut(3)
{
  XGLOG_INFO( "xGateConnector::xGateConnector(%s)", (char*)ur_log_string (serverType));
  ...
  XGLOG_INFO( "xGateConnector::xGateConnector(%s)"\
      " Timer Interval :%d Connection Time Out:%d", (char*)ur_log_string (m_serverType), m_timerInterval, m_connectTimeOut);
}
...
xGateRetVal xGateConnector::init(vector<xGateListenAddress> listenAddressList)
{
  XGLOG_INFO( "xGateConnector::init(%s)", (char*)ur_log_string (m_serverType));
  for( unsigned int i =0 ; i < listenAddressList.size();i++)
  {
    ...
    m_ConnectorList.push_back(listenAddressList[i]);
  }
  ...
  {
    // Spawn Failure
    XGLOG_INFO( "xGateConnector::init(%s)ace error while spawning thread", (char*)ur_log_string (m_serverType));
...
{
  XGLOG_INFO( "xGateConnector::~xGateConnector(%s) destructor called", (char*)ur_log_string (m_serverType));
...
{
  XGLOG_INFO( "xGateConnector::stopThread(%s) called", (char*)ur_log_string (m_serverType));
  ...
  XGLOG_INFO( "xGateConnector::stopThread(%s) Stopped xGateConnector", (char*)ur_log_string (m_serverType));
  return EN_XGATE_STATUS_SUCCESS;
} 
