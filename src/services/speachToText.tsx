...
import editorvoicewhite from "../assets/images/editor/voiceicon.svg";
import styles from "../assets/styles/commoneditor.module.scss";

import { v4 as uuid } from 'uuid'
import { config } from "../base/constant";
import NlpWebSocketService from "./nlp_stt";
...
export const SpeachToText = ({ textAreaRef, textareaId, onText, placeholderText, page, open, setIsOpen }: any) => {
    ...
    const insertAtCursor = (newText: string) => {
        try {
            ...
            const currentTextRaw = el.innerText.trim() || el.innerHTML.trim();
    ...
    return (
        <div className={`${styles.micIcon} ${micOn ? styles.active : ""}`}>
            <img
                src={editorvoicewhite}
                alt="Voice"
                onClick={micToggle}
            />
        </div>
