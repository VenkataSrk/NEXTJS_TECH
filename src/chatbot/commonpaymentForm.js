...
import { newChatSendMsg, postIntegrationApiData, checkoutProductStock } from "./utils";
...
function CommonPaymentForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        const createTransactionInput = {
            ...
            city: "New York",
            postalCode: "10003",
            token: builderList?.bigCommerceOrderToken,
            paymentMethodId: builderList?.bigCommerceSelectedId[0]?.id
        }
        const url = `${config().BIG_COMMERCE_PAYMENT}`
        const response = await postIntegrationApiData(url, builderList?.ecommercePurchase?.token, createTransactionInput)
        store.dispatch(chatBuilderActions.setCommonPaymentForm(false));
        const recommendMsg = `Your order is placed successfully`
        newChatSendMsg('Text', recommendMsg, "IN", 0);
...
export default CommonPaymentForm;