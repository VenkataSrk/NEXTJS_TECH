...
bool MBCLinkMsgProcessor::read_media_detail(const Value& val, MgMediaDetail& detail)
{
  ...
  if(val.HasMember("prev_act_spkr")) {
    detail.prev_act_spkr = val["prev_act_spkr"].GetString();
  } else {
    XGLOG_ERROR("read_media_detail failed. 'prev_act_spkr' field is missing !");
    return false;
  }
                                        ...
                                        case EN_XGATE_MG_SDP_UPDATE_REQUEST:
                                                {
                                                        ...
                                                        break;
																				...
																				case EN_XGATE_MG_ACTIVE_SPEAKER_INFO:
																								{
																									pMsg->setOpCode(EN_MBC_LINK_EVENT_ACTIVE_SPEAKER_INFO);
																									pMsg->setDstModuleId(IURDefines::UR_MODULE_SERVICE_CONTROLLER);
																									pMsg->setMsgType(EN_XGATE_MSG_MGC_MG_OUT);
																									break;
																								}
