...
int MBCLinkTcpAcceptor::handle_connection(ACE_HANDLE aceHandle)
{
  ...
  send_client_connected(fd,peerIp,peerPort);
...
//To send MBC connected info to controller
xGateRetVal MBCLinkTcpAcceptor::send_client_connected(unsigned short fd,string Ip,unsigned short port)
{
  ...
  pMsg->setOpCode(EN_MBC_LINK_EVENT_CLIENT_CONNECTED);
  ...
  XGLOG_INFO("MBCLinkTcpAcceptor::send_client_connected client connected info fd:(%d),IP:(%s)",fd,Ip.c_str());
