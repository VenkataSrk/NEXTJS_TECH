...
xGateDBConnector::xGateDBConnector(std::string &dbSrvrIpAddr, unsigned int &dbSrvrPort, std::string &username, std::string &password, std::string &database, DBConnectionType dbConnType)
{
  ...
  //MSSQL connection string
    ...
    sprintf((char*)connectstr,"DRIVER=FreeTDS;SERVER=%s;UID=%s;PWD=%s;DATABASE=%s;TDS_version=7.0;port=%d;", dbSrvrIpAddr.c_str(), username.c_str(), password.c_str(), database.c_str(), dbSrvrPort);
  #endif
  if(dbConnType == DBConnectionType::EN_DB_CONNECTION_TYPE_MYSQL)
  {
  ...
  else if(dbConnType == DBConnectionType::EN_DB_CONNECTION_TYPE_POSTGRESQL)
  {
    // CHANGE TO — PostgreSQL
    sprintf((char *)connectstr, "DRIVER={PostgreSQL Unicode};SERVER=%s;UID=%s;PWD=%s;DATABASE=%s;port=%d;",
            dbSrvrIpAddr.c_str(), username.c_str(), password.c_str(), database.c_str(), dbSrvrPort);
  }
  else
  {
    XGLOG_ERROR("Unsupported DB Connection Type specified: %d", static_cast<int>(dbConnType));
    return;
  }
