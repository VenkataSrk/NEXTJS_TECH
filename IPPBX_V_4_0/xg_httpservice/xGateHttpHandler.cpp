...
#else
  XGLOG_INFO("sending http_request to url: %s for trans_id: %s @ %s", \
...
#else
  //XGLOG_INFO("got http response (%s)", httpReqInfo->m_output.c_str());
#endif
  return written;
}
...
bool xGateHttpHandler::getHttpResponse()
{
  ...
  while(still_running) {
	...
	#else
			  //XGLOG_INFO("Received http response (%d %s) for trans_id: %s", \
