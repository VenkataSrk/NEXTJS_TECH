...
#if 0
/** RTP payload constants. // now declared in quality.h
*/
...
class Rtp_Media : public Media, public Rtp_Quality_Source {
  public:
...
#ifdef XGATE_HAS_HMP
    // bool   PlayFileWav(char const *filename, const char* targetHost, bool autorepeat);
    bool   PlayFileWav(char const *filename, bool autorepeat);
    bool   PlayFileWav(char const *filename, bool autorepeat,void *call);

    bool   StopFileWav(int waitTime);
    bool   RecordFile(const char *filename, Rtp_Media *rtpMedia2);
    void   EndRecordFile();
    bool   ConfCreateRoom(const char *room_name);
    bool   ConfAddParty(const char *room_name);
    bool   ConfRecordFile(const char *room_name, const char *filename);
    void ConfEndRecordFile(const char *room_name);
    int    GetDtmfDigit(char* dtmf_digits);
    void   SetDtmfDigitTimeout(int max_time);
    bool   AttachTask(void *call);
    ...
    */
    ...
    //	  void   GetDtmfDigit(int number_of_digits, int wait_time, char* dtmf_digits);
...
////#ifdef XGATE_HAS_MGC_MG_COMMUNICATION
    bool allocate_interface(Call *call, Media *&media, CallDirection dir, CallParameter *cparam, char *epName);
    /*void set_interface_config( RLS_RECORD *rec);
    bool is_signal_matched(SignallingType sig_type);*/
////#endif
    ...
    */
    ...
    /*string getNextSbcAddr() {
      return m_sbcAddrList[0];
    }*/
    string selectSbcAddrFromList() 
    {
      unsigned int size = m_sbcAddrList.size();
      if(size > 0) {
        m_selectedSbcIndex  = (m_selectedSbcIndex % size);
        if(m_selectedSbcIndex > size)
          m_selectedSbcIndex = 0;
        return m_sbcAddrList[m_selectedSbcIndex++];
      }
      return "";
    }

    vector<std::string> getSbcAddrList()
    {
      return m_sbcAddrList;
    }
    void addSbcAddrInList(std::string& sbcAddr);
#endif
    /**
     * Get RTP_Config from RTP interface settings.
     */
...
//////added for ippbx
    /**
 *  get the Routing mode for sip proxy dial
 */
    std::string GetProxyRoute() {return proxyroute;};


  private:
#ifdef XGATE_HAS_ACME_SBC
  vector<std::string> m_sbcAddrList;
  static unsigned int m_selectedSbcIndex;
#endif
    AcChannelParam channelParam;
    Rtp_Quality_Monitor _QMonitor;
	std::string proxyroute; // added for ippbx
