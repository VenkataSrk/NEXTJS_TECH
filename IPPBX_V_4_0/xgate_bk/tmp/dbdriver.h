...
private:
    ...
    int Checkdatabase_connectivity(String connect_string);

    DB_Async_Object *GetTaskExclusive(CDbTLS *tls);
    int handle_event_count;
    int db_profile_not_reachable_count;
    int can_reload_db_profile;
};
...
public:
    ...
    //STATUS  API_DbOpen_and_DbGetValStr( CTask *task, const char *profile, const char *sql, char* field, char* password);
...
/********************
    STATUS API_CALL API_DbOpen_and_DbGetValStr( CTask *task, const char *profile, const char *sql, char* field, char* password) {
        return m_Driver->API_DbOpen_and_DbGetValStr( task, profile, sql, field, password);
    }
*******************/
