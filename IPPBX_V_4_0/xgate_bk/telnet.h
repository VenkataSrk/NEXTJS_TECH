...
#ifndef _CTELNETSOCKET//SL
#define _CTELNETSOCKET//SL

#define ifdef P_USE_PRAGMA
#pragma interface
#endif


#include "../pwlib/include/ptlib/contain.h"

#define PINDEX int	//linux porting
typedef unsigned short WORD;	//linux porting

class PBYTEArray;	//linux porting
class PString;	//linux porting


//SL
//#undef String //ptlib Strings conflicts with xgate Strings
//#define

//# regex_t ptlib_regex_t//regex_t defined in both ptlib and pcre so this stops conflicts hopefully 
//#include <ptlib.h>
//#undef regex_t
...
class PTelnetSocket
{
  ...
  // Overrides from class PChannel
     ...
     */
    virtual bool Accept(
      int & socket          // Listening socket making the connection.
    );
  ...
  // New functions
    ...
    void SetOurOption(
     // BYTE code,          // Option to check.
      ...
      bool state // New state for for option.
...
/*
    ...
    const PString & GetTerminalType() const { return terminalType; }  //CBM reverted back
    //const PString  *const& GetTerminalType() const { return terminalType; }	//linux porting
...
//    PString *terminalType;	//linux porting
    PString terminalType; //CBM
  ...
  private:
    ...
    PBYTEArray subOption; //CBM // reverted back
//    PBYTEArray *subOption;	//linux porting
    ...
    //this stuff added by switchlab
    //    |    |    |
    //    v    v    v
    //
    bool WriteOutOfBand(const unsigned char *buf, int len);

    int lastWriteCount, lastReadCount;
    int os_handle;
    Circular_Buffer* cb;
    HANDLE events[2];
    DWORD self_thread_id;
    DWORD read_timeout;
    bool opened;
  ...
  public:
    ...
    void Close();
    bool IsOpen();
    bool MyRead(unsigned char *buf, int len);
    bool MyWrite(const unsigned char* buf, int len);
    void SetReadTimeout(unsigned newval);

    int GetHandle() const {
	return os_handle;
    }
    int GetHandledummy() const;
/*{
	return os_handle;
    }*/
