...
gboolean xGateHmpGstPipeline::simple_pipeline_create() 
{
  ...
  m_sendBusId = gst_bus_add_watch(m_sendBus, &xGateHmpGstPipeline::handle_event, this);

  m_recvBus = gst_pipeline_get_bus(GST_PIPELINE(m_recvPipeline));
  m_recvBusId = gst_bus_add_watch(m_recvBus, &xGateHmpGstPipeline::handle_event, this);
...
gboolean xGateHmpGstPipeline::conf_pipeline_create()
{
  ...
  m_confBusId = gst_bus_add_watch(m_confBus, &xGateHmpGstPipeline::handle_event, this);
...
gboolean xGateHmpGstPipeline::rbt_pipeline_create()
{
  ...
  m_rbtBusId = gst_bus_add_watch(m_rbtBus, &xGateHmpGstPipeline::handle_event, this);
...
gboolean xGateHmpGstPipeline::play_ringback_tone(Client *sClient, bool autorepeat)
{
   string client_ip("");
   unsigned int client_port = 0;
   xGateHmpGstUdp *hmpUdprbt = NULL;
   xGateHmpGstBin *hmpBinrbt = NULL;
   GSocket *udpSockrbt = NULL;
   CLog::Detail(THISMODULE, "xGateHmpGstPipeline::play_ringback_tone in %s", m_rbtPipelineName.c_str());

   ClientDetail &client_Detail = sClient->m_detail;
   client_ip = client_Detail.m_ip;
   client_port = client_Detail.m_port;

   hmpUdprbt = sClient->m_hmpUdp;
   udpSockrbt = sClient->m_hmpUdp->m_udpSocket;
  ...
  //   m_autoRepeat = autorepeat;
   ...
   if(hmpUdprbt && !client_ip.empty() && client_port > 0) {
      hmpUdprbt->add_client_rbt(client_ip, client_port);
      g_object_set(G_OBJECT(hmpUdprbt->m_rudpSink), "socket", udpSockrbt, NULL);
   }
...
gboolean xGateHmpGstPipeline::pipeline_connect_clients(Client *sClient, Client *dClient)
{
  ...
  hmpUdpA = sClient->m_hmpUdp;
  udpSockA = sClient->m_hmpUdp->m_udpSocket;
  ...
  hmpUdpB = dClient->m_hmpUdp;
  udpSockB = dClient->m_hmpUdp->m_udpSocket;
  ...
  if(hmpUdpA && udpSockB && !clientA_ip.empty() && clientA_port > 0) {
      hmpUdpA->add_client_addr(clientA_ip, clientA_port);     
      g_object_set(G_OBJECT(hmpUdpA->m_udpSink), "socket", udpSockB, NULL);
      g_object_set(G_OBJECT(hmpUdpA->m_udpSrc), "socket", udpSockB, NULL);
      if (clientA_Detail.m_mediaMode == EN_MEDIA_DTLS) {
         hmpBinA->set_stun_target_client(clientA_ip, clientA_port, hmpUdpB);        
      }
  } else {
  ...
  if(hmpUdpB && udpSockA && !clientB_ip.empty() && clientB_port > 0) {
      hmpUdpB->add_client_addr(clientB_ip, clientB_port);     
      g_object_set(G_OBJECT(hmpUdpB->m_udpSink), "socket", udpSockA, NULL);
      g_object_set(G_OBJECT(hmpUdpB->m_udpSrc), "socket", udpSockA, NULL);
      if (clientB_Detail.m_mediaMode == EN_MEDIA_DTLS) {
         hmpBinB->set_stun_target_client(clientB_ip, clientB_port, hmpUdpA);      
      }
  } else {
...
gboolean xGateHmpGstPipeline::stop_ringback_tone(Client *client)
{
  xGateHmpGstBin *hmpBinrbt = NULL;
  CLog::Detail(THISMODULE, "HostGSTRtpBin::stop_ringback_tone: Stop playing ringback tone\n");
  gst_element_set_state(m_rbtPipeline, GST_STATE_NULL);
...
gboolean xGateHmpGstPipeline::handle_event(GstBus* bus, GstMessage* msg, gpointer data)
{
  //CLog::Detail(THISMODULE, "Enter xGateHmpGstPipeline::handle_event"); //this log priting frequently. so commented
...
/* HmpGstUdp class
*/
xGateHmpGstUdp::xGateHmpGstUdp() 
{
  CLog::Detail(THISMODULE, "xGateHmpGstUdpSrc::xGateHmpGstUdpSrc Construct");
  m_udpSrc = NULL; 
  m_udpSink = NULL;
  m_rudpSink = NULL;
  m_udpSocket = NULL;
  ...
  m_udpCaps = NULL;
  m_isLinked = FALSE;
  //m_codecType = EN_PCMA_CODEC;
}

xGateHmpGstUdp::~xGateHmpGstUdp() 
{
  CLog::Detail(THISMODULE, "xGateHmpGstUdpSrc::xGateHmpGstUdpSrc Destructing");
 
  if(m_udpCaps) {
    g_object_unref(G_OBJECT(m_udpCaps));
    m_udpCaps = NULL;
  }
...
gboolean xGateHmpGstUdp::create_udp_src_sink(HmpGstCodecType codecType, unsigned int serverPort)
{
  ...
  if(retVal) {
    ...
    //setting properties
    g_object_set(G_OBJECT(m_udpSrc),"socket", m_udpSocket, NULL);
    g_object_set(G_OBJECT(m_udpSrc),"timeout", 0, NULL);
    g_object_set(G_OBJECT(m_udpSrc),"reuse", TRUE, NULL);
    g_object_set(G_OBJECT(m_udpSrc), "close-socket", FALSE, NULL);
#if 0
    if(!set_udp_caps(m_codecType)) {
      CLog::Error(THISMODULE, "xGateHmpGstUdpSrc::xGateHmpGstUdpSrc set_udp_caps failed");
      return FALSE;
    }
#endif
  }
  
  m_udpSink = gst_element_factory_make("multiudpsink", NULL);

  if(!m_udpSink) {
    CLog::Error(THISMODULE, "xGateHmpGstUdpSink::create_udp_sink gst_element_make failed");
    return FALSE;
  }
  g_object_set(G_OBJECT(m_udpSink), "close-socket", FALSE, NULL);
  
  return TRUE; 
}

gboolean xGateHmpGstUdp::create_udprbt_sink()
{
  m_rudpSink = gst_element_factory_make("multiudpsink", NULL);
  ...
  g_object_set(G_OBJECT(m_rudpSink), "close-socket", FALSE, NULL);

  return TRUE;
}

gboolean xGateHmpGstUdp::create_udp_socket() 
{
...
gboolean xGateHmpGstUdp::set_udp_caps(HmpGstCodecType codecType) 
{
  CLog::Detail(THISMODULE, "xGateHmpGstUdpSrc::set_udp_caps for CodecType: %d", codecType);

  switch(codecType) {
    case EN_PCMU_CODEC:
      m_udpCaps = gst_caps_from_string(UDP_CAPS_G711U);
      break;
    case EN_RTP_CODEC:
    case EN_PCMA_CODEC:
      m_udpCaps = gst_caps_from_string(UDP_CAPS_G711A);
      break;
    case EN_G729_CODEC:
      m_udpCaps = gst_caps_from_string(UDP_CAPS_G729);
      break;
    case EN_OPUS_CODEC:
      m_udpCaps = gst_caps_from_string(UDP_CAPS_OPUS);
      break;
    default:
      CLog::Warning(THISMODULE, "xGateHmpGstUdpSrc::xGateHmpGstUdpSrc setting default PCMA caps");
      m_udpCaps = gst_caps_from_string(UDP_CAPS_G711A); //default is PCMA
  } 

  if(!m_udpCaps) {
    CLog::Error(THISMODULE, "xGateHmpGstUdpSrc::xGateHmpGstUdpSrc make udpCaps failed");
    return FALSE;  
  }

  g_object_set(G_OBJECT(m_udpSrc), "caps", m_udpCaps, NULL);
  CLog::Detail(THISMODULE, "xGateHmpGstUdpSrc::set_udp_caps success for codec: %d", codecType);
  return TRUE;
}

void xGateHmpGstUdp::set_server_addr(string serverIp, unsigned int serverPort)
{
  m_serverIp = serverIp;  
  m_udpPort = serverPort;
...
void xGateHmpGstUdp::set_client_addr(string clientIp, unsigned int clientPort)
{
  m_clientIp = clientIp;
...
void xGateHmpGstUdp::add_client_addr(string clientIp, unsigned int clientPort)
{
  ...
  g_object_set(G_OBJECT(m_udpSink), "clients", buff, NULL);
}

void xGateHmpGstUdp::add_client_rbt(string clientIp, unsigned int clientPort)
{
  ...
  g_object_set(G_OBJECT(m_rudpSink), "clients", client_rbt, NULL);
  g_object_set(G_OBJECT(m_rudpSink),"async", FALSE, "sync", TRUE, NULL);
}

void xGateHmpGstUdp::remove_client_addr()
{
  CLog::Detail(THISMODULE, "xGateHmpGstUdpSink::remove_client_addr");
  g_signal_emit_by_name(m_udpSink, "clear", this); 
}

void xGateHmpGstUdp::update_socket(GSocket *udpSocket)
{
  CLog::Detail(THISMODULE, "xGateHmpGstUdpSink::update_socket");
  g_object_set(G_OBJECT(m_udpSink), "socket", udpSocket, NULL);
}
...
gboolean xGateHmpGstBin::ringbkBin_create(xGateHmpGstUdp *udpSink, char* filename)
{
    m_ringbkBin = GST_BIN(gst_bin_new(m_ringbkBinName.c_str()));
    gint64 ptime;

    m_fileSrc = gst_element_factory_make ("filesrc", NULL);
    g_object_set(G_OBJECT(m_fileSrc),"location", filename, NULL);
    ...
    m_rbtUdp = GST_ELEMENT(udpSink->m_rudpSink);

    gst_bin_add_many(m_ringbkBin, m_fileSrc, m_wavParse, m_rbtEnc, m_rtpRbtPay, m_rbtUdp, NULL);
    gst_element_link_many(m_fileSrc, m_wavParse, m_rbtEnc, m_rtpRbtPay, m_rbtUdp, NULL);
...
GstBin* xGateHmpGstBin::sendBin_create(xGateHmpGstUdp *udpSink) {
...
gboolean xGateHmpGstBin::sendBin_remove(xGateHmpGstUdp *phmpUdp)
{
   CLog::Detail(THISMODULE, "xGateHmpGstBin::sendBin_remove Started");
   m_sendUdp      = GST_ELEMENT(phmpUdp->m_udpSink);
...
gboolean xGateHmpGstBin::sendBin_update(xGateHmpGstUdp *phmpUdp)
{
   m_sendUdp      = GST_ELEMENT(phmpUdp->m_udpSink);
...
GstBin * xGateHmpGstBin::recvBin_create(xGateHmpGstUdp *udpSrc) {
  CLog::Detail(THISMODULE, "xGateHmpGstBin::bin_create bass class implementation return NULL");
  gboolean retVal = FALSE;
  GstPad *pad;
  ...
  m_recvUdp = GST_ELEMENT(udpSrc->m_udpSrc);
...
gboolean xGateHmpGstBin::recvBin_remove(xGateHmpGstUdp *phmpUdp)
{
   CLog::Detail(THISMODULE, "xGateHmpGstBin::recvBin_remove Started");
   m_recvUdp         = GST_ELEMENT(phmpUdp->m_udpSrc);
...
gboolean xGateHmpGstBin::recvBin_update(xGateHmpGstUdp *phmpUdp)
{
   m_recvUdp         = GST_ELEMENT(phmpUdp->m_udpSrc);
...
void xGateHmpGstBin::set_stun_target_client(string client_ip, unsigned int client_port, xGateHmpGstUdp *udpSrc)
{
...
gboolean HmpG711Bin::ringbkBin_create(xGateHmpGstUdp *udpSink, char* filename)
{
...
GstBin * HmpG711Bin::sendBin_create(xGateHmpGstUdp *udpSink) 
{
...
GstBin * HmpG711Bin::recvBin_create(xGateHmpGstUdp *udpSrc) {
...
void HmpG711Bin::set_stun_target_client(string client_ip, unsigned int client_port, xGateHmpGstUdp *udpSrc)
{
   sockfd = g_socket_get_fd(udpSrc->m_udpSocket);
...
gboolean HmpG729Bin::ringbkBin_create(xGateHmpGstUdp *udpSink, char* filename)
{
 return TRUE;
}

GstBin * HmpG729Bin::sendBin_create(xGateHmpGstUdp *udpSink) {
...
GstBin * HmpG729Bin::recvBin_create(xGateHmpGstUdp *udpSrc) {
...
void HmpG729Bin::set_stun_target_client(string client_ip, unsigned int client_port, xGateHmpGstUdp *udpSrc)
{
   sockfd = g_socket_get_fd(udpSrc->m_udpSocket);
   sprintf(targetIp,client_ip.c_str());
   targetPort = client_port;
...
gboolean HmpOpusBin::ringbkBin_create(xGateHmpGstUdp *udpSink, char* filename)
{
  return TRUE;
}

GstBin * HmpOpusBin::sendBin_create(xGateHmpGstUdp *udpSink) {
...
GstBin * HmpOpusBin::recvBin_create(xGateHmpGstUdp *udpSrc) {
  CLog::Detail(THISMODULE, "HmpOpusRecvBin::bin_create (creating OpusRecvBin)");
  gboolean retVal = FALSE;

  m_recvBin = GST_BIN(gst_bin_new(m_recvBinName.c_str()));

  GstElement* m_recvUdp = GST_ELEMENT(udpSrc->m_udpSrc);
  m_rtpJitterBuffer = gst_element_factory_make("rtpjitterbuffer", NULL);
...
void HmpOpusBin::set_stun_target_client(string client_ip, unsigned int client_port, xGateHmpGstUdp *udpSrc)
{
   sockfd = g_socket_get_fd(udpSrc->m_udpSocket);
   sprintf(targetIp,client_ip.c_str());
   targetPort = client_port;
...
xGateHmpGstBin * HmpGstBinFactory::bin_create(string& binName, HmpGstBinType binType, xGateHmpGstUdp *hmpUdp, ClientDetail &cliDetail)
{
  ...
  switch(binType) {
    ...
    case EN_BIN_RTP:
        ...
        pHmpGstBin->sendBin_create(hmpUdp);
        pHmpGstBin->recvBin_create(hmpUdp);
        break;
