...
int xGateHmpGstPipeline::get_dtmf_digit(char* dtmf_digits)
{
  ...
  CLog::Debug(THISMODULE, "HostGSTRtpBin::getDtmfDigit : [%s] \n", dtmf_digits);
  return retval;
}

void xGateHmpGstPipeline::SetDtmfDigitTimeout(int max_time)
{
  struct timespec ts;
  int rc = clock_gettime(CLOCK_MONOTONIC, &ts);
  mDtmfDigitTimeout = TimeSpecToSeconds(&ts) + (double)max_time;
  CLog::Debug(THISMODULE, "HostGSTRtpBin::setDtmfDigitTimeout calculated elapsed time is: [%d]\n", mDtmfDigitTimeout);
}
