...
void DB_Connection_Slot::HandleEvents()
{
  ...
  if (dbConnection) {
    ...
    //Reconnect(false); //TODO: Yoga testing purpose
    lastConnectTry = time(NULL);
  }
    ...
    const char *sql)
{
  ...
  if (GetState() != State_Active || !dbConnection) {
    ...
    printf("\n ERROR  %d: connection is NOT available", this->slotIndex);
    return NULL;
  }
  ...
  if (dbConnection->GetExclusiveObject () == NULL) {
    ...
    /* printf("%s: Using connection slot %u. %s",
       dbProfile.Name().c_str(),slotIndex,(task_has_exclusive_lock ? "WARNING: task has exclusive lock" : "")); */
    ...
    /* printf("%s: Using connection slot %u (currently locked by task %u)\n",dbProfile.Name().c_str(),slotIndex,
  ...
  CLog::Detail (DBDRIVER, task, 
      ...
      slotIndex, 
      dbConnection->GetExclusiveOwner()->GetPID());
  printf("\n ERROR : %s: Database connection %u is locked by task %u and can not be used\n",
      dbProfile.Name().c_str(),slotIndex,dbConnection->GetExclusiveOwner()->GetPID());
...
void DB_Connection_Slot::Show (String & output)
{
  ...
  switch (GetState()) {
    ...
    case State_Connecting:  statestr = "trying"; break;
    case State_Holding:	    statestr = "waiting"; break;
    case State_Failed:	    statestr = "FAILED"; break;
    case State_Active:	    statestr = "active"; break;
    case State_Closing:	    statestr = "closing"; break;
    case State_CloseWait:   statestr = "closewt"; break;
  }
...
void DB_Service::DetachTask(CTask *task)
{
  ...
  for (int i=0; i<MAX_DB_HANDLE; i++) {
    ...
    /* Hack fix -- Srinivas - remove later once RCA found 
    */
    if (obj) {
...
int DB_Service::AllocHandle(CTask *task, const char *sql)
{
  ...
  /* CLog::Error( DBDRIVER, task, "AllocHandle: out of handles (max: %d). SQL='%s'",
     MAX_DB_HANDLE-1, sql); 
     */
  printf("\n DB_Service::AllocHandle: AllocHandle: out of handles (max: %d). SQL='%s'",MAX_DB_HANDLE-1, sql);
  task->SetLastError(-1);
  return -1;
}
    ...
    int *handle, const char *sql)
{
  ...
  *handle = AllocHandle(task, sql);
  if (*handle == -1)
  {
    printf("\n %s : %s DB_Service::CreateRecordset AllocHandle return -1, SQL %s",__FUNCTION__,__LINE__, sql);
    return 0;
  }
...
STATUS DB_Service::API_DbExecute( CTask *task, const char *profile_name, const char *sql)
{
  ...
  if (tls==NULL) {
    ...
    */
    //        printf("\n dbExecSQL '%s': Task doesn't have database connection", sql);
    task->SetLastError(-1);
    return OK;
  }
...
STATUS DB_Service::API_DbGetRowsAffected( CTask *task, int *nrow)
{
  ...
  if (tls->m_Handle[0] == NULL) {
    ...
    printf("\n DbGetRowsAffected: invalid handle\n");
    task->SetLastError(-1);
    return OK;
  }
