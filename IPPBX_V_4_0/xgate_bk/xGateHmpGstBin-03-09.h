...
//#define LOCAL_IP_ADDR "10.22.3.152"
//#define LOCAL_IP_ADDR "xGateUtil::GetLocalIPAddress()"
...
struct Client {
  ...
  unsigned int m_orgport;
  xGateHmpGstBin *m_hmpBin;
  xGateHmpGstUdp *m_hmpUdp;
  HmpGstBinFactory *m_binFactory;
  Client(ClientDetail &detail) : m_detail(detail), m_hmpBin(NULL), m_hmpUdp(NULL)
  {
  };

  Client(xGateHmpGstUdp *hmpUdp, ClientDetail &detail) :
    m_hmpUdp(hmpUdp), m_detail(detail), m_hmpBin(NULL)
  {
  };

  Client(xGateHmpGstBin *hmpBin, ClientDetail &detail)
    : m_detail(detail), m_hmpBin(hmpBin)
  {
  };
};
...
class xGateHmpGstUdp {
  public:
    ...
    GstElement *m_rudpSink;
    GstElement *m_peerElement;
    GSocket *m_udpSocket;
    ...
    GstPad *m_peerPad;
    gboolean m_isLinked;
    HmpGstCodecType m_codecType; 

    gboolean create_udp_src_sink(HmpGstCodecType codecType, unsigned int serverPort);
    gboolean create_udprbt_sink();
    gboolean create_udp_socket();
    gboolean set_udp_caps(HmpGstCodecType codecType);
    void set_server_addr(string serverIp, unsigned int serverPort);
    void add_client_addr(string clientIp, unsigned int clientPort);
    void add_client_rbt(string clientIp, unsigned int clientPort);
    void set_client_addr(string clientIp, unsigned int clientPort);
    void update_socket(GSocket *udpSocket);
    void remove_client_addr();
};
...
class xGateHmpGstBin {
  public:
    ...
    virtual GstBin * sendBin_create(xGateHmpGstUdp *udpSink);
    virtual GstBin * recvBin_create(xGateHmpGstUdp *udpSrc);
    gboolean ringbkBin_create(xGateHmpGstUdp *udpSink, char* filename);
    virtual void set_stun_target_client(string client_ip, unsigned int client_port, xGateHmpGstUdp *udpSrc);

    void generateConnectId(char *pout, int len);
    void set_client_details(ClientDetail &cliDetail);

    gboolean recvBin_remove(xGateHmpGstUdp *phmpUdp);
    gboolean sendBin_remove(xGateHmpGstUdp *phmpUdp);

    gboolean sendBin_update(xGateHmpGstUdp *phmpUdp);
    gboolean recvBin_update(xGateHmpGstUdp *phmpUdp);
...
class HmpG711Bin : public xGateHmpGstBin
{
  public:
    ...
    void set_stun_target_client(string client_ip, unsigned int client_port, xGateHmpGstUdp *udpSrc);
...
class HmpGstBinFactory {
  public:
    virtual xGateHmpGstBin *bin_create(string& binName, HmpGstBinType binType, xGateHmpGstUdp *hmpUdp, ClientDetail &cliDetail);
...
class xGateHmpGstPipeline {
  public:
    xGateHmpGstPipeline(string pipelineName, HmpGstPipelineType m_pipelineType);
    ~xGateHmpGstPipeline();

    HmpGstPipelineType m_pipelineType;
    string m_sendPipelineName;
    string m_recvPipelineName;
    string m_confPipelineName;
    string m_rbtPipelineName;
    GstElement *m_sendPipeline;
    GstElement *m_recvPipeline;
    GstElement *m_confPipeline;
    GstElement *m_rbtPipeline;

    GstElement *recvBinA;
    GstElement *sendBinA;
    GstElement *recvBinB;
    GstElement *sendBinB;
    GstElement *ringbkBin;
    GstElement *udpsrc;
    GstElement *udpsink;

    xGateHmpGstUdp *hmpUdpA;
    xGateHmpGstUdp *hmpUdpB;
    xGateHmpGstBin *hmpBinA;
    xGateHmpGstBin *hmpBinB;

    GstClock *m_pipelineClock;
    unsigned short m_poolIndex;
    CLIENT_LIST m_clientList;
    unsigned short m_clientCount;
    gboolean m_rbtActive;
    gboolean m_autoRepeat;

    virtual gboolean simple_pipeline_create();
    virtual gboolean conf_pipeline_create();
    virtual gboolean rbt_pipeline_create();
    gboolean play_ringback_tone(Client *sClient, bool autorepeat);
    ...
    gboolean disconnect_simple_bins(Client *sClient);
    gboolean stop_ringback_tone(Client *sClient);
  ...
  private:
    ...
    GstElement *m_audioMixerIn;
    GstElement *m_audioMixerOut;
    GstBus *m_recvBus;
    GstBus *m_sendBus;
    GstBus *m_confBus;
    GstBus *m_rbtBus;
    ...
    string clientA_ip;
    string clientB_ip;
    unsigned int clientA_port;
    unsigned int clientB_port;
    GSocket *udpSockA;
    GSocket *udpSockB;
