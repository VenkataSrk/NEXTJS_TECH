...
struct Client {
  ...
  xGateHmpGstPipeline *m_gstPipeline;
  xGateHmpGstManager *m_gstManager;

  Client(ClientDetail &detail)
	: m_detail(detail), m_hmpBin(NULL), m_binFactory(NULL)
  {
  };
...
class xGateHmpGstBin {
  public:
    ...
    gboolean ringbkBin_create();
    virtual void set_stun_target_client(string client_ip, unsigned int client_port);
...
class HmpG711Bin : public xGateHmpGstBin
{
  public:
    ...
    void set_stun_target_client(string client_ip, unsigned int client_port);
...
class xGateHmpGstPipeline {
  public:
    ...
    char *m_dtmfBuffer;
    double m_dtmfTimeout;


    virtual gboolean simple_pipeline_create();
    virtual gboolean conf_pipeline_create();
    virtual gboolean rbt_pipeline_create();
    GstElement* get_rbt_pipeline()const {return m_rbtPipeline;};
    gboolean play_hmp_wavfile(Client *sClient, char* playWavfile);
    ...
    gboolean stop_hmp_wavfile(Client *sClient);
  ...
  private:
    ...
    gboolean handle_event(GstBus* bus, GstMessage* msg, Client *client);
};
