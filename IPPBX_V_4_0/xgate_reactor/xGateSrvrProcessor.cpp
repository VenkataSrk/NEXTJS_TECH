...
{
  XGLOG_WARN( "xGateSrvrProcessor::xGateSrvrProcessor");
}
...
{
  XGLOG_INFO("Enter xGateSrvrProcessor::process_msg(%s)", m_serverType); 
  const char *data;
  ...
  switch(pMsg->getMsgType()) {
    case EN_XGATE_MSG_MGC_MG_IN: //we got msg from MG server
      {
        ...
        if(decode_media_msg(pMgMsg) != EN_XGATE_STATUS_SUCCESS) {
          XGLOG_ERROR("xGateSrvrProcessor::process_msg data mg_msg decode or validation failed");
        }
    ...
    case EN_XGATE_MSG_MGC_MG_OUT:
      {
        XGLOG_WARN( "Note: MG Dispatcher thread directly handle EN_XGATE_MSG_MGC_MG_OUT");
        break;
      }
    case EN_XGATE_MSG_UNKNOWN:
    default:
      {
        XGLOG_ERROR("Enter xGateSrvrProcessor::process_msg is EN_XGATE_MSG_UNKNOWN");
        break;
      }      
  }
...
{
  XGLOG_WARN( "Enter xGateSrvrProcessor::dispatch_msg(%s)", m_serverType); 
  return EN_XGATE_STATUS_SUCCESS;  
}
...
{
  XGLOG_INFO("Enter xGateSrvrProcessor::decode_media_msg(%s)", m_serverType); 
  const char  *data = 0;
  ...
  if(!data && dataLen <=0) {
    XGLOG_ERROR("xGateSrvrProcessor::decode_media_msg invalid data in xGateMgMsg.");
    return EN_XGATE_STATUS_ERROR;
  }
  XGLOG_ERROR("xGateSrvrProcessor::decode_media_msg data: %s, len: %d", data, dataLen);
  ...
  if(create_doc_object(doc, data, dataMemObj) != EN_XGATE_STATUS_SUCCESS) {
    XGLOG_INFO("xGateSrvrProcessor::decode_media_msg DOC object creation failed");
    return EN_XGATE_STATUS_ERROR;
  }
...
pMgMsg->set_mg_msg_type(mgMsgType);
  switch(mgMsgType) {
    case EN_XGATE_MG_REGISTER:
      {
        XGLOG_INFO("decode_media_msg decoding EN_XGATE_MG_REGISTER");
    ...
    case EN_XGATE_MG_CONFERENCE_FOR_CLIENT:
      {
        XGLOG_INFO("decode_media_msg decoding %d response",mgMsgType);  
    ...
    {
     XGLOG_INFO("decode_media_msg decoding EN_XGATE_MG_18X_RESPONSE response");
     handle_media_resp(dataMemObj, mediaDetail);
     if(post_media_detail(pMgMsg, mediaDetail) != EN_XGATE_STATUS_SUCCESS) {
       return EN_XGATE_STATUS_ERROR;
     }
    ...
    default:
      {
        XGLOG_ERROR("decode_media_msg unable to decode UNKNOWN message");
        return EN_XGATE_STATUS_ERROR;
        break;
      }
  }
...
{
  XGLOG_INFO("Enter xGateSrvrProcessor::create_doc_object(%s)", m_serverType);
  if (doc.Parse(jsonData).HasParseError()) {
    XGLOG_ERROR("xGateSrvrProcessor::create_doc_object json parse error");
    return EN_XGATE_STATUS_ERROR;
  } else if(!doc.IsObject()) {
    XGLOG_ERROR("xGateSrvrProcessor::create_doc_object not able to created document object");
    return EN_XGATE_STATUS_ERROR;
  } else if(!doc.HasMember("msg_type")) {
    XGLOG_ERROR("xGateSrvrProcessor::create_doc_object msg_type field is missing in json msg");
    return EN_XGATE_STATUS_ERROR;
  ...
  dataMemObj = doc["data"];
  XGLOG_INFO("xGateSrvrProcessor::create_doc_object successfully created DOC object !!");
  return EN_XGATE_STATUS_SUCCESS;
}

xGateRetVal xGateSrvrProcessor::handle_media_resp(Value& dataMemObj, MgMediaDetail& serverMediaDetail)
{
  XGLOG_INFO("Enter xGateSrvrProcessor::handle_media_resp(%s)", m_serverType);
    ...
    for (itr; itr != itrEnd; ++itr) {
      if(!strcmp(itr->name.GetString(), "call_id")) {
        ...
        serverMediaDetail.rtp_ep = itr->value.GetString();
...
//           Value& sdp =  itr->value;
  ...
  XGLOG_INFO("xGateSrvrProcessor::handle_media_resp successfully handled");
  return EN_XGATE_STATUS_SUCCESS;
}
...
for (SizeType i = 0; i < sdpObj.Size(); i++){
 ...
 XGLOG_INFO("xGateSrvrProcessor::handle_sdpinfo_resp successfully handled");
 return EN_XGATE_STATUS_SUCCESS;
} 
xGateRetVal xGateSrvrProcessor::handle_gateway_reg(Value& dataMemObj, MgRegistrationDetail& gatewayRegDetail)
{
#if 0
  XGLOG_INFO("Enter xGateSrvrProcessor::handle_gateway_reg(%s)", m_serverType);
  Value::ConstMemberIterator itr = dataMemObj.MemberBegin();
  Value::ConstMemberIterator itrEnd = dataMemObj.MemberEnd();
  ...
  XGLOG_INFO("xGateSrvrProcessor::handle_gateway_reg successfully handled");
#endif
  return EN_XGATE_STATUS_SUCCESS;
}
...
{
  XGLOG_INFO("Enter xGateSrvrProcessor::post_media_detail(%s)", m_serverType);
 ...
 // printf("xGateSrvrProcessor::post_media_detail (posting msg: %d)", pMgMsg->get_mg_msg_type());
  ...
  if(!taskPtr) {
    XGLOG_ERROR("xGateSrvrProcessor::post_media_detail SipProcessor task not found");
    return EN_XGATE_STATUS_ERROR;
  }
  ...
  if (taskPtr->putq(pAmb, &tval)) {
    XGLOG_INFO("xGateSrvrProcessor::post_media_detail SipProcessor enqueue message success");
  } else {
    XGLOG_ERROR("xGateSrvrProcessor::post_media_detail SipProcessor enqueue message failed");
    delete pMgMsgNew;
    return EN_XGATE_STATUS_ERROR;
  }
