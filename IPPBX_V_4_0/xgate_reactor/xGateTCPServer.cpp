  ...
  m_reactor(NULL)
{
 ...
 XGLOG_WARN( "xGateTCPServer::xGateTCPServer(%s)", m_serverType);
}

xGateRetVal xGateTCPServer::init (vector<xGateListenAddress>listenAddressList)
{
 XGLOG_WARN( "xGateTCPServer::init(%s)", m_serverType);
  for( unsigned int i =0 ; i < listenAddressList.size();i++)
  {
   ...
   XGLOG_ERROR( "xGateTCPServer::init(%s) Listening on Address: %s:%d", 
    ...
    if(errno != 0 )
    {
     XGLOG_ERROR( "xGateTCPServer::init(%s) Error opening the socket Errno: %d"\
    ...
    {
     XGLOG_ERROR( "xGateTCPServer::init(%s) Error While registering socket"\
  ...
  {
   XGLOG_ERROR( "xGateTCPServer::init(%s)"\
...
void xGateTCPServer::run (void* threadClass)
{
 ...
 XGLOG_WARN( "xGateTCPServer::run Starting acceptor event processing loop (%s)", pReceiver->m_serverType);
...
{
 XGLOG_WARN( "xGateTCPServer::handle_close(%s)", m_serverType);
  return 0;
}

int xGateTCPServer::handle_connection(ACE_HANDLE aceHandle)
{
 XGLOG_WARN( "xGateTCPServer::handle_connection(%s)", m_serverType);
  return 0;
}

xGateRetVal xGateTCPServer::stop_thread( void )
{
 XGLOG_WARN( "xGateTCPServer::stop_thread(%s) called", m_serverType);
  m_reactor->end_reactor_event_loop();
  while (m_running)
  {
    sleep(1);
  }
 XGLOG_WARN( "xGateTCPServer::stop_thread(%s) Stopped xGateTCPServer", m_serverType);
  return EN_XGATE_STATUS_SUCCESS;
}

int xGateTCPServer::handle_timeout(const ACE_Time_Value&, const void *arg)
{
 XGLOG_WARN( "xGateTCPServer::handle_timeout(%s) called",m_serverType);
  return 0;
}
