  ...
  m_taskPtr(taskPtr)
{
  ...
  XGLOG_WARN( "xGateTCPSocketHandler::xGateTCPSocketHandler(%s)", m_serverType);
} 
...
{ 
  XGLOG_INFO("xGateTCPSocketHandler::init(%s)", m_serverType);
  m_peer = peerStream; 
  ...
  {
    XGLOG_ERROR( "xGateTCPSocketHandler::init(%s) Failed to register" \
...
{
  XGLOG_WARN( "xGateTCPSocketHandler::handle_close(%s)" \
  ...
  if (pReceiver)
  {
    ...
    if (timerId != 0)
    {
      ...
      {
        XGLOG_WARN( "SipProxyTCPSocketHandler::handle_close cancel_timer successful");
        if (pFd)
        {
          delete pFd;
        }
  ...
  XGLOG_WARN( "xGateTCPSocketHandler::handle_close(%s) Removing the Address:%s:%d  Fd:%d" \
...
{
  int retVal = 0;
  XGLOG_WARN( "xGateTCPSocketHandler::handle_input(%s)", m_serverType);
  ...
  if(0 < bufLen)
  {
    ...
    printf("****************************\n");
    m_recvdBuf->pendingMsg.append(reinterpret_cast<char*>(m_socketData), (size_t)bufLen);
  ...
  XGLOG_ERROR( "xGateTCPSocketHandler::handle_input(%s) Successfully handled the Message", m_serverType);
  return retVal;
} 
...
void xGateTCPSocketHandler::post( char *pBuf,\
    int sizeOfMessage)
{
  XGLOG_WARN( "Enter xGateTCPSocketHandler::post");
  ...
  if(data) {
    ...
    {
      XGLOG_WARN( "xGateTCPSocketHandler::post "\
  ...
  XGLOG_WARN( "Exit  xGateTCPSocketHandler::post");
}
