...
bool XGLog::create_directory()
{
  ...
  snprintf(dirName, 85, "URWssLogDir_%s", buffer);
...
bool XGLog::create_file()
{
        ...
        snprintf(file_name,85 , "%s_STT_TTS_MG_LOG-%03d.TXT", buffer,m_fileCount);

        m_filePath=string("/var/log/xgate/log")  + string("/")  + file_name ;
...
__ur_function_log__::__ur_function_log__(const char* ptfunction,const char* filename, int line, const char* ptMsg...)
{
   ...
   try {
	 ...
	 snprintf(fullInfo, MAX_SIZE_PER_MSG, "%s %s:%d Calling function %s! %s %s", TRACE_PREFIX, ptFileName, ptLineNumber, ptFunctionName, info, NEWLINE);
      } else {
