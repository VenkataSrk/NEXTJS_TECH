...
XGLog::XGLog() :
  ...
  m_run(false), m_logStream(NULL), m_strDirName(""), m_filePath(""), m_fileCount(0)
{
}
...
bool XGLog::create_directory()
{
  ...
  snprintf(dirName, 91, "URCBLogDir_%s", buffer);
...
bool XGLog::create_file()
{
        ...
        snprintf(file_name, 25+sizeof(buffer), "%s_URAVCONF_LOG-%03d.TXT", buffer,m_fileCount);
    ...
    unsigned int lineNum, const char* fmt...)
{
  ...
  try { //TODO: Yoga need to handle the exception properly
    ...
    snprintf(fullInfo, MAX_SIZE_PER_MSG + 36, "%s %s:%d %s %s", verbos, fileName, lineNum, info, NEWLINE);
 ...
 *    @version  0.0.1
 *     @date 08/10/2021
 *    @brief default constructor of class __ur_function_log__
 *
 */
 ...
 *    @version  0.0.1
 *     @date 08/10/2021
 *    @brief Entry and Exit log constructor of class __ur_function_log__.
 *          *
 *        Entry and exit log counstror is called in the begining of
 *                the function by using macro XLOG_FUNCTION_EN_EX().
 *        This will print debug level "TRACE", with file name,
 *                line number and function name automatically
 *                You can use the macro in following two ways
 *                XLOG_FUNCTION_EN_EX("");
 *        XLOG_FUNCTION_EN_EX("Provide additional info here");
 */
...
__ur_function_log__::__ur_function_log__(const char* ptfunction,const char* filename, int line, const char* ptMsg...)
{
   ...
   try {
	 ...
	 //snprintf(fullInfo, MAX_SIZE_PER_MSG, "%s %s:%d Calling function %s! %s %s", TRACE_PREFIX, ptFileName, ptLineNumber, ptFunctionName, ptMsg, NEWLINE);
	 snprintf(fullInfo, MAX_SIZE_PER_MSG + 33, "%s %s:%d Calling function %s! %s %s", TRACE_PREFIX, ptFileName, ptLineNumber, ptFunctionName, info, NEWLINE);
      } else {
	 snprintf(fullInfo, MAX_SIZE_PER_MSG, "%s %s:%d Calling function %s! %s", TRACE_PREFIX, ptFileName, ptLineNumber, ptFunctionName, NEWLINE);
      }
   } catch(...) {
 ...
 *    @version  0.0.1
 *     @date 08/10/2021
 *    @brief Entry and Exit log destructor of class __ur_function_log__.
 *          *
 *
 *              Destructor is called automatically exiting from the called function
 *              And it prints information like
 *              This will print debug level "TRACE", with file name,
 *                line number and function name automatically
 */
...
__ur_function_log__::~__ur_function_log__()
{
        ...
        try {
                snprintf(fullInfo, MAX_SIZE_PER_MSG, "%s %s:%d Exiting function %s! %s", TRACE_PREFIX, ptFileName, ptLineNumber, ptFunctionName, NEWLINE);
        } catch(...) {
                snprintf(fullInfo, MAX_SIZE_PER_MSG, "Invalid log format\n");
                logLevel = EN_LOG_LEVEL_ERROR;
        }
