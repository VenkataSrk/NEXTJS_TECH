...
XGLog::XGLog() :
  ...
  m_run(false), m_logStream(nullptr), m_filePath(""),m_fileCount(0)
{
}
...
bool XGLog::create_file()
{
        ...
        snprintf(file_name,20+sizeof(buffer) , "%s_SGW_LOG-%03d.TXT", buffer,m_fileCount);
    ...
    unsigned int lineNum, const char* fmt...)
{
  ...
  char fullInfo[MAX_SIZE_PER_MSG+210] = {0};
  memset(fullInfo,0,MAX_SIZE_PER_MSG+210);
  ...
  try {
    ...
    snprintf(fullInfo, MAX_SIZE_PER_MSG+210, "%s %s:%d %s %s", verbos, fileName, lineNum, info, NEWLINE);
...
__ur_function_log__::__ur_function_log__(const char* ptfunction,const char* filename, int line, const char* ptMsg...)
{
   ...
   try {
	 ...
	 snprintf(fullInfo, MAX_SIZE_PER_MSG+210, "%s %s:%d Calling function %s! %s %s", TRACE_PREFIX, ptFileName,
                     ptLineNumber, ptFunctionName, info, NEWLINE);
      } else {
	 snprintf(fullInfo, MAX_SIZE_PER_MSG+210, "%s %s:%d Calling function %s! %s", TRACE_PREFIX, ptFileName, 
                     ptLineNumber, ptFunctionName, NEWLINE);
      }
   } catch(...) {
      snprintf(fullInfo, MAX_SIZE_PER_MSG+210, "Invalid log format\n");
      logLevel = EN_LOG_LEVEL_ERROR;
   }
