...
NetIOMsgProcessor::NetIOMsgProcessor() :
  ACE_Task<ACE_MT_SYNCH>(),
  m_run(false)
{
}

NetIOMsgProcessor::~NetIOMsgProcessor(void)
{
}

bool NetIOMsgProcessor::stop(void)
{
  m_run = false;
  msg_queue()->deactivate();
  wait();
  XGLOG_INFO("NetIOMsgProcessor stopped !");
  return true;
}

bool NetIOMsgProcessor::init()
{
  m_run = true;
  if(activate(THR_NEW_LWP, 1) == -1) {
    XGLOG_ERROR("NetIOMsgProcessor::init() failed to start VMS controller thread !");
    return false;
  }
  XGLOG_INFO("Initialized NetIOMsgProcessor successfully !");
  return true;
}

int NetIOMsgProcessor::svc(void)
{
  ...
  while(m_run) {
    ...
    if((result != -1)) {
      ...
      XGLOG_ERROR("NetIOMsgProcessor::svc Error in Message Queue");
      m_run = false;
    }
  ...
  XGLOG_INFO("NetIOMsgProcessor::svc() stopped");
  return 0;
}
bool NetIOMsgProcessor::handle_msg(ACE_Message_Block *pAmb)
{
...
#if 0
bool NetIOMsgProcessor::decode_msg(const char* buf, size_t bufLen)
{
  if(!buf && bufLen <=0) {
...
bool NetIOMsgProcessor::create_doc_object(Document &doc, const char *jsonBuf)
{
...
bool NetIOMsgProcessor::read_media_detail(const Value& val, MgMediaDetail& detail)
{
...
bool NetIOMsgProcessor::read_sdpinfo(const Value& val, MgMediaDetail& detail)
{
...
bool NetIOMsgProcessor::read_audio_sdpinfo(const Value& val, MgMediaDetail& detail)
{
...
bool NetIOMsgProcessor::read_video_sdpinfo(const Value& val, MgMediaDetail& detail)
{
  XGLOG_WARN("read_video_sdpinfo not yet been implemented !");
  return false;
}

bool NetIOMsgProcessor::post(MgMediaDetail& mediaDetail)
{
