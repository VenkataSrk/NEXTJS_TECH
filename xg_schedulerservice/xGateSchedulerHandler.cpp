...
xGateSchedulerHandler* xGateSchedulerHandler::m_instance = nullptr;
...
int xGateSchedulerHandler::handle_timeout(const ACE_Time_Value&, const void *arg)
{
  ...
  switch(data->m_timerType) {
    case EN_SCHEDULER_QUEUE_HOLD:
      {
        XGLOG_DEBUG("EN_SCHEDULER_QUEUE_HOLD timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_QUEUE_HOLD))
        {
    ...
    case EN_SCHEDULER_AGENT_WAIT:
      {
        XGLOG_DEBUG("EN_SCHEDULER_AGENT_WAIT timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_AGENT_WAIT))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_AGENT_WARMUP:
      {
        XGLOG_DEBUG("EN_SCHEDULER_AGENT_WARMUP timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_AGENT_WARMUP))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_EXTN_WAIT:
      {
        XGLOG_DEBUG("EN_SCHEDULER_EXTN_WAIT timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_EXTN_WAIT))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_REFRESH_HUNTGROUP_INFO:
      {
        XGLOG_DEBUG("EN_SCHEDULER_REFRESH_HUNTGROUP_INFO timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_REFRESH_HUNTGROUP_INFO))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_PARK_TIMER:
      {
        XGLOG_DEBUG("EN_SCHEDULER_PARK_TIMER timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_PARK_TIMER))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_UNPARK_TIMER:
      {
        XGLOG_DEBUG("EN_SCHEDULER_UNPARK_TIMER timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_UNPARK_TIMER))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_URCONF_USER_DISCONNECT_NOTIFY:
     ...
     case EN_SCHEDULER_LWSS_DB_CONFIG_QUERY_TIMER:
      {
        XGLOG_DEBUG("EN_SCHEDULER_LWSS_DB_CONFIG_QUERY_TIMER timeout occurred for uid: %s", uid.c_str());
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_LWSS_DB_CONFIG_QUERY_TIMER))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
     ...
     case EN_SCHEDULER_INVITE_TIMER:
       {
        XGLOG_DEBUG("EN_SCHEDULER_INVITE_TIMER timeout occurred for uid: %s", uid.c_str());
	      if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_INVITE_TIMER))
	      {
	       XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
	      } else {
	        XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
    ...
    case EN_SCHEDULER_WHITE_LIST_REFRESH:
      {
        XGLOG_DEBUG("EN_SCHEDULER_WHITE_LIST_REFRESH timeout occurred for uid: %s", uid.c_str());
	      if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_WHITE_LIST_REFRESH))
	      {
	       XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
	      } else {
	        XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
      ...
      {
        XGLOG_DEBUG("EN_SCHEDULER_OPTIONS_TIMER timeout occurred for uid: %s", uid.c_str());
        if (!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_OPTIONS_TIMER))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
