...
bool xGateSchedulerHandler::doSchedulerRequest(SchedulerInfo &schedulerInfo)
{
  ...
  switch(timerFunc)
  {
    case SCHEDULER_FUNCTION_SET_TIMER:
    {
      ...
      if(!m_tSchedulerContext.insertSchedulerContext(uid, schedulerType, timerId, timeoutSecond))
      {
    ...
    case SCHEDULER_FUNCTION_CANCEL_TIMER:
    {
      ...
      if(reactor->cancel_timer(timerId)) 
      {
        XGLOG_DEBUG("xGateSchedulerHandler::cancel_timer success for uid: %s and timer_id: %ld", \
            uid.c_str(), timerId);
				long int timeLeft(0);
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, schedulerType, timeLeft))
        {
          ...
          XGLOG_INFO("SCHEDULER_FUNCTION_CANCEL_TIMER deleteSchedulerContext success - %s", uid.c_str());
					if(schedulerType == EN_SCHEDULER_URCONF_MEETING_DURATION)
					{
						xGateSchedulerServiceMsg *pMsg = new xGateSchedulerServiceMsg();
						SchedulerInfo& scheduler_info = pMsg->get_scheduler_info();
						scheduler_info.m_timeoutSecond = timeLeft;
						scheduler_info.m_schedulerType = schedulerType;
						pMsg->setDstModuleId(IURDefines::UR_MODULE_SERVICE_CONTROLLER);
						pMsg->setUid(uid);
						pMsg->setMsgType(EN_XGATE_MSG_SCHEDULER_UPDATE);
						XGUTIL->post(pMsg);
					}
        } 
      } else {
    ...
    case EN_SCHEDULER_URCONF_MEETING_DURATION:
      {
        ...
        if(!m_tSchedulerContext.deleteSchedulerContext(uid, EN_SCHEDULER_URCONF_MEETING_DURATION))
        {
          XGLOG_ERROR("handle_timeout deleteSchedulerContext failed - %s", uid.c_str());
        } else {
          XGLOG_INFO("handle_timeout deleteSchedulerContext success - %s", uid.c_str());
