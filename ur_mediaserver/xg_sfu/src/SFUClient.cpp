...
bool SFUClient::set_conn_id(unsigned int connection_id)
{
	m_connection_id = connection_id;
	return true;
}

unsigned int SFUClient::get_conn_id()
{
...
{
	XGLOG_INFO("SFUClient::send_msg_to_mbc with conn_id:%d!!!!",m_connection_id);
	xGateSFUServiceMsg *pmsg = new xGateSFUServiceMsg();
	pmsg->setDstModuleId(IURDefines::UR_MODULE_MEDIA_PROXY_SERVICE);
	pmsg->setSrcModuleId(IURDefines::UR_MODULE_SFU_SERVICE);
	pmsg->set_connection_id(m_connection_id);
...
xGateRetVal SFUClient::decode_media_msg(const char *data,int dataLen)
{
	...
	switch(mgMsgType) {
		...
		case EN_XGATE_MG_VIDEO_REC_MSG:
			{
				XGLOG_INFO( "decode_mg_msg decoding EN_XGATE_MG_VIDEO_REC_MSG");
				handle_media_req(mgMsgType,dataMemObj, clientMediaDetail);
				break;
			}
	...
	if (mgMsgType == EN_XGATE_MG_SDP_UPDATE_REQUEST){
...
{
	XGLOG_TRACE( "Enter SFUClient::create_doc_object");
	doc.Parse(jsonData); 
	...
	dataMemObj = doc["data"];
	XGLOG_TRACE( "SFUClient::create_doc_object successfully created DOC object !!");
	return EN_XGATE_STATUS_SUCCESS;
}
...
{
	XGLOG_TRACE( "Enter SFUClient::handle_media_req mgMsgType: %d", mgMsgType);
	...
	for (itr; itr != itrEnd; ++itr) {
		...
		} else if(!strcmp(itr->name.GetString(), "record_id")) {
			clientMediaDetail.record_id = itr->value.GetString();
		...
		} else if(!strcmp(itr->name.GetString(), "customer_num")) {
			clientMediaDetail.customer_num = itr->value.GetString();
		} else if(!strcmp(itr->name.GetString(), "domain_id")) {
			clientMediaDetail.domain_id = itr->value.GetInt();
		} else if(!strcmp(itr->name.GetString(), "agent_id")) {
			clientMediaDetail.agent_id = itr->value.GetInt();
...
xGateRetVal SFUClient::handle_sdpinfo_process(const Value& sdpObj, MgMediaDetail& clientMediaDetail) {
	...
	XGLOG_TRACE( "SFUClient::handle_sdpinfo_process successfully handled");
	return EN_XGATE_STATUS_SUCCESS;
}
		...
		if(ParticipantListObj[i].IsObject()){
			...
			for (Value::ConstMemberIterator itr = ParticipantListObj[i].MemberBegin();
				...
				if(!strcmp(itr->name.GetString(), "call_id")) {
					...
					XGLOG_INFO("Extracted Participant: %p callId: %s",pPInfo,
...
xGateRetVal SFUClient::handle_urmeet_userlist(const Value& UrmeetistObj, MgMediaDetail& clientMediaDetail)
{
	...
	for (SizeType i = 0; i < UrmeetistObj.Size(); i++){
		if(UrmeetistObj[i].IsObject()){
			...
			clientMediaDetail.meetuser_info.emplace(clientMediaDetail.meetuser_info.begin() + i,
					URUInfo);
		}
	}
	XGLOG_INFO( "SFUClient::handle_urmeet_userlist successfully decoded");
	return EN_XGATE_STATUS_SUCCESS;
}
...
{
	XGLOG_INFO("SFUController:: send_msg!!!!!!");
...
int SFUClient::create_media_msg(xGateMgMsg *pMgMsg, string &jsonBuff, int &gatewayId)
{
	...
	doc["data"]["record_id"] = setJsonParam(mediaDetail.record_id,doc);
	doc["data"]["meeting_name"] = setJsonParam(mediaDetail.meeting_name,doc);
	doc["data"]["sip_id"] = setJsonParam(mediaDetail.sip_id,doc);
	doc["data"]["is_ccaas"] = setJsonParam(mediaDetail.is_ccaas,doc);
	doc["data"]["customer_num"] = setJsonParam(mediaDetail.customer_num,doc);
	doc["data"]["domain_id"] = setJsonParam(mediaDetail.domain_id,doc);
	doc["data"]["agent_id"] = setJsonParam(mediaDetail.agent_id,doc);
	...
	XGLOG_TRACE( "SFUClient::create_media_msg created media msg  with length of (%d),", jsonBuff.length());
