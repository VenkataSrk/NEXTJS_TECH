...
bool xGateRedisService::initRedisHandler()
{
   ...
   socketItr = XG_CONFIG::instance()->m_redisModProfile.m_mapSubRedisConnInfo.begin();
   for(socketItr; socketItr != XG_CONFIG::instance()->m_redisModProfile.m_mapSubRedisConnInfo.end(); socketItr++)
   {
   ...
   socketItr = XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.begin();
   for(socketItr; socketItr !=XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.end(); socketItr++)
   {
      xGateRedisHandler *redisHandler = nullptr;
      RedisConnInfo *& redisConnInfo = socketItr->second;
      if(redisConnInfo)
   ...
   auto getItr = XG_CONFIG::instance()->m_redisModProfile.m_mapGetRedisConnInfo.begin();
   for(getItr; getItr != XG_CONFIG::instance()->m_redisModProfile.m_mapGetRedisConnInfo.end(); getItr++)
   {
      xGateRedisHandler *redisHandler = nullptr;
   ...
   switch(pMsg->getMsgType())
   {
	 ...
	 switch(redisEvent)
	 {
	    case EN_REDIS_EVENT_PUBLISH_TO_UCAAS_SIO:
		  ...
		  itr = XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.find(EN_REDIS_MSG_TYPE::EN_REDIS_MSG_TYPE_UCAAS_SIO);
		  break;
	       }
	    case EN_REDIS_EVENT_PUBLISH_TO_CCAAS_SIO:
	       {
		  XGLOG_INFO("handle_msg CONFBRIDGE_TO_CCAAS_SIO");
		  itr = XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.find(EN_REDIS_MSG_TYPE::EN_REDIS_MSG_TYPE_CCAAS_SIO);
		  break;
	       }
	    case EN_REDIS_EVENT_PUBLISH_TO_CONFBRIDGE:
	       {
		  XGLOG_INFO("handle_msg PUBLISH_TO_CONFBRIDGE");
		  itr = XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.find(EN_REDIS_MSG_TYPE::EN_REDIS_MSG_TYPE_CONFBRIDGE);
		  break;
	       }
	    case EN_REDIS_EVENT_PUBLISH_TO_ACD:
	       {
		  XGLOG_INFO("handle_msg CONFBRIDGE_TO_ACD");
		  itr = XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.find(EN_REDIS_MSG_TYPE::EN_REDIS_MSG_TYPE_ACD);
		  break;
	       }
	    default:
	       {
	 ...
	 if(itr != XG_CONFIG::instance()->m_redisModProfile.m_mapPubRedisConnInfo.end())
	 {
	 ...
	 switch(redisEvent)
	 {
	    case EN_REDIS_EVENT_GET_ACD_CHANNEL:
		  ...
		  itr = XG_CONFIG::instance()->m_redisModProfile.m_mapGetRedisConnInfo.find(EN_REDIS_GET_TYPE::EN_REDIS_GET_TYPE_ACD_CHANNEL);
		  break;
	       }
	    default:
	       {
	 ...
	 if(itr != XG_CONFIG::instance()->m_redisModProfile.m_mapGetRedisConnInfo.end())
	 {
	    RedisConnInfo * redisConnInfo = itr->second;
	    std::string strRedisIp = redisConnInfo->m_strServerHost.c_str();
	    if(!strRedisIp.empty())
