...
{
  XGLOG_TRACE("process_msg MediaMsgDispatcher for uid: %s !", m_uid.c_str());
...
bool MediaMsgDispatcher::fill_media_detail(Value& val, MgMediaDetail& detail)
{
  ...
  val["retrieved_audiossrc"] = setJsonParam(detail.retrieved_audiossrc);
  val["retrieved_videossrc"] = setJsonParam(detail.retrieved_videossrc);
  ...
  val["browser_type"] = setJsonParam(detail.browser_type, m_doc);
...
bool MediaMsgDispatcher::send_msg_on_tcp(xGateMediaServiceMsg *pMsg)
{
  ...
  if(bytes_sent == len) {
    //XGLOG_DEBUG("successfully sent FROM MG %d bytes on fd: %hu data: (%s) on ", len, m_fd, data);
