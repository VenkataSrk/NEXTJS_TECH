...
#define MEDIATCPCONNECTOR TcpConnector::instance()

  class MediaTcpConnector : public xGateConnector
  {			    
  
public:	  
    MediaTcpConnector();   

    virtual ~MediaTcpConnector (void);
  ...
  private:
     ...
     **/
    MediaTcpConnector(const MediaTcpConnector& rhs);

    /**
     * Private assignment operator - disallow assignment and automatic methods.
     **/
    MediaTcpConnector& operator= (const MediaTcpConnector& rhs);

    /// Interface to make the connections
    xGateRetVal make_connection();
    xGateRetVal send_client_connected(unsigned int,int,string,int,xGateMSMsgType ClientType);
...
typedef ACE_Unmanaged_Singleton<MediaTcpConnector, ACE_Null_Mutex> TcpConnector;
#endif
