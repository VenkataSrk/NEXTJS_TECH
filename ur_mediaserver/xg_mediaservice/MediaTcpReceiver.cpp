...
MediaTcpReceiver::MediaTcpReceiver ():
  ACE_Event_Handler(),
  m_reactor(NULL),
  m_running(false)
{
...
xGateRetVal MediaTcpReceiver::init (void)
{
  XGLOG_DEBUG("Initializing MediaTcpReceiver");
  if (ACE_Thread_Manager::instance()->spawn((ACE_THR_FUNC) run,
        (void*) this,
        THR_NEW_LWP|THR_DETACHED) == -1)
  {
    XGLOG_ERROR("Initializing MediaTcpReceiver failed."\
...
void MediaTcpReceiver::run (void* threadClass)
{
  XGLOG_DEBUG("Starting acceptor event processing loop");

  MediaTcpReceiver* pReceiver = static_cast<MediaTcpReceiver*>(threadClass);
...
MediaTcpReceiver::~MediaTcpReceiver (void)
{
  if(m_reactor)
    delete m_reactor;
  m_running = false;
...
int MediaTcpReceiver::handle_timeout(const ACE_Time_Value&, const void *arg)
{
...
xGateRetVal MediaTcpReceiver::stop_thread(void)
{
  ...
  XGLOG_DEBUG("stopped MediaTcpReceiver thread !");
  return EN_XGATE_STATUS_SUCCESS;
}

void MediaTcpReceiver::addFdToSocketHandlerMap(int fd, MediaTcpSocketHandler *pHandler)
{
   XGLOG_DEBUG("Added TcpSocketHandler in TcpSocketHandler map for fd: %d", fd);
   m_fdToSocketHandlerMap[fd] = pHandler;
}

xGateRetVal MediaTcpReceiver::getHandlerFromFdToSocketHandlerMap(int fd, MediaTcpSocketHandler *&pHandler)
{
...
void MediaTcpReceiver::removeFromFdToSocketHandlerMap(int fd)
{
...
ACE_Reactor * MediaTcpReceiver::get_reactor(void)
{
  return m_reactor;
}
