...
MediaServiceUtil::MediaServiceUtil():m_connectionId(0)
{
}
...
bool MediaServiceUtil::getConnectionId(unsigned int &nextConnId)
{
...
bool MediaServiceUtil::addToConnTupleInfo(unsigned int connId,TcpConTuple tcpConn)
{
...
bool MediaServiceUtil::removeConnTupleInfo(unsigned int connId)
{
...
bool MediaServiceUtil::getConnTupleInfo(unsigned int connId,TcpConTuple &connTuple)
{
  ACE_GUARD_RETURN(ACE_Thread_Mutex,guard,m_ConnTupleLock,EN_XGATE_STATUS_ERROR);
  map<unsigned int,TcpConTuple>::iterator it;
  if((it = m_ConnMap.find(connId)) != m_ConnMap.end()){
...
bool MediaServiceUtil::removeConnectionId(unsigned int &connId)
{	
