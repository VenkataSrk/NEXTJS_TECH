...
#define SFU_TIMEOUT_DURATION 108000 //30 hours

static map<string, long int> sfuTimerMap;
class xGateMBCUtil {
  public:

    static void insert_mgresourceID_sfu_Timer_map(const char* resId);
    static void removeTimerIdFromSfuTimerMap(string resId);
    ...
    static int getRecConnIdFromMbcConnectionMap();
    static void removeFromMbcConnectionMap(int conn_id);
    ...
    static void insert_sfuId_recConnId_map(int sfuId,int recConnId);
    static int get_recConnId_from_sfuId(int sfuId);
    static void removeSfuIdFromRecConnIdMap(int sfu_id);
    ...
    static void insert_sfuConnId_recConnId_map(int sfuConnId,int pbxConnId);
    static void get_recConnId_from_sfuConnId(int sfuConnId,int &recConnId);
    static void removeSfuConnIdFromRecConnIdMap(int sfu_conn_id);
...
class SfuTimerCallback : public ACE_Event_Handler
{
  public:
    SfuTimerCallback();
    ~SfuTimerCallback();
    SfuTimerCallback(string resId);
    static SfuTimerCallback* getInstance();
    ACE_Reactor* getReactor()
    {
      return m_reactor;
    }
    void set_resId(string resID){
      resId = resID;
    }

    bool init(void);
    static void start_reactor_handle_event();
    virtual int handle_timeout(const ACE_Time_Value&, const void *arg);

  private:
    string resId;
    bool m_running;
    static  SfuTimerCallback *m_instance;
  protected:
