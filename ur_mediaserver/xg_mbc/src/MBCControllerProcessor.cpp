    ...
    xGateMgMsgType mgMsgType,MgMediaDetail& clientMediaDetail)
{
	  ...
	  || (mgMsgType == EN_XGATE_MG_MEDIA_CREATE))
  {
    ...
    if (sfuConnId < 0){
      ...
      xGateMBCUtil::insert_mgresourceID_sfu_Timer_map(clientMediaDetail.mgresource_id.c_str());
  ...
  if(mgMsgType == EN_XGATE_MG_RELEASE){
    ...
    xGateMBCUtil::removeTimerIdFromSfuTimerMap(clientMediaDetail.mgresource_id);
  }
  return EN_XGATE_STATUS_SUCCESS;
}
