...
map<int, int>xGateMBCUtil:: m_sfuIdRecConnIdMap;
map<int, int>xGateMBCUtil:: m_sfuIdSfuConnIdMap;
map<int, int>xGateMBCUtil:: m_sfuConnIdPbxConnIdMap;
map<int, int>xGateMBCUtil:: m_sfuConnIdRecConnIdMap;
...
ACE_Thread_Mutex xGateMBCUtil::m_sfuIdRecConnIdMapLock;
ACE_Thread_Mutex xGateMBCUtil::m_sfuIdSfuConnIdMapLock;
ACE_Thread_Mutex xGateMBCUtil::m_sfuConnIdPbxConnIdMapLock;
ACE_Thread_Mutex xGateMBCUtil::m_sfuConnIdRecConnIdMapLock;
...
SfuTimerCallback* SfuTimerCallback::m_instance = NULL;
...
void xGateMBCUtil::removeFromMbcConnectionMap(int conn_id)
{
  XGLOG_INFO( "xGateMBCUtil::removeFromMbcConnectionMapfor conn_id:%d",conn_id);
  ACE_GUARD(ACE_Thread_Mutex,guard, m_mbcConnectionMapLock);
  map<int,MBCTcpConnInfo>::iterator it = m_mbcConnectionMap.find(conn_id);
  if(it != m_mbcConnectionMap.end()) {
    m_mbcConnectionMap.erase(conn_id);
    XGLOG_INFO("removeFromMbcConnectionMap Success for conn_id:%d",conn_id);
    return;
  }
  XGLOG_WARN( "removeFromMbcConnectionMap Failed for conn_id:%d Entry Not Found",conn_id);
}
...
void xGateMBCUtil::insert_sfuId_pbxConnId_map(int sfu_id, int pbx_conn_id)
{
  ...
  XGLOG_WARN( "xGateMBCUtil::insert_sfuId_pbxConnId_map success");
}
...
int xGateMBCUtil::getRecConnIdFromMbcConnectionMap()
{
  XGLOG_INFO( "xGateMBCUtil::getRecConnIdFromMbcConnectionMap");
  MBCTcpConnInfo mbcTcpConnInfo;
  ACE_GUARD(ACE_Thread_Mutex,guard, m_mbcConnectionMapLock);
  map<int,MBCTcpConnInfo>::iterator it = m_mbcConnectionMap.begin();
  for(it;it != m_mbcConnectionMap.end();it++){
    mbcTcpConnInfo = it->second;
    if(mbcTcpConnInfo.m_mbcClientType == EN_XG_MBC_REC_CLIENT){
      XGLOG_INFO("xGateMBCUtil::getRecConnIdFromMbcConnectionMap Found recConn_id:%d",it->first);
      return it->first;
    }
  }
  XGLOG_WARN( "xGateMBCUtil::getRecConnIdFromMbcConnectionMap Failed Entry Not Found");
  return -1;
}

//To store rec_conn_id for sfu_id
void xGateMBCUtil::insert_sfuId_recConnId_map(int sfu_id, int rec_conn_id)
{
  XGLOG_WARN( "xGateMBCUtil::insert_sfuId_recConnId_map");
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuIdRecConnIdMapLock);
  map<int,int>::iterator it = m_sfuIdRecConnIdMap.begin();
  m_sfuIdRecConnIdMap.insert(it, pair<int,int>(sfu_id,rec_conn_id));
  XGLOG_WARN( "xGateMBCUtil::insert_sfuId_recConnId_map success");
}

int xGateMBCUtil::get_recConnId_from_sfuId(int sfuId)
{
  XGLOG_WARN( "xGateMBCUtil::get_recConnId_from_sfuId");
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuIdRecConnIdMapLock);
  map<int,int>::iterator it = m_sfuIdRecConnIdMap.find(sfuId);
  if(it != m_sfuIdRecConnIdMap.end()) {
    XGLOG_INFO("xGateMBCUtil::get_recConnId_from_sfuId Found entry for sfu_id:%d",sfuId);
    return it->second;
  }
  XGLOG_INFO("xGateMBCUtil::get_recConnId_from_sfuId Entry Not Found for sfu_id:%d",sfuId);
  return -1;
}

void xGateMBCUtil::removeSfuIdFromRecConnIdMap(int sfu_id)
{
  XGLOG_WARN( "xGateMBCUtil::removeSfuIdFromRecConnIdMap sfuId=%d",sfu_id);
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuIdRecConnIdMapLock);
  m_sfuIdRecConnIdMap.erase(sfu_id);
}
...
{
  XGLOG_WARN( "xGateMBCUtil::get_sfuId_from_sfuConnId");
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuIdSfuConnIdMapLock);
  map<int,int>::iterator it = m_sfuIdSfuConnIdMap.begin();
...
void xGateMBCUtil::get_pbxConnId_from_sfuConnId(int sfu_conn_id, int &pbx_conn_id)
{
  ...
  if(it != m_sfuConnIdPbxConnIdMap.end()) {
    ...
    XGLOG_INFO("xGateMBCUtil::get_pbxConnId_from_sfuConnId:%d Found entry for sfu_id:%d",pbx_conn_id,sfu_conn_id);
  }
}

void xGateMBCUtil::removeSfuConnIdFromPbxConnIdMap(int sfu_conn_id)
{
  XGLOG_WARN( "xGateMBCUtil::removeSfuConnIdFromPbxConnIdMap sfu_conn_id=%d",sfu_conn_id);
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuConnIdPbxConnIdMapLock);
  m_sfuConnIdPbxConnIdMap.erase(sfu_conn_id);
}

//To store recConnId for sfuConnId
void xGateMBCUtil::insert_sfuConnId_recConnId_map(int sfu_conn_id, int rec_conn_id)
{
  XGLOG_WARN( "xGateMBCUtil::insert_sfuConnId_recConnId_map");
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuConnIdRecConnIdMapLock);
  map<int,int>::iterator it = m_sfuConnIdRecConnIdMap.begin();
  m_sfuConnIdRecConnIdMap.insert(it, pair<int,int>(sfu_conn_id,rec_conn_id));
  XGLOG_WARN( "xGateMBCUtil::insert_sfuConnId_recConnId_map success");
}

void xGateMBCUtil::get_recConnId_from_sfuConnId(int sfu_conn_id, int &rec_conn_id)
{
  XGLOG_WARN( "xGateMBCUtil::get_recConnId_from_sfuConnId");
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuConnIdRecConnIdMapLock);
  map<int,int>::iterator it = m_sfuConnIdRecConnIdMap.find(sfu_conn_id);
  if(it != m_sfuConnIdRecConnIdMap.end()) {
    rec_conn_id = it->second;
    XGLOG_INFO("xGateMBCUtil::get_recConnId_from_sfuConnId rec_cono_id:%d Found entry for sfu_conn_id:%d",rec_conn_id,sfu_conn_id);
  }
}

void xGateMBCUtil::removeSfuConnIdFromRecConnIdMap(int sfu_conn_id)
{
  XGLOG_WARN( "xGateMBCUtil::removeSfuIdFromRecConnIdMap sfu_conn_id=%d",sfu_conn_id);
  ACE_GUARD(ACE_Thread_Mutex,guard, m_sfuConnIdRecConnIdMapLock);
  m_sfuConnIdRecConnIdMap.erase(sfu_conn_id);
}
...
SfuTimerCallback::SfuTimerCallback():
  m_running(false),
  m_reactor(NULL)
{
...
bool SfuTimerCallback::init(void)
{
  // Spawn the thread
  if(ACE_Thread_Manager::instance()->spawn((ACE_THR_FUNC) start_reactor_handle_event,
        (void*) this,
        THR_NEW_LWP|THR_DETACHED) == -1)
  {
    XGLOG_ERROR("SfuTimerCallback init failed while spawning ace thread !");
    return false;
  }
  return true;
}

SfuTimerCallback* SfuTimerCallback::getInstance(void)
{
  if(m_instance == NULL) {
    m_instance = new SfuTimerCallback();
    if(!m_instance) {
      XGLOG_ERROR("SfuTimerCallback initialization failed !");
      m_instance = NULL;
    } else {
      XGLOG_INFO("SfuTimerCallback initialized successfully !");
    }
  }
  return m_instance;
}

void SfuTimerCallback::start_reactor_handle_event()
{
  XGLOG_DEBUG("starting reactor event loop");
  int rc;
  rc = pthread_setname_np(ACE_Thread::self(), "reactor_handle");
  if(rc!=0)
    XGLOG_INFO("Failed to set name for reactor handle event thread");
  SfuTimerCallback* pReceiver = SfuTimerCallback::getInstance();
...
void xGateMBCUtil::insert_mgresourceID_sfu_Timer_map(const char* resId)
{
  map<string, long int>::iterator itr;
  itr = sfuTimerMap.find(resId);
  if(itr != sfuTimerMap.end()){
    XGLOG_INFO("Found already created TimerId with the same MG resourceID.");
    return;
  }
  SfuTimerCallback *timer = SfuTimerCallback::getInstance();
  SfuTimerCallback *timer_callback = new SfuTimerCallback();
  timer_callback->set_resId(resId);
  ACE_Time_Value delay(SFU_TIMEOUT_DURATION, 0);
  int timerID = timer->getReactor()->schedule_timer(timer, (void*)timer_callback, delay);
  if(timerID > -1){
    sfuTimerMap[resId] = timerID;
    XGLOG_INFO("Successfully inserted timer and started timer for mg resource ID : %s and timerID : %d", resId, timerID);
  }
  else{
    XGLOG_INFO("Failed to insert timer and start timer for mg resource ID : %s and timerID : %d", resId, timerID);
  }
}

void xGateMBCUtil::removeTimerIdFromSfuTimerMap(string resId)
{
  SfuTimerCallback *timer = SfuTimerCallback::getInstance();
  map<string, long int>::iterator itr;
  itr = sfuTimerMap.find(resId);
  if(itr != sfuTimerMap.end() && itr->first.compare(resId) == 0){
    int res = timer->getReactor()->cancel_timer(itr->second);
    if(res == 1)
      XGLOG_INFO("Successfully cancelled the timer with resID : %s with timerID : %d", itr->first.c_str(), itr->second);
    else
      XGLOG_ERROR("Failed to cancelled the timer with resID : %s with timerID :%d", itr->first.c_str(), itr->second);
  }
}

int SfuTimerCallback::handle_timeout(const ACE_Time_Value& tv, const void* arg) {
  string jsonBuff("");
  SfuTimerCallback *timer_callback = (SfuTimerCallback*)arg;
  string resId = timer_callback->resId;
  map<string, long int>::iterator itr = sfuTimerMap.find(resId);
  if(itr != sfuTimerMap.end()){
    XGLOG_INFO("Successfully removed timer from SfuTimerMap and SFU Timeout for resId: %s with timerID : %d", itr->first.c_str(),itr->second);
    sfuTimerMap.erase(itr->first);
  }
  else{
      XGLOG_ERROR("Failed to remove the timer with resId: %s", resId.c_str());
      return 0;
  }
  MgMediaDetail sMediaDetail;
  xGateMgMsgType mgMsgType = EN_XGATE_MG_RELEASE;
  sMediaDetail.mgresource_id = resId;
  sMediaDetail.call_type = EN_XGATE_CALL_TYPE_CONFERENCE_VIDEO;
  sMediaDetail.mgmsg_type = EN_XGATE_MG_RELEASE;

  MBCControllerProcessor *pmbcCtrlProcessor = new MBCControllerProcessor("MBCControllerProcessor");
  int len = xGateMBCUtil::create_media_msg(sMediaDetail, jsonBuff,  mgMsgType);
  jsonBuff = jsonBuff.substr(0,len-4);
  jsonBuff.append("\r");
  len-=3;
  char *data = new char[len+1];
  memset(data,0,len+1);
  memcpy(data,(char*)jsonBuff.c_str(),len);
  xGateMBCServiceMsg* ptmediaBCastMsg = new xGateMBCServiceMsg();
  ptmediaBCastMsg->setMsgType(EN_XGATE_MSG_XG_MBC_MODULE);
  ptmediaBCastMsg->set_mbc_event_type(EN_XG_MBC_EVENT_SEND);
  ...
  ptmediaBCastMsg->setMsgLen(len);

  pmbcCtrlProcessor->send_media_msg_to_sfu(ptmediaBCastMsg, sMediaDetail, mgMsgType);
  return 0;
}
