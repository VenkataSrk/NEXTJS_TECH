...
bool XGLog::create_directory()
{
  ...
  snprintf(dirName, 91, "MBC_SFU_%s", buffer);
...
{
  unsigned long eLogMask = LM_EMERGENCY;

  /* load log profile stored in XGConfig */
  printf ("loglevel read from config service %d \n", CONFIGSERVICE->m_logProfile.m_fileLogLevel);

  m_logProfile.m_fileLogLevel = CONFIGSERVICE->m_logProfile.m_fileLogLevel;

  if(m_logProfile.m_fileLogLevel == EN_LOG_LEVEL_OFF
           || m_logProfile.m_fileLogLevel == EN_LOG_LEVEL_FATAL)
    eLogMask = LM_EMERGENCY;

  else if (EN_LOG_LEVEL_CRITICAL == m_logProfile.m_fileLogLevel)
    eLogMask = (LM_CRITICAL | LM_EMERGENCY);

  else if (EN_LOG_LEVEL_ERROR == m_logProfile.m_fileLogLevel)
    eLogMask = (LM_CRITICAL | LM_EMERGENCY | LM_ERROR);

  else if (EN_LOG_LEVEL_WARN == m_logProfile.m_fileLogLevel)
    eLogMask = (LM_CRITICAL | LM_EMERGENCY | LM_ERROR
                      | LM_WARNING);

  else if (EN_LOG_LEVEL_INFO == m_logProfile.m_fileLogLevel)
    eLogMask = (LM_CRITICAL | LM_EMERGENCY | LM_ERROR
                      | LM_WARNING | LM_INFO);

  else if (EN_LOG_LEVEL_DEBUG == m_logProfile.m_fileLogLevel)
    eLogMask = (LM_CRITICAL | LM_EMERGENCY | LM_ERROR
                      | LM_WARNING | LM_INFO | LM_DEBUG);

  else if (EN_LOG_LEVEL_TRACE == m_logProfile.m_fileLogLevel)
    eLogMask = (LM_CRITICAL | LM_EMERGENCY | LM_ERROR
                      | LM_WARNING | LM_INFO | LM_DEBUG | LM_TRACE);

  ACE_LOG_MSG->priority_mask(eLogMask, ACE_Log_Msg::PROCESS);
  ...
  snprintf(file_name,120 , "%s_%s_LOG-%03d.TXT", buffer,m_proc_name.c_str(),m_fileCount);
    ...
    unsigned int lineNum, const char* fmt...)
{
  ...
  /*return without logging if loglevel sent is less than the configured one */
  if (logLevel < m_logProfile.m_fileLogLevel)
    return true;
