    ...
    default:
      {
        ...
        {
          XGLOG_INFO("XGController handling 'IURDefines::UR_MODULE_HTTP_SERVICE' event from ivrservice");
...
bool XGController::handle_HttpResponse(xGateBaseMsg *pMsg)
{
  XGLOG_DEBUG("XGController::Inside handle_HttpResponse");
  ...
  if(XGController::XGController_STATUS_REQ_SRV_INFO == m_eCtrlStatus) {
    if(m_tXGControllerHandler.Handle_response_srvr_info_msg(ptHttpMsg, m_tSelfServerInfo)) {
      m_eCtrlStatus = XGController_STATUS_MODULES_INIT;
      initModule();
    } else {
    ...
    if (ptCtx->getStatus() &  VMSCallContext::VMSCALL_STATUS_CALL_DISCONNECTED) {
      ...
      if (!m_tXGControllerHandler.Handle_response_user_info_msg(ptHttpMsg, ptCtx->m_tUserInfo, strID)) {
      ...
      if (!m_tXGControllerHandler.Handle_response_Validate_Extn(ptHttpMsg, ptCtx->m_tUserPinInfo, strID)) {
      ...
      if (!m_tXGControllerHandler.Handle_response_Validate_Pin(ptHttpMsg, ptCtx->m_tUserPinInfo, strID)) {
