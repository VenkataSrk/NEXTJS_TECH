...
import { invalidCredentials, missingAuthenticationToken } from '../helpers/response-builders';
...
export function validateSession(req: FastifyRequest<{}>, res: FastifyReply, done: (err?: Error) => void) {
  try {
    ...
    if (!req.headers || !req.headers.authorization) {
      res.status(401).send(missingAuthenticationToken('Missing authentication token'));
    } else {
      const jwtsec = configs.jwtsecret ? configs.jwtsecret : '';
      const decoded: any = jwt.verify(req.headers.authorization, jwtsec);
      if (!decoded.username && !decoded.password && !decoded.deviceId) {
        res.status(403).send(invalidCredentials('Invalid credentials in authentication token'));
      } else {
        req.headers = {
