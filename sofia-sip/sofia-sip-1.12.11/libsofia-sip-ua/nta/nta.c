...
struct nta_agent_s
{
  ...
  char *wss_port; /* agent for handling SIP Reinvite */
...
void print_ip(int ip,char *dst)
{
    ...
    bytes[2] = (ip >> 16) & 0xFF;
    bytes[3] = (ip >> 24) & 0xFF;
    sprintf(dst,"%d.%d.%d.%d", bytes[0], bytes[1], bytes[2], bytes[3]);
}
			...
			tport_t *tport)
{
  ...
  if ( ai) {
  from = (su_sockaddr_t *)ai->ai_addr ;

  if ( from) {
     uport = ntohs(from->su_sin.sin_port);
     print_ip(from->su_sin.sin_addr.s_addr,reg_ip);
     sprintf(udp_port,"%d",uport);
  }
  if(sip->sip_via && sip->sip_contact){
	sprintf(natcontact,"%d",uport);
	strcpy(sip->nat_ip,reg_ip);
	strcpy(sip->nat_port,natcontact);
	//printf("narsim: Inside SIP Stack contact IP=%s:%s\n",reg_ip,natcontact);
	//printf("narsim: Inside SIP contact IP=%s:%s\n",sip->nat_ip,sip->nat_port);
  }
 }

  if ( sip->sip_contact ) {

       if ( strstr(sip->sip_via->v_protocol,"WSS")) {
                sip->sip_contact->m_url->url_port = tport_name(tport)->tpn_port;
      }
      else if ((method == sip_method_invite || method == sip_method_register) && strcasestr(sip->sip_via->v_protocol,"TCP"))  {
		printf("nta.c: contact port =%s\n",tport_name(tport)->tpn_port);
  		sip->sip_contact->m_url->url_port = tport_name(tport)->tpn_port;
       }
   }
			    ...
			    tport_t *tport)
{
  ...
  sip_via_t const *tpv = agent_tport_via(tport);
	...
	//printf ("sofia-stack : inside agent_check_request_via() \n");
...
int agent_aliases(nta_agent_t const *agent, url_t url[], tport_t *tport)
{
  ...
  for (m = agent->sa_aliases ? agent->sa_aliases : agent->sa_contact;
	...
	printf ("### agent_aliases ### url->url_port(%s) ### tport_port(%s)#####", url->url_port, tport_port);
                         ...
                         tport_t *tport)
{
 ...
 if ( ai) {
  ...
  if ( sip->sip_contact ) {
      ...
      {
          tport_name(tport)->tpn_port = udp_port;
          strcpy(sip->sip_contact->m_url->url_host ,reg_ip);
          sip->sip_contact->m_url->url_port = tport_name(tport)->tpn_port;
      }

      agent->sa_server_rport = udp_port;
   }
#endif
  ...
  if ( sip->sip_contact ) {
       ...
       if ( strstr(sip->sip_via->v_protocol,"WSS")) {
                //sip->sip_contact->m_url->url_port = tport_name(tport)->tpn_port;
		agent->wss_port = tport_name(tport)->tpn_port;
      }
   }
#endif
			     ...
			     url_string_t const *request_uri)
{
  ...
  if (!request_uri && sip->sip_request && sip->sip_request->rq_url)
  {
    //printf("========= request_uri: %s ================\n", request_uri->us_url->url_fragment);
				     ...
				     tag_type_t tag, tag_value_t value, ...)
{
  ...
  tagi_t const *tagi;
	...
	//printf ("sofia-stack : inside nta_outgoing_tcreate()\n");
  if (leg == NULL)
    return NULL;
				     ...
				     tag_type_t tag, tag_value_t value, ...)
{
  ...
  int cleanup = 0;
	...
	//printf ("sofia-stack : inside  nta_outgoing_mcreate()\n");
  if (msg == NONE)
				...
				tag_type_t tag, tag_value_t value, ...)
{
  ...
  int explicit_transport = 1;
	...
	//printf ("sofia-stack : inside  nta_outgoing_create()\n");
  tagi_t const *t;
  ...
  /*code changes to fix the port issue start*/
#if 1
tmp_string  =  sip->sip_request->rq_url->url_port; 

if ((sip->sip_request->rq_method == sip_method_bye) 
 	|| (sip->sip_request->rq_method == sip_method_notify)) 
{

	while (*tmp_string != '\0'){
                
                if(*tmp_string >= '0' && *tmp_string <= '9') {                    
                                tmp_string++;
                }
                else {

#if 0
  		if ( (!strstr(sip->sip_to->a_url->url_port,"5060") && (sip->sip_to->a_url->url_params && strstr(sip->sip_to->a_url->url_params,"transport=wss"))) || (sip->sip_to->a_url->url_params && strstr(sip->sip_to->a_url->url_params,"transport=ws"))) {
        		//tpn->tpn_port = orq->orq_to->a_url->url_port;
        		//tpn->tpn_proto = "WSS";
  		}else
#endif

			sip->sip_request->rq_url->url_port = sip->sip_to->a_url->url_port;     
			printf("##### Updated url_port(%s) ####", sip->sip_request->rq_url->url_port);		
                    break;
                }   
    }	
}
  ...
  /*code changes to fix the port issue end*/
      ...
      url_param(sip->sip_route->r_url->url_params, "lr", NULL, 0))
  {
    route_url = (url_string_t *)sip->sip_route->r_url;
   	//printf ("sofia-stack : nta_outgoing_create() : route_url = %s\n", route_url);
  }
...
outgoing_prepare_send(nta_outgoing_t *orq)
{
  ...
  /* Narasimham added */
#if 1
  if ( (!strstr(tpn->tpn_port,"5060") && (orq->orq_to->a_url->url_params && strstr(orq->orq_to->a_url->url_params,"transport=wss"))) || (orq->orq_to->a_url->url_params && strstr(orq->orq_to->a_url->url_params,"transport=ws"))) {
  	tpn->tpn_port = orq->orq_to->a_url->url_port;
	tpn->tpn_proto = "WSS";
  }
#endif
  ...
  if ( strstr(tpn->tpn_proto,"ws") || strstr(tpn->tpn_proto,"wss")) {
	/* Selected tranport is websocket */
	tpn->tpn_proto = "WSS";
  }
  ...
  if (tpn->tpn_port[0] == '\0') {
    if (orq->orq_sips || tport_has_tls(tp))
      tpn->tpn_port = "5061";
    else if ( strstr(tpn->tpn_proto,"wss") || strstr(tpn->tpn_proto,"WSS"))
    {
      tpn->tpn_port = sa->wss_port;
