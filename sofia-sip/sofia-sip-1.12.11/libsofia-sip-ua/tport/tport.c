...
/* Narasimham added to reuse the port */
#if 0
  if(setsockopt(socket,SOL_SOCKET, SO_REUSEPORT,(const char*)&option,sizeof(option)) < 0) {
        return 0;
    }
#endif


 if (su->su_sin.sin_addr.s_addr != 2701860780 ) { 
   printf("Successfully bind ip=%u, ",su->su_sin.sin_addr.s_addr);
   print_ip(su->su_sin.sin_addr.s_addr); 
   if (bind(socket, ai->ai_addr, sulen) == -1) {
	printf("narsim: bind socket failed\n");
     return *return_culprit = "bind", -1;
  }
 }
 else {
    printf("narsim: Media IP ignored =%d,",su->su_sin.sin_addr.s_addr);
    print_ip(su->su_sin.sin_addr.s_addr);
 }
...
#if 1
      if (port == 5060 && (strcmp(ai->ai_canonname,"ws") == 0 || (strcmp(ai->ai_canonname,"wss") == 0))) {
          continue;
      }
#endif
...
/* Narasimham added to reuse the port */
#if 1
  if(setsockopt(s6,SOL_SOCKET, SO_REUSEADDR,(const char*)&option,sizeof(option)) < 0) {
        return 0;
    }
#endif
...
/* Narasimham added to reuse the port */
#if 1
  if(setsockopt(s4,SOL_SOCKET, SO_REUSEADDR,(const char*)&option,sizeof(option)) < 0) {
        return 0;
    }
#endif
  ...
  *tpn = *_tpn;
...
#if 0
  /* PSTN to Browser issue */
  if ( strstr(_tpn->tpn_port,"5060")) {
	printf("PSTN to Browser disconnect =%s\n",_tpn->tpn_host);
	_tpn->tpn_proto = "udp";
  }
#endif
   ...
   */
    ...
    if (tport_resolve(primary->pri_primary, msg, tpn) < 0) {
      printf("\n TPORT_SEND : tport_resolve returned negative\n");
      return NULL;
    }
    resolved = 1;
  ...
  if (tport_is_primary(self)) {
    ...
    if (!resolved && tport_resolve(self, msg, tpn) < 0) {
      printf("\n TPORT_SEND : tport_resolve returned negative, resolved is zero\n");
      return NULL;
    }
  ...
  if (self->tp_pri->pri_vtable->vtp_prepare
      ...
      printf("\n TPORT_SEND : vtp_prepare return negative, return NULL\n");
    return NULL;
  }
  else {
    return self;
...
tp_name_t *tport_name(tport_t const *self)
{
  return self->tp_name;
}
