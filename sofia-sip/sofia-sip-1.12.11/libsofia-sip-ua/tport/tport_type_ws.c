				 ...
				 char const **return_culprit)
{
  ...
  if (path) {
    ...
    key  = su_sprintf(autohome, "%s/%s", path, "STAR_unifiedring_co_uk_key.pem");
	if (access(key, R_OK) != 0) key = NULL;

	//cert = su_sprintf(autohome, "%s/%s", path, "wss.crt");
	cert = su_sprintf(autohome, "%s/%s", path, "STAR_unifiedring_co_uk.crt");
	if (access(cert, R_OK) != 0) cert = NULL;

	chain = su_sprintf(autohome, "%s/%s", path, "STAR_unifiedring_co_uk.ca-bundle");
	...
	//if ( !chain ) chain = su_sprintf(autohome, "%s/%s", path, "wss.pem");
       	if ( !key )  key  = su_sprintf(autohome, "%s/%s", path, "STAR_unifiedring_co_uk_key.pem");
        if ( !cert ) cert = su_sprintf(autohome, "%s/%s", path, "STAR_unifiedring_co_uk.crt");
        if ( !chain ) chain = su_sprintf(autohome, "%s/%s", path, "STAR_unifiedring_co_uk.ca-bundle");
	if (access(key, R_OK) != 0) key = NULL;
	if (access(cert, R_OK) != 0) cert = NULL;
	if (access(chain, R_OK) != 0) chain = NULL;
  }
