...
#define LOG5(m) \
  ...
  if (!ss || !sip || 300 <= status)
    ...
    else if ((retval = soa_process_answer(nh->nh_soa, NULL)) < 0) {
      printf("TAG: soa_process_answer : returned error %d \n",retval); 
      LOG5("error processing SDPP");
      /* XXX */
      sdp = NULL;
    }
...
int nua_invite_client_ack(nua_client_request_t *cr, tagi_t const *tags)
{
  ...
  if (!cseq)
	...
	status = 900, phrase = "Internal media error";
        printf("NUA_INVITE_CLIENT_ACK : Internal Media Error : status %d\n",status);
	...
	ss->ss_sdp_version = soa_get_user_version(nh->nh_soa);
      ...
      for (cru = ds->ds_cr; cru; cru = cru->cr_next) {
        printf("\nNUA_INVITE_CLIENT_ACK : offer_sent %d answer_recv %d \n",cru->cr_offer_sent,cru->cr_answer_recv);
	if (cr != cru && cru->cr_offer_sent && !cru->cr_answer_recv)
        {
           printf("NUA_INVITE_CLIENT_ACK : BREAK\n");
	  break;
        }
      }
	...
	status = 988, phrase = "Incomplete offer/answer";
        printf("NUA_INVITE_CLIENT_ACK : NO SDP ANSWER: status %d\n",status);
	...
	nua_stack_event(nh->nh_nua, nh, NULL,
      ...
      printf("NUA_INVITE_CLIENT_ACK : Cannot Send ACK : status %d\n",status);
