...
class xGateNetConnection
{
  public:
    ...
    xGateNetConnection(const xGateNetConnection &src)
    {
      m_IpAdd = src.m_IpAdd;
      m_Port = src.m_Port;
      m_protocol = src.m_protocol;
      m_fd = src.m_fd;
      m_connid = src .m_connid;
    }
};

struct xGateSocketTimerData
{
   int fd;
...
{
  xGateTimerData(){};
  xGateTimerData(std::string &uid, unsigned int type, int retryCount, IURDefines::MODULE_ID requestorModule, void * ctxData = nullptr) :
...
typedef enum
{
  ...
  EN_XGATE_MSG_DB_MODULE,
  EN_XGATE_MSG_NETIO_MODULE,
  EN_XGATE_MSG_MONITOR_MODULE
}xGateMsgType;
