...
typedef enum
{
  ...
  EN_USER_DEVICE_WEB_VIDEOMEET=7,
  EN_USER_DEVICE_IOS_VIDEOMEET=8,
  EN_USER_DEVICE_ANDROID_VIDEOMEET=9,
} UserDevice;
...
class xGateUtil : public ACE_Task<ACE_MT_SYNCH> {
	public:  
		...
		//static bool create_thread(void (*ptFunction)(void *),string class_name);
...
typedef enum
{
  ...
  EN_XGATE_MG_DTLS_CONN_FAILED = 26,
  EN_XGATE_MG_DTLS_RECONNECT = 27,
  EN_XGATE_MG_RECORDING_EVENT = 28,
  EN_XGATE_MG_TILE_CHANGE_REQUEST = 31,
  EN_XGATE_MG_SCREENSHARE_VIDEO_REQUEST = 32,
  EN_XGATE_MG_ACTIVE_SPEAKER_VIDEO_REQUEST = 33,
  EN_XGATE_MG_ACTIVE_SPEAKER_INFO = 34,
  EN_XGATE_MG_MAX_TYPE,
} xGateMgMsgType;
...
typedef enum
{
  ...
  EN_MEDIA_EVENT_RECORD_PAUSE = 22,
  EN_MEDIA_EVENT_RECORD_RESUME = 23,
  ...
  EN_MEDIA_EVENT_RECORD_PLAY = 29,
  EN_MEDIA_EVENT_RECORD_SAVE = 30,
  EN_MEDIA_EVENT_RECORD_DELETE = 31,
} xGateMediaEvent;
...
typedef enum{
        CALL_MESSAGE_NULL       = 0,
        CALL_QUEUE_MESSAGE      = 1,
        CALL_PARK_MESSAGE       = 2,
        CALL_HUNT_MESSAGE       = 3,
        CCAAS_MESSAGE           = 4,
        CALL_INFO_MESSAGE_PBX   = 5,
} PBXModuleType;

typedef enum{
        CALL_CHANNEL_NULL       = 0,
        CALL_CHANNEL_AUDIO      = 1,
        CALL_CHANNEL_VIDEO      = 2,
} CcaasChannelType;
...
typedef enum
{
	CB_COMMAND_NULL=0,
	/* CCAAS */
	EN_CCAAS_CB_ACD_CALL_INIT=1,
	EN_CCAAS_CB_ACD_CALL_INIT_SUCCESS=2,
	EN_CCAAS_CB_ACD_CALL_Q_ANNC=3,
	EN_CCAAS_CB_ACD_CALL_AGENT_CONNECTED=4,
	EN_CCAAS_CB_ACD_CALL_DISCONNECTED=5,
	EN_CCAAS_CB_ACD_TRANSFER_AGENT_DIALOUT=6,
	EN_CCAAS_CB_ACD_TRANSFER_AGENT_CONNECTED=7,
	EN_CCAAS_CB_ACD_TRANSFEREE_DIAL_TIMEOUT=8,
	EN_CCAAS_CB_ACD_TRANSFEREE_REJECT=9,
	EN_CCAAS_CB_ACD_AGENT_DISCONNECTED=10,
	EN_CCAAS_CB_ACD_CUSTOMER_DISCONNECTED=11,
...
class ParticipantDetail {
  public:
    ParticipantDetail():
              m_callId(""),m_joinerType(0) {};
    ~ParticipantDetail() {};
    ParticipantDetail& operator=(ParticipantDetail &src)
    {
      m_callId = src.m_callId;
      m_joinerType = src.m_joinerType;
    };

    unsigned int m_joinerType;
    string m_callId;
};

class URMeetUserDetail {
  public:
    URMeetUserDetail():
              m_strEmail(""), m_strProfileImg(""), m_strSipId(""), m_strName(""), 
							m_strExtn(""), m_strUserType(""), m_iRoleType(0) {};
    ~URMeetUserDetail() {};
    URMeetUserDetail& operator=(URMeetUserDetail &src)
    {
      m_strEmail = src.m_strEmail;
			m_strProfileImg = src.m_strProfileImg;
			m_strSipId = src.m_strSipId;
			m_strName = src.m_strName;
			m_strExtn = src.m_strExtn;
			m_strUserType = src.m_strUserType;
			m_iRoleType = src.m_iRoleType;
    };
...
class MgMediaDetail
{
  public:
    ...
    std::list<ParticipantDetail> participantList;
    std::list<URMeetUserDetail> meetUserList;

    MgMediaDetail() 
      : mgresource_id(""), mgmsg_type(0), mg_ip(""), call_type(0), 
      call_id(""), gateway_id(0), leg_id(0), context_id(0), is_ccaas(0), 
      media_proto(""), out_proto(""), browser_type(""), record_file(""), record_url(""), file_size(0), 
      dtmf_digits(""), media_event(0), joiner_type(0), plan_type(0), screenshare_id(""), 
      conn_id(""), device_id(""), meeting_name(""), sip_id(""), call_dir(0), sig_type(3), 
			sdp_str(""), org_rtp_ep(""), dst_rtp_ep(""), record_id(""), customer_num(""), domain_id(0), agent_id(0) {};
