...
{
  XGLOG_DEBUG("loadDocument called");
  if(m_parserString.empty()){
    XGLOG_ERROR("loadDocument failed. given xml data is empty !");
    setError(RETURN_FAIL,"XML String empty");
    return m_erid;
  }
  try {
    m_doc.parse<0>(&m_parserString[0]);
    setError(RETURN_SUCCESS, "XML String empty");
  }
  catch (const std::runtime_error& e) {
    XGLOG_ERROR("loadDocument failed due to xml parse runtime error: %s !", e.what());
    setError(RUNTIME_ERROR,e.what());
  }
  catch (const rapidxml::parse_error& e) {
    XGLOG_ERROR("loadDocument failed due to xml parse error what(): '%s'!", e.what());
    std::cout << e.where<char>();
    setError(PARSER_ERROR,e.what());
  }
  catch (const std::exception& e) {
    XGLOG_ERROR("loadDocument failed due to xml parse error: %s !", e.what());
    setError(STANDARD_ERROR,e.what());
  }
  catch (...) {
    XGLOG_ERROR("loadDocument failed due to xml parse unknown error!");
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::getRootNode succesfully");
  if(getDoc().first_node()) { 
    return getDoc().first_node(); 
  } else { 
    setError(RETURN_FAIL,"Root Node Invalid"); 
  }
  return NULL ;
}
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::getSubNode success");
  if(nodeName.empty()) {
    setError(RETURN_FAIL,"xGateRapidXML::getSubNode nodeName empty");
    return NULL;
  }

  if(node == NULL) {
    setError(RETURN_FAIL,"xGateRapidXML::getSubNode parent node is null");
    return NULL;
  }

  rapidxml::xml_node<>*subnode = node->first_node(nodeName.c_str());
  if(subnode == NULL) {
    setError(RETURN_FAIL,"xGateRapidXML::getSubNode sub node is null");
  }
  return subnode;
}

XPARSER_ERROR xGateRapidXML::getAttributeValue(std::string attName, rapidxml::xml_node<>* node, std::string &ret)
{
  setError(RETURN_SUCCESS,"xGateRapidXML::getAttributeValue success");
  ret ="";
  if(attName.empty()) {
    setError(RETURN_FAIL,"xGateRapidXML::getAttributeValue attribute name empty");
    return m_erid;
  }

  if(node == NULL) {
    setError(RETURN_FAIL,"xGateRapidXML::getAttributeValue  node is null");
    return m_erid;
  }
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::createNode successfull");
  ...
  try {
    switch(typ) {
      ...
      default:
        {
          setError(RETURN_FAIL,"xGateRapidXML::createNode default case failed");
          break;
        }
    }
  } catch(...) {
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::encodeToXMLString successfull");
  try{
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::createAttributewithValue successfull");
  if (node == NULL) {
    setError(RETURN_FAIL,"xGateRapidXML::createAttributewithValue Node is Null failed");
    return m_erid;
  }
  if(key.empty() || value.empty()){
    setError(RETURN_FAIL,"xGateRapidXML::createAttributewithValue key or value is empty failed");
    return m_erid;
  }
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::appendNodeToDoc successfull");
  try{
    if(node == NULL){
      setError(RETURN_FAIL,"xGateRapidXML::appendNodeToDoc Node is Null failed");
      return m_erid;
    }
...
{
  setError(RETURN_SUCCESS,"xGateRapidXML::appendNodeToNode successfull");
  if(parent == NULL){
    setError(RETURN_FAIL,"xGateRapidXML::appendNodeToNode parent is NULL failed");
    return m_erid;
  }
  if(child == NULL){
    setError(RETURN_FAIL,"xGateRapidXML::appendNodeToNode child is NULL failed");
    return m_erid;
  }
  try{
