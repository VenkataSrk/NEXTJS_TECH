  ...
  m_run(false)
{
  ...
  //CLog::Warning(THISMODULE, "xGateDispatcher::xGateDispatcher(%s)", m_serverType);
}
...
{
  //CLog::Warning(THISMODULE, "xGateDispatcher::stop(%s)", m_serverType);
  ...
  //CLog::Warning(THISMODULE, "xGateDispatcher::stop Stopped xGateDispatcher(%s)", m_serverType);
...
{
  //CLog::Warning(THISMODULE, "xGateDispatcher::init(%s)", m_serverType);
  m_run = true;
  ...
  if (activate(THR_NEW_LWP , 1) == -1 )
  {
    ...
    //CLog::Error(THISMODULE, "xGateDispatcher::startThread(%s)"\
  ...
  {
    // Successfull start
    //CLog::Warning(THISMODULE, "xGateDispatcher::init(%s) Success", m_serverType);
  }
  return retVal;
}
...
{
  //CLog::Warning(THISMODULE, "xGateDispatcher::svc(%s)", m_serverType);
   int rc;
   rc = pthread_setname_np(ACE_Thread::self(), "SipDispatcher");
   if(rc!=0)
     printf("Failed to set name for sip xGateDispatcher thread");
  ...
  while(m_run)
  {
    ...
    else
    {
      ...
      //CLog::Error(THISMODULE, "xGateDispatcher::svc(%s) Error in Message Queue", m_serverType);
      m_run = false;
    }
...
{
  //CLog::Warning(THISMODULE, "xGateDispatcher::dispatchMessage(%s)", m_serverType);
  return EN_XGATE_STATUS_SUCCESS;
}
