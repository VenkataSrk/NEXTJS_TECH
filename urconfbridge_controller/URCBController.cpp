...
bool URCBController::init()
{
   ...
   RedisInfo redisInfo;
   redisInfo.m_strContextId = "preet1234567890";
   redisInfo.m_enReqModule = IURDefines::MODULE_ID::UR_MODULE_SERVICE_CONTROLLER;
   redisInfo.m_strKey = "ACD_PREET_123";
   redisInfo.m_strValue = "PREET_ACD_CHANNEL";
...
bool URCBController::update_callid_in_socketio_user_info(URCBCallContext *& ptCtx)
{
   ...
   if(itr != tSIOUserInfoMap.end())
   {
      ...
      ptCtx->m_strCustomerId = ptSioUserInfo->m_tURConfCcaasInfo.m_strCustomerId.c_str();
      ptCtx->m_strAgentId = ptSioUserInfo->m_tURConfCcaasInfo.m_strAgentId.c_str();
      ptCtx->m_strCli = ptSioUserInfo->m_tURConfCcaasInfo.m_strCli.c_str();
      ptCtx->m_strDdi = ptSioUserInfo->m_tURConfCcaasInfo.m_strDdi.c_str();
      ptCtx->m_isCcaas = ptSioUserInfo->m_isCcaas;
      getConfCallType(ptSioUserInfo->m_tURConfCcaasInfo.m_strCallType, ptCtx->m_enCallType);
...
bool URCBController::handleCallConnectedNotify(xGateBaseMsg *pMsg) 
{
   ...
   else if(ptCtx->m_enumConfUserType == CONF_USER_TYPE_AGENT)
   {
      if(m_tURConfRoomCtxHndlr.get_acd_channel(strConfRoomId, ptCtx->m_strChannel))
      {
				...
				if(ptCtx->m_enCallType == URCONF_CALL_TYPE_TRANSFER)
				{
					ptCtx->setCcaasEvent(EN_CCAAS_CB_ACD_TRANSFER_AGENT_CONNECTED);
				}
				else
				{
   ...
   else 
   {
	 ...
	 if(ptCtx->m_enCallType == URCONF_CALL_TYPE_TRANSFER)
	 {
		 ptSioUserInfo->m_tURConfEvent.m_strEventName = "transfer_agent_connected";
		 ptSioUserInfo->m_tURConfCcaasInfo.m_strPrimaryAgentId = urConfRoom->m_strPrimaryAgentId.c_str();
...
bool URCBController::handleCallClearing(URCBCallContext * ptCtx)
{
   ...
   if(confCallCount == 0) 
   {
      cancel_timer(strConfRoomId, EN_SCHEDULER_URCONF_ACTIVE_SPEAKER_INFO);
      ...
      if(ptCtx->m_enumConfUserType == CONF_USER_TYPE_CUSTOMER)
      {
	      ptCtx->setCcaasEvent(EN_CCAAS_CB_ACD_AGENT_DISCONNECTED);
	      post_ccaas_conf_info_to_acd(ptCtx);
      }
      else if(ptCtx->m_enumConfUserType == CONF_USER_TYPE_AGENT)
      {
	      ptCtx->setCcaasEvent(EN_CCAAS_CB_ACD_CUSTOMER_DISCONNECTED);
	      post_ccaas_conf_info_to_acd(ptCtx);
      }
   }
...
bool URCBController::sendConfUserRecStartReq(URCBCallContext * ptCtx)
{
   ...
   mediaDetail.record_id = ptCtx->m_strRecId.c_str();
	 if(!ptCtx->m_strDomainId.empty())
   	mediaDetail.domain_id = stoi(ptCtx->m_strDomainId);
	 if(!ptCtx->m_strAgentId.empty())
   	mediaDetail.agent_id = stoi(ptCtx->m_strAgentId);
   mediaDetail.customer_num = ptCtx->m_strCustomerNum.c_str();
	 mediaDetail.is_ccaas = ptCtx->m_isCcaas;
...
bool URCBController::handleRedisGetResponse(xGateBaseMsg *pMsg)
{
   ...
   if (!m_tURCBCallCtxHndlr.getURCBCallContext(strCallId, ptCtx))
   {
      XGLOG_ERROR("handleRedisGetResponse get call context failed for callid (%s)", strCallId.c_str());
      return false;
   }
...
bool URCBController::handleRedisModuleMsg(xGateBaseMsg *pMsg)
{
   ...
   {
      XGLOG_INFO("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
      XGLOG_INFO(" ==> SOCKET TO CONFBRIDGE : %s", strMbcMsg.c_str());
      XGLOG_INFO("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
...
bool URCBController::handle_ccaas_acd_broadcast_msg(URCcaasBroadcastMsg * ptURCcaasBcastMsg)
{
   ...
   switch(event)
   {
      ...
      case EN_CCAAS_CB_ACD_CALL_Q_ANNC:
      {
			...
			case EN_CCAAS_CB_ACD_TRANSFEREE_REJECT:
			{
				XGLOG_INFO("handle_ccaas_acd_broadcast_msg processing TRANSFEREE_REJECT event for conf_room:%s", \
						ptURCcaasBcastMsg->m_strUuid.c_str());

				std::string strCallId = ptURCcaasBcastMsg->m_strCallId.c_str();
				std::string strConnId("");
				URCBCallContext * ptCtx = NULL;
				if (!m_tURCBCallCtxHndlr.getURCBCallContext(strCallId, ptCtx)) 
				{
					XGLOG_ERROR("handle_ccaas_acd_broadcast_msg TRANSFEREE_REJECT get URCBCallContext failed for callid (%s)",\
							strCallId.c_str());
					return false;
				}
				strConnId = ptCtx->m_strConfConnId.c_str();
				...
				ptSioUserInfo->m_tURConfEvent.m_strEventName = "transferee_declined";
				ptSioUserInfo->m_tURConfUserMediaInfo.m_strSdp = "";
				post_msg_to_socketio(ptSioUserInfo);
				break;
			}
      case EN_CCAAS_CB_ACD_TRANSFEREE_DIAL_TIMEOUT:
      {
        XGLOG_INFO("handle_ccaas_acd_broadcast_msg processing TRANSFEREE_DIAL_TIMEOUT event for conf_room:%s", \
            ptURCcaasBcastMsg->m_strUuid.c_str());
        ...
        if (!m_tURCBCallCtxHndlr.getURCBCallContext(strCallId, ptCtx))
        {
          XGLOG_ERROR("handle_ccaas_acd_broadcast_msg TRANSFEREE_DIAL_TIMEOUT get URCBCallContext failed for callid (%s)",\
              strCallId.c_str());
          return false;
        }
        strConnId = ptCtx->m_strConfConnId.c_str();
...
bool URCBController::handle_sio_mbc_message(URCBSIOUserInfo * ptSioUserInfo)
{
   ...
   switch(msgType) 
   {
      ...
      case URCONF_SIO_MSG_AGENT_TRANSFER_INIT :
      {
         XGLOG_INFO("In handle_sio_mbc_message URCONF_SIO_MSG_AGENT_TRANSFER_INIT");
         URCBCallContext * ptCtx = NULL;
         URConfRoom * urConfRoom = NULL;
         std::string strID("");
         if(!m_tURConfRoomCtxHndlr.getConfRoomCallContext(strConfRoomId.c_str(), urConfRoom))
         {
            XGLOG_ERROR("handle_sio_mbc_message URCONF_SIO_MSG_AGENT_TRANSFER_INIT getConfRoomCallContext conf room id (%s) not found",\
                  strConfRoomId.c_str());
            return false;
         }
         ...
         for(userItr; userItr != ptSioUserInfo->m_tURConfUserInfoList.end();userItr++)
         {
            ...
            if(itr != urConfRoom->m_URCBSIOUserInfoList.end())
            {
               ...
               if (!m_tURCBCallCtxHndlr.getURCBCallContext(strID, ptCtx))
               {
                  XGLOG_ERROR("handle_sio_mbc_message URCONF_SIO_MSG_AGENT_TRANSFER_INIT get URCBCallContext failed for callid (%s)",\
                        strID.c_str());
                  return false;
               }
							 urConfRoom->m_strPrimaryAgentId = strConnId.c_str();
               ptCtx->m_strAgentExt = ptSioUserInfo->m_tURConfCcaasInfo.m_strAgentExt.c_str();
							 ptCtx->m_strAgentMail = ptSioUserInfo->m_tURConfCcaasInfo.m_strAgentMail.c_str();
							 ptCtx->setCcaasEvent(EN_CCAAS_CB_ACD_TRANSFER_AGENT_DIALOUT);
							 post_ccaas_conf_info_to_acd(ptCtx);			   			   
            }
         }
         break;
         ...
         for(userItr; userItr != ptSioUserInfo->m_tURConfUserInfoList.end();userItr++)
         {
            ...
            if(itr != urConfRoom->m_URCBSIOUserInfoList.end())
            {
               ...
               ptCtx->m_strRecUrl = userInfo.m_strRecUrl.c_str();
							 ptCtx->m_strRecId = userInfo.m_strRecId.c_str(); 
      ...
      case URCONF_SIO_MSG_ACTIVE_SPEAKER_INFO :
      {
         ...
         if(userItr != ptSioUserInfo->m_tURConfUserInfoList.end())
         {
            strConnId = userItr->first;           
            URActiveSpeakerInfo t_urActiveSpeakerInfo = urConfRoom->m_URActiveSpeakerInfo;
            XGLOG_INFO("$$$$$$$ ACTIVE SPEAKER INFO IN URCONFROOM MAP connId:%s and callid:%s", \
                           t_urActiveSpeakerInfo.m_strConnId.c_str(), t_urActiveSpeakerInfo.m_strCallId.c_str());

            bool b_isNewConfActiveSpeaker = false;
            if(t_urActiveSpeakerInfo.m_strConnId.empty())
            {
               b_isNewConfActiveSpeaker = true;
               XGLOG_INFO("URCONF_SIO_MSG_ACTIVE_SPEAKER_INFO found as new meet:%s", strConfRoomId.c_str());
            }
            ...
            if(b_isNewConfActiveSpeaker)
            {
               XGLOG_INFO("$$$$$$$$$$$$ SENDING ACTIVE SPEAKER INFO && SCHEDULING TIMER for new meet:%s $$$$$$$$$$$", strConfRoomId.c_str());
...
std::string URCBController::form_socketIO_json_msg(URCBSIOUserInfo *ptSioUserInfo)
{
   ...
   if(ptSioUserInfo->m_tURConfUserInfoList.size()>0) 
   {
      ...
      for(itr; itr != ptSioUserInfo->m_tURConfUserInfoList.end(); itr++) 
      {
         ...
         newUserInfoVal.AddMember("call_id", setJsonParam(ptSioUserInfo->m_strConfCallId, doc), allocator);
   ...
   urConfCcaasInfo.AddMember("primary_agent", setJsonParam(confCcaasInfo.m_strPrimaryAgentId, doc), allocator);
...
bool URCBController::decode_sio_json_msg_from_MBC(const char* jsonData,URCBSIOUserInfo *& ptSioUserInfo)
{
   ...
   //XGLOG_INFO( "parsing socket-io json data: %s", jsonData);
   Document doc;
   ...
   if(doc.HasMember("ur_conf_message"))
   {
	 ...
	 const Value &confMediaInfoDetails = confMsgDetails["ur_media_info"];
      ...
      if(confMsgDetails.HasMember("ur_ccaas_info"))
      {
         ...
         if(confCcaasInfoDetails.HasMember("cli"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strCli  = confCcaasInfoDetails["cli"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'cli' is missing in json message");
         }

         if(confCcaasInfoDetails.HasMember("ddi"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strDdi  = confCcaasInfoDetails["ddi"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'ddi' is missing in json message");
         }
         ...
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'domain_id' is missing in json message");
				 ...
				 if(confCcaasInfoDetails.HasMember("call_type"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strCallType  = confCcaasInfoDetails["call_type"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'call_type' is missing in json message");
         }
         ...
         if(confCcaasInfoDetails.HasMember("customer_id"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strCustomerId  = confCcaasInfoDetails["customer_id"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'customer_id' is missing in json message");
         }

         if(confCcaasInfoDetails.HasMember("agent_id"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strAgentId  = confCcaasInfoDetails["agent_id"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'agent_id' is missing in json message");
         }

         if(confCcaasInfoDetails.HasMember("agent_ext"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strAgentExt  = confCcaasInfoDetails["agent_ext"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'agent_ext' is missing in json message");
         }

         if(confCcaasInfoDetails.HasMember("agent_mail"))
         {
            ptSioUserInfo->m_tURConfCcaasInfo.m_strAgentMail  = confCcaasInfoDetails["agent_mail"].GetString();
         }
         else
         {
            XGLOG_ERROR("readMediaInfo Details failed. 'agent_mail' is missing in json message");
         }
      }
   }
   else
...
URCcaasBroadcastMsg* URCBController::fill_ccaas_bcast_msg_info(URCBCallContext * ptCtx)
{
   ...
   ptURCcaasBcastMsg->m_strUuid = ptCtx->m_strConfRoomId.c_str();
	 ptURCcaasBcastMsg->m_strCallId = ptCtx->m_strCallId.c_str();
   ptURCcaasBcastMsg->m_enCcaasEvent = ptCtx->getCcaasEvent();
   ptURCcaasBcastMsg->m_strCli = ptCtx->m_strCli.c_str();
   ptURCcaasBcastMsg->m_strDdi = ptCtx->m_strDdi.c_str();
   ...
   ptURCcaasBcastMsg->m_strCustomerNum = ptCtx->m_strCustomerNum.c_str();
	 ptURCcaasBcastMsg->m_strCustomerId = ptCtx->m_strCustomerId.c_str();
	 ptURCcaasBcastMsg->m_strAgentExt = ptCtx->m_strAgentExt.c_str();
	 ptURCcaasBcastMsg->m_strAgentMail = ptCtx->m_strAgentMail.c_str();
...
std::string URCBController::encode_ccaas_conf_info_json_msg(URCcaasBroadcastMsg * ptURCcaasBcastMsg)
{
    ...
    outputVal.AddMember("msg_uuid", setJsonParam(ptURCcaasBcastMsg->m_strUuid, doc), allocator);
		outputVal.AddMember("call_id", setJsonParam(ptURCcaasBcastMsg->m_strCallId, doc), allocator);
    ...
    outputVal.AddMember("customer_num", setJsonParam(ptURCcaasBcastMsg->m_strCustomerNum, doc), allocator);
		outputVal.AddMember("customer_id", setJsonParam(ptURCcaasBcastMsg->m_strCustomerId, doc), allocator);
	
	  switch(ptURCcaasBcastMsg->m_enCcaasEvent)
		{
			case EN_CCAAS_CB_ACD_TRANSFER_AGENT_DIALOUT:
			{
    		outputVal.AddMember("agent_ext", setJsonParam(ptURCcaasBcastMsg->m_strAgentExt, doc), allocator);
    		outputVal.AddMember("agent_mail", setJsonParam(ptURCcaasBcastMsg->m_strAgentMail, doc), allocator);
				break;
			}
			default:
			{
...
URCcaasBroadcastMsg * URCBController::decode_ccaas_info_json_message(const char* data, URCcaasBroadcastMsg *& ptURCcaasBcastMsg)
{
   ...
   {
      ptURCcaasBcastMsg->m_strCallId = doc["call_id"].GetString();
   }
   else
   {
      XGLOG_ERROR("URCBController::decode_ccaas_info_json_message failed. 'call_id' detail is missing in json message !");
   }
...
bool URCBController::fillConfUserInfoDetails(const Value &userInfoDetails, URCBSIOUserInfo *ptSioUserInfo)
{
   ...
   if(userInfoDetails.HasMember("rec_id"))
   {
      ConfUserInfo.m_strRecId  = userInfoDetails["rec_id"].GetString();
   }
   ptSioUserInfo->m_tURConfUserInfoList.insert({ConfUserInfo.m_strConnId,ConfUserInfo});
...
bool URCBController::getConfSIOMsgType(string &strEventName,URConfSIOMsgType &msgType)
{
   ...
   else if(strncmp(strEventName.c_str(),"customer_joining",strEventName.length()) == 0 )
   {
      msgType = URCONF_SIO_MSG_CUSTOMER_JOINING;
   }
   else if(strncmp(strEventName.c_str(),"agent_joining",strEventName.length()) == 0 )
   {
      msgType = URCONF_SIO_MSG_AGENT_JOINING;
   }
   else if(strncmp(strEventName.c_str(),"transfer_init",strEventName.length()) == 0 )
   {
      msgType = URCONF_SIO_MSG_AGENT_TRANSFER_INIT;
   }
...
bool URCBController::getConfCallType(string &strCallType, URConfCallType &iCallType)
{
	
	if(!strCallType.empty() && strncmp(strCallType.c_str(),"transfer",strCallType.length()) == 0 )
	{
		iCallType = URCONF_CALL_TYPE_TRANSFER;
	}
  else
	{
		iCallType = URCONF_CALL_TYPE_UNKNOWN;
	}
	return true;
}
