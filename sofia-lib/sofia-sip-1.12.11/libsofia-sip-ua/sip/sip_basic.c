...
issize_t sip_via_d(su_home_t *home, sip_header_t *h, char *s, isize_t slen)
{
  sip_via_t *v = (sip_via_t *)h;
	...
	//printf ("sofia-stack : inside sip_via_d().... host = %s\n", v->v_host);
  assert(h);
...
issize_t sip_via_e(char b[], isize_t bsiz, sip_header_t const *h, int flags)
{
  ...
  assert(sip_is_via(h));
	...
	//printf ("sofia-stack : 1) inside sip_via_e().... host = %s\n", v->v_host);
	//v->v_host = getLocalInterface();
	//v->v_host = "10.22.7.75";
	//printf ("sofia-stack : 2) inside sip_via_e().... host = %s\n", v->v_host);
...
isize_t sip_via_dup_xtra(sip_header_t const *h, isize_t offset)
{
  sip_via_t const *v = (sip_via_t *)h; 
	...
	//printf ("sofia-stack : 1) inside sip_via_dup_xtra().... host = %s\n", v->v_host);
	//v->v_host = getLocalInterface();
	/////v->v_host = "10.22.7.75";
	//printf ("sofia-stack : 2) sip_via_dup_xtra().... host = %s\n", v->v_host);
		      ...
		      char *b, isize_t xtra)
{
  ...
  char *end = b + xtra;
	...
	//printf ("sofia-stack : inside sip_via_dup_one().... host = %s\n", v->v_host);
  ...
  MSG_STRING_DUP(b, v->v_host, o->v_host);
	// sofia-stack
	//MSG_STRING_DUP(b, v->v_host, "10.22.7.75");
	/////////////////////////////
                          ...
                          ...)
{
  ...
  if (transport)
	...
	//printf ("sofia-stack : inside sip_via_create().... host = %s\n", host);
  via->v_host = host;
  via->v_port = port;
	//printf ("sofia-stack : inside sip_via_create().... via->v_host = %s\n", via->v_host);
