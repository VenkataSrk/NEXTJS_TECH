...
bool xGateDBService::init()
{
  ...
  {
    XGLOG_ERROR("xGateDBService::init() failed to start thread !");
    return false;
  }
  else
  {
    XGLOG_INFO("xGateDBService::init() success !");
  }
...
{
  int rc;
  rc = pthread_setname_np(ACE_Thread::self(), "DBSrvc");
  if(rc!=0)
    printf("Failed to set name for DB service thread");
...
bool xGateDBService::handle_msg(ACE_Message_Block *pAmb)
{
   ...
   if(pMsg  == nullptr) {
      XGLOG_ERROR("xGateDBService::handle_msg recvd invalid xGBaseMsg Message");
      return false;
   }
