...
import { updateQueueIdInsta } from '../dao/instagram';
import { logger } from '../plugins/log';
import { socialmediaRedisClient } from '../plugins/redis';
  ...
  try {
    ...
    if (agentDetails.length === 0) {
      ...
      if (readySupervisorsData?.length !== 0) { // Connecting chat with supervisors
        ...
        logger.info('liveagent Supervisors', readySupervisorsData);
      }
    }

    if (agentDetails?.length === 0) {
      const readyAdminsData: any = await readyAdmins(channelId); // checking admins in ready state
      logger.info('readyAdminsData', JSON.stringify(readyAdminsData));
      if (readyAdminsData?.length !== 0) { // Connecting chat with admins
        ...
        logger.info('liveagent Admins', agentDetails);
      }
    }
    return agentDetails;
  } catch (err) {
    logger.error('transferChatToAvailableAgent err', err);
    return [];
  }
};
...
const readyAgentsSupervisorsAdmins = async (channel: any) => {
  try {
    ...
    const data = await socialmediaRedisClient.hgetall(`${channel?.domainId}_presence`);
...
const chatLimitFilter = async (channel: any, currentNode: any, filteredData: any) => {
  try {
    ...
    logger.info(' chatLimitFilter currentChatCount', JSON.stringify(currentChatCount));
    ...
    logger.error('chatLimitFilter err', err);
    return [];
  }
};
  ...
  try {
    if (channelId?.channelType === 'Facebook') {
      if (previousStatus?.length === 0 || previousStatus === undefined) {
        ...
        const insertNewStatus: any = await insertStatus(channelId?.FacebookHistoryId, channelId?.domainId, insertArr);
    ...
    if (previousStatus?.length === 0 || previousStatus === undefined) {
      const insertArr: any = [+(agent?.ext)];
      const insertNewStatus: any = await insertStatus(channelId?.InstagramHistoryId, channelId?.domainId, insertArr);
      logger.info('sendAgentDetails insertNewStatus', insertNewStatus);
    } else {
      const verifyArr: any = JSON.parse(previousStatus[0]?.chatuserlog);
      const insertArr: any = [...verifyArr, +(agent?.ext)];
...
const agentChatLimitFilter = async (channel: any, currentNode: any, filteredData: any) => {
  try {
    ...
    logger.error('agentChatLimitFilter err', err);
    return [];
  }
};
...
const queueChat = async (channelId: any, currentNode: any, readyAgentsData: any) => {
  try {
    ...
    const routingProfiles: any = await getRoutingProfiles(currentNode?.data?.information?.transferTo, channelId?.domainId);
    ...
    const sortedRoutingProfiles: any = routingProfiles.sort((a: any, b: any) => a.priority - b.priority);
...
const skillRouting = async (channelId: any, readyAgentsData: any) => {
  try {
    ...
    logger.error('skillRouting err', err);
    return [];
  }
};
