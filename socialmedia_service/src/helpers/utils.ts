...
import { getconnectDataFB, getCustomerCoId } from '../dao/facebook';
import { getconnectData, getCustomerCoIdInsta } from '../dao/instagram';
import { logger } from '../plugins/log';
import { CRM_DOMAINS, RESPONSE } from './constants';
...
export const getPlandetails = (plan_details: any) => {
  try {
    if (plan_details?.planId === 32) { //Xaia1Plus
      ...
      return plan_details?.isBot === 0 && plan_details?.FacebookFlowId === 0 ? 'Traditional' : 'CAI'
    } else if (plan_details?.planId === 33) { //Xaia3Plus
      return plan_details?.isBotPurchased === 1 && plan_details?.isBot === 1 ? (plan_details?.FacebookFlowId === 0 ? 'CAI' : 'Flow') : 'Traditional'
    ...
    logger.error("getPlandetails err",error);
  }
};
 ...
 */
/* tslint:disable */
export const getPlandetailsInsta = (plan_details: any) => {
  try {
    if (plan_details?.planId === 32) { //Xaia1Plus
      return 'Traditional'
    } else if (plan_details?.planId === 141) { //Xaia2Plus
      return plan_details?.isBot === 0 && plan_details?.InstagramFlowId === 0 ? 'Traditional' : 'CAI'
    } else if (plan_details?.planId === 33) { //Xaia3Plus
      return plan_details?.isBotPurchased === 1 && plan_details?.isBot === 1 ? (plan_details?.InstagramFlowId === 0 ? 'CAI' : 'Flow') : 'Traditional'
    } else if (plan_details?.planId === 42) { //Xaia4Plus
      return plan_details?.isBot === 0 && plan_details?.InstagramFlowId === 0 ? 'Traditional' : 'CAI'
    } else if (plan_details?.planId === 54) { //Xaia1
      return 'Traditional'
    ...
    logger.error("getPlandetailsInsta err",error);
  }
};
...
export async function sendEmailPlainText(toEmail: any, templateData: any, req: any, adminEmail: any) {
  try {
    ...
    return { success: true, message:RESPONSE?.emailSent };
  } catch (error) {
...
// send Email PlainText
export async function sendEmailPlainTextFB(toEmail: any, templateData: any, req: any, adminEmail: any) {
  try {
    const connect: any = await getconnectDataFB(req);
    let checkArray: any[] = [];
    if (connect?.connectorData) {
      const parsedData = JSON.parse(connect.connectorData);
      checkArray = Array.isArray(parsedData) ? parsedData : [parsedData];
    ...
    logger.error('sendEmailPlainTextFB err', error);
...
export const correctjsondata = async (btninputCount: any) => {
  try {
    ...
    logger.error('correctjsondata err', error);
    return null;
  }
};
...
export const contextMatch = (str: any) => {
  try {
    ...
    logger.error('contextMatch err', error);  }
};
...
export const callSessionExpiryAPI = async (data: any) => {
  ...
  try {
    ...
    logger.error('❌ Error calling session expiry API:', error.response?.data || error.message);
    throw error;
  }
};
...
// crm contact create
export async function insertUpdateCrmContact(data: any) {
  try {
    logger.info(`insertUpdateCrmContact socialmedia`, `${process.env.core_service}/crm_insert_contact - ${JSON.stringify(data)}`);

    let body: any = {};
    if (CRM_DOMAINS.LORIO === `${data?.domainId}`) {
      let customer: any = {}

      try {
        const customerList = data?.customerFacebookId ? await getCustomerCoId(data) : await getCustomerCoIdInsta(data);
        customer = customerList?.[0] ?? {};
      } catch (error) {
        console.log(error)
      }

      if (customer?.primaryPhoneNumber || data?.phone) {
        body.name = `${data?.firstName || customer?.firstName || ""} ${data?.lastName || customer?.lastName || ""}`?.trim();
        body.phone = data.phone || customer?.primaryPhoneNumber || "";
        body.email = data.email || customer?.primaryEmail || "";
...
export function lorioTeleCrm(data: any) {
  try {
    axios.post(`${process.env.core_service}/crm_insert_contact`, data)
      .catch((err) => {
        logger.error('insertUpdateCrmContact failed:', err);
      });
  } catch (error) {
    logger.error('insertUpdateCrmContact failed:', error);
  }
}
