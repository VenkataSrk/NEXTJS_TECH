...
pthread_mutex_t appLock=PTHREAD_MUTEX_INITIALIZER;
...
static GstFlowReturn gst_file_sink_render_list_internal (GstFileSink * sink, GstBufferList * buffer_list);
...
gst_file_sink_dispose (GObject * object)
{
  ...
  g_print("I am in filesink dispose\n");
  g_free (sink->uri);
  sink->uri = NULL;
    ...
    GError ** error)
{
 // if (sink->file)
 //   goto was_open;
  g_print("I am in file sink set location\n");
  g_free (sink->filename);
  g_free (sink->uri);
  if (location != NULL) {
 ...
 if(sink->current_buffer_size !=0 ){
    if(sink->current_buffer_size < sink->buffer_size){
        GST_DEBUG_OBJECT(sink,"gst_file_sink_set_location pos: %lu ,current_buffer_size:%d",sink->current_pos ,sink->current_buffer_size);
        guint length = gst_buffer_list_length (sink->buffer);
        if (length > 0) {
            GST_DEBUG_OBJECT (sink, "Flushing out buffer of size %u",sink->current_buffer_size);
            gst_file_sink_render_list_internal (sink, sink->buffer);
            gst_buffer_list_remove (sink->buffer, 0, length);
...
gst_file_sink_open_file (GstFileSink * sink)
{
  ...
  if (sink->file == NULL)
    goto open_failed;
  GST_LOG_OBJECT (sink,"gst_file_sink_open_file");
  sink->current_pos = 0;
...
gst_file_sink_query (GstBaseSink * bsink, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_POSITION:
      GST_LOG_OBJECT (bsink,"gst_file_sink_query, GST_QUERY_POSITION");
      gst_query_parse_position (query, &format, NULL);

      switch (format) {
        case GST_FORMAT_DEFAULT:
        case GST_FORMAT_BYTES:
      GST_LOG_OBJECT (bsink,"gst_file_sink_query,GST_FORMAT_BYTES");
    ...
    case GST_QUERY_FORMATS:
      GST_LOG_OBJECT (bsink,"gst_file_sink_query,GST_QUERY_FORMATS");
      gst_query_set_formats (query, 2, GST_FORMAT_DEFAULT, GST_FORMAT_BYTES);
      res = TRUE;
      break;

    case GST_QUERY_URI:
      GST_LOG_OBJECT (bsink,"gst_file_sink_query, GST_QUERY_URI");
      gst_query_set_uri (query, self->uri);
      res = TRUE;
      break;

    case GST_QUERY_SEEKING:
      GST_LOG_OBJECT (bsink,"gst_file_sink_query, GST_QUERY_SEEKING");
...
gst_file_sink_event (GstBaseSink * sink, GstEvent * event)
{
  ...
  switch (type) {
    case GST_EVENT_SEGMENT:
    {
      ...
      GST_LOG_OBJECT (filesink,"gst_file_sink_event,GST_EVENT_SEGMENT");
    ...
    case GST_EVENT_FLUSH_STOP:
      GST_LOG_OBJECT (filesink,"gst_file_sink_event,GST_EVENT_FLUSH_STOP");
      ...
      break;
    case GST_EVENT_EOS:
      GST_LOG_OBJECT (filesink,"gst_file_sink_event,GST_EVENT_EOS");
      if (gst_file_sink_flush_buffer (filesink) != GST_FLOW_OK)
        goto flush_buffer_failed;
      break;
    default:
    ...
    guint num_buffers, guint8 * mem_nums, guint total_mems, gsize size)
{
 GST_DEBUG_OBJECT (sink,
  ...
  GstFlowReturn Ret = gst_writev_buffers (GST_OBJECT_CAST (sink), fileno (sink->file), NULL,
...
gst_file_sink_render_list (GstBaseSink * bsink, GstBufferList * buffer_list)
{
  ...
  if (sync_after || !sink->buffer) {
    GST_LOG_OBJECT (sink,"gst_file_sink_render_list,if buffer is not created");
    ...
    GST_LOG_OBJECT (sink,"gst_file_sink_render_list");
    ...
    if (sink->current_buffer_size > sink->buffer_size){
    GST_LOG_OBJECT (sink,"gst_file_sink_render_list,current_buffer_size > buffer_size");
      flow = gst_file_sink_flush_buffer (sink);
    }
    else
      flow = GST_FLOW_OK;
...
gst_file_sink_render (GstBaseSink * sink, GstBuffer * buffer)
{
  ...
  if (n_mem > 0 && (sync_after || !filesink->buffer)) {
      ...
      GST_LOG_OBJECT(filesink,"gst_file_sink_render,if buffer not created");
    ...
    if (filesink->current_buffer_size > filesink->buffer_size){
      GST_LOG_OBJECT (filesink, "current_buffer_size:%d > buffer_size %d",filesink->current_buffer_size,filesink->buffer_size); 
      flow = gst_file_sink_flush_buffer (filesink);
      }
    else
      flow = GST_FLOW_OK;
